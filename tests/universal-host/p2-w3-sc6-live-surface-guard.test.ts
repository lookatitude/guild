...
const PINNED_BASELINE = "4a64b9b"; // RE-RATIFIED 2026-07-17 (operator-ratified) for the deliberate task-cell-runtime G6b-2b surface change. Two live-surface facts are ratified here: (1) guild-quality + guild-operations no longer hardcode a fixed [security, architect] challenger pair — the trail now DERIVES from the station composer's advisory_panel, where security is BASELINE (always-on safety floor) and architect is GATED on the multi_component signal; (2) team-compose / plan / execute-plan additionally emit the companion typed guild.team_plan.v1 + guild.team_result.v1 artifacts (the .guild/team yaml stays authoritative for dispatch). Commit 4a64b9b is the LAST commit on this branch that changes the frozen surface (skills/** only; .claude-plugin/** + commands/** are untouched), is an ancestor of HEAD, is not HEAD, and leaves ZERO delta — per the RE-RATIFICATION RULE above. NOTE: the prior pin 631e1b5 was orphaned by #37's squash-merge and is no longer an ancestor of next, which left this guard failing on next independently of this branch; re-pin to the merged SHA after this PR squash-merges.
