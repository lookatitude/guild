...
Writing-group principle reminder: **match existing voice** before imposing a new one. Read recent posts from the account first and calibrate tone, hashtag habits, emoji use, and cadence. Don't rewrite adjacent scheduled posts the user didn't scope in. Evidence = a scannable sample the reader can skim in one pass — including seeing each post in its intended platform format.
