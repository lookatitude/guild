...
Detail for `guild:review` Stages 1–2.
...
2. **Are `assumptions` within the autonomy policy?** The plan records a per-lane autonomy tier. An assumption that exceeds the tier (e.g., a lane on "implement-only" tier inventing new API surface) is a Stage 1 failure. An assumption within tier is fine.
