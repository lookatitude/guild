...
function checkContained(root, target, options = {}) {
  ...
  let realRoot;
  try {
    realRoot = fs.realpathSync(path2.resolve(root));
  } catch {
...
var fs, path2, CONTAINMENT_REFUSAL_CODES;
...
function defaultProbeEnv(cwd) {
  return {
    ...
    readStoredCodexAuth: () => safe(() => {
      const home = process.env["CODEX_HOME"] || path4.join(os2.homedir(), ".codex");
...
var import_child_process2, fs3, os2, path4, PROVIDER_REGISTRY, KNOWN_FAMILIES;
...
function writeSessionContext(root, ctx, fs30) {
  const f = fs30 ?? realFs();
...
function loadSessionContext(root, runId, fs30) {
  const f = fs30 ?? realFs();
...
function readRunBindingRecord(opts) {
  const fs30 = opts.fs ?? realBindingFs();
  const raw = fs30.readFile(runBindingPath(opts.root, opts.run_id));
...
function reopenRunBinding(opts, binding_ref) {
  ...
  fs30.writeFile(runBindingPath(opts.root, opts.run_id), JSON.stringify(reopened, null, 2) + "\n");
  return reopened;
}
...
function resolveGitDir(gitPath, fsi = fs2) {
  try {
    ...
    if (st.isFile()) {
      ...
      return path3.isAbsolute(target) ? target : path3.resolve(path3.dirname(gitPath), target);
    }
  } catch {
  }
  return null;
}
function readGitHead(gitDir, fsi = fs2) {
  ...
  if (fsi.existsSync(commondirFile)) {
    ...
    refRoots.push(path3.isAbsolute(common) ? common : path3.resolve(gitDir, common));
  }
  for (const root of refRoots) {
    const looseRef = path3.join(root, "refs", "heads", branch);
    if (fsi.existsSync(looseRef)) {
      const sha = fsi.readFileSync(looseRef, "utf8").trim();
      return { branch, sha: /^[0-9a-f]{40}$/.test(sha) ? sha : null };
    }
    const packed = path3.join(root, "packed-refs");
...
function resolveInstallState(pluginRoot, opts = {}) {
  ...
  const managedRoots = [
    path3.join(home, ".claude", "plugins"),
    path3.join(home, ".config", "claude", "plugins")
  ].map((m) => {
    try {
  ...
  const isManaged = managedRoots.some(
    (m) => real === m || real.startsWith(m + path3.sep)
  );
  const gitDir = resolveGitDir(path3.join(real, ".git"), fsi);
  if (gitDir) {
  ...
  const receiptPath = path3.join(real, RECEIPT_BASENAME);
...
function readInstalledVersion(pluginRoot, fsi = fs2) {
  for (const [dir, file] of PLUGIN_MANIFEST_CANDIDATES) {
    try {
      const manifest = JSON.parse(fsi.readFileSync(path3.join(pluginRoot, dir, file), "utf8"));
...
function cachePath(homedir4 = os.homedir()) {
  return path3.join(homedir4, ".guild", "update-check.json");
}
function readCache(file, fsi = fs2) {
  try {
...
function refreshCache(opts) {
  ...
  try {
    ...
    fsi.mkdirSync(path3.dirname(file), { recursive: true });
...
function alreadyStaged(target) {
  try {
    const m = JSON.parse(fs29.readFileSync(stagedMarkerPath(), "utf8"));
