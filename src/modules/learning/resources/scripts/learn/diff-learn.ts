...
/**
 * learn/diff-learn.ts — P2 diff blast-radius (SCRIPT, no LLM).
 *
 * Changed files (git base..head) → affected graph nodes → affected layers →
 * blast-radius/risk → .guild/runs/<run-id>/diff-learn.json
 ...
 *
 * Usage:
 *   npx tsx diff-learn.ts [--cwd <path>] --base <sha> [--head <sha>] [--run-id <id>] [--print]
 */
...
function main(): void {
  ...
  const outPath = path.join(gp.runsDir, runId, "diff-learn.json");
