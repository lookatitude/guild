...
function readReceiptFrontmatter(text) {
  ...
  let parsed;
  try {
    const yaml5 = loadYamlApi();
    parsed = yaml5.load(frontmatter, { schema: yaml5.JSON_SCHEMA });
  } catch {
...
function readSecurityConfig(cwd) {
  ...
  let raw;
  try {
    raw = fs11.readFileSync(settingsPath, "utf8");
  } catch {
...
function appendSecurityEvent(runDir3, record) {
  try {
    const logsDir = path14.join(runDir3, "logs");
    fs12.mkdirSync(logsDir, { recursive: true });
    fs12.appendFileSync(path14.join(logsDir, "security-events.jsonl"), JSON.stringify(record) + "\n", "utf8");
    return true;
  } catch (err) {
...
var fs12, path14, SECURITY_EVENT_SCHEMA_VERSION, KNOWN_GUILD_HOST_KINDS, KNOWN_GUILD_HOST_ID_SET, LEGACY_HOST_ALIASES2;
...
function writeScrubApprovalRequest(runDir3, runId, surface, outPath, laneId2) {
  try {
    const approvalDir = path15.join(runDir3, "agent-bus", "approvals");
    fs13.mkdirSync(approvalDir, { recursive: true });
    ...
    const record = {
      ...
      run_id: runId,
      tool: "scrubbedWrite",
      reason: `Secret scrub failed for durable surface "${surface}" \u2014 write blocked. Human review required. Path: ${path15.basename(outPath)}`,
    ...
    fs13.writeFileSync(path15.join(approvalDir, fileName), content, "utf8");
  } catch {
  }
}
function scrubbedWrite(outPath, content, opts) {
  ...
  if (scrubResult.ok) {
    try {
      ...
      fs13.writeFileSync(outPath, scrubResult.value, "utf8");
    } catch (err) {
      process.stderr.write(
...
`
    ...
    if (opts.surface === "bus") {
      result.sha256 = crypto3.createHash("sha256").update(scrubResult.value, "utf8").digest("hex");
    }
    return result;
  }
  if (failMode === "open") {
    process.stderr.write(
      `[scrubbed-write] WARN: secret scrub custom-pattern failure for surface "${opts.surface}" at ${path15.basename(outPath)} \u2014 writing built-in-redacted content (fail-open). Failures: ${scrubResult.failures.join("; ")}
`
    );
    try {
      fs13.mkdirSync(path15.dirname(outPath), { recursive: true });
...
`
    ...
    try {
      const evt = buildSecurityEvent({
        ...
        decision: "degraded",
        tool: "scrubbedWrite",
        detail: `Secret scrub custom-pattern failure (fail-open) for surface "${opts.surface}" at ${path15.basename(outPath)}. Built-in-redacted content written.`,
...
`
  ...
  try {
    const evt = buildSecurityEvent({
      ...
      decision: "blocked",
      tool: "scrubbedWrite",
      detail: `Secret scrub failed for durable surface "${opts.surface}" at ${path15.basename(outPath)} \u2014 write blocked (fail-closed).`,
      permission_mode: "blocked"
    });
    appendSecurityEvent(opts.runDir, evt);
  } catch {
...
function genSpanId(runId, eventType, ts, actorId) {
  ...
  return crypto4.createHash("sha256").update(material).digest("hex").slice(0, 16);
}
...
var crypto4, SIDECAR_MAX_BYTES2;
...
function readSentinel(cwd) {
  const sentinelPath = path17.join(cwd, ".guild", "runs", "current-run-id");
  try {
...
var fs14, path17, VALID_EVENTS, VALID_LAYERS, VALID_TERMINATED;
...
function loadRunState(runDir3) {
  let raw;
  try {
    raw = fs15.readFileSync(runStatePath(runDir3), "utf8");
  } catch {
    return null;
  }
  let parsed;
...
function readResumeEnabled(cwd) {
  ...
  try {
    const raw = fs15.readFileSync(settingsPath, "utf8");
    const parsed = JSON.parse(raw);
    const defs = parsed["defaults"];
...
function loadLaneResumeCheckpoint(runDir3, laneId2) {
  try {
    const raw = fs15.readFileSync(laneResumeCheckpointPath(runDir3, laneId2), "utf8");
    const parsed = JSON.parse(raw);
...
function markLaneDead(runDir3, init2, laneId2, signal, cwd) {
  ...
  if (readResumeEnabled(cwd)) {
    ...
    fs15.mkdirSync(path18.dirname(checkpointPath), { recursive: true });
    fs15.writeFileSync(checkpointPath, JSON.stringify(checkpoint, null, 2) + "\n", "utf8");
  }
  return state;
}
var fs15, path18, RUN_STATE_SCHEMA_VERSION, LANE_RESUME_SCHEMA_VERSION;
...
function readPlanOwnerTaskIds(guildRoot, slug) {
  ...
  let raw;
  try {
    raw = fs16.readFileSync(path20.join(guildRoot, ".guild", "plan", `${slug}.md`), "utf8");
  } catch {
    return map;
  }
...
var crypto5, RELEASE_CLAIM_SCHEMA, OPERATION_KINDS, ACCEPTED_CONFORMANCE_ARTIFACTS;
...
function canonicalJournalPath(journalPath2) {
  let current = path22.resolve(journalPath2);
  ...
  "src/modules/telemetry/workflows/receipt-journal.ts"() {
    ...
    defaultJournalIo = {
      appendLine(journalPath2, text, bound) {
        ...
        fs17.mkdirSync(path22.dirname(journalPath2), { recursive: true });
        const fd = fs17.openSync(journalPath2, "a");
        try {
      ...
      readAll(journalPath2, bound) {
        ...
        try {
          return fs17.readFileSync(journalPath2, "utf8");
        } catch {
          return null;
        }
      },
      ...
      truncate(journalPath2, size, bound) {
        ...
        fs17.truncateSync(journalPath2, size);
      },
      ...
      acquireLock(lockPath2) {
        fs17.mkdirSync(path22.dirname(lockPath2), { recursive: true });
        try {
          fs17.mkdirSync(lockPath2);
        } catch (err) {
...
function isExistingDirectory(target) {
  try {
    return fs18.statSync(target).isDirectory();
  } catch {
    return false;
  }
}
...
function evaluateReceiptJournalConformance(request) {
  ...
  const usableRoot = typeof root === "string" && root.length > 0 && path23.isAbsolute(root) && isExistingDirectory(root);
  if (!usableRoot) {
    return refuseEvaluation(MH06_REFUSAL_CONTROLS.journalRootUnusable, "scenario_evidence_incomplete", [
  ...
  const identity = request.evidence_identity;
  const workspace = fs18.mkdtempSync(path23.join(root, PROBE_WORKSPACE_PREFIX));
  const verdicts = {};
  try {
    for (const stableId of MH06_SCENARIO_IDS) {
...
var fs18, path23, OUTCOME_ENVELOPE_SCHEMA, OUTCOME_CONTRACT_VERSION, MH06_SUITE_ID, MH06_SUITE_VERSION, MH06_OWNER_KEY, MH06_PACKET_SCHEMA, MH06_SCENARIO_IDS, MH06_CATEGORY, MH06_EVIDENCE_PROFILE, MH06_EXPECTED_OUTCOMES, SCENARIO_RESULT_MISMATCH, EVIDENCE_FRESHNESS_VERDICTS, EVIDENCE_IDENTITY_FIELDS, MH06_WAVE_OWNER, MH06_SCENARIOS, MH06_PRODUCTION_JOURNAL, MH06_REFUSAL_CONTROLS, MH06_SOURCE_VERSION, PROBE_RECORDED_AT, PROBE_RECONCILED_AT, PROBE_WORKSPACE_PREFIX, TORN_APPEND_CUT_BYTES, PROBE_DIRS, JOURNAL_LEAF, CHECKPOINT_LEAF, CHECKPOINT_SCHEMA, PROBES;
...
`;
  const coreDir = path25.resolve(__dirname, "../../lifecycle/workflows");
  const coreWorkflows = {};
  for (const member of NEUTRAL_CORE_MEMBERS) {
    coreWorkflows[member] = fs20.readFileSync(path25.join(coreDir, member), "utf8");
  }
  ...
  for (const [rel, content] of Object.entries(files)) {
    const target = path25.join(base, rel);
    fs20.mkdirSync(path25.dirname(target), { recursive: true });
    fs20.writeFileSync(target, content);
  }
}
function batteryWorkspace() {
  if (batteryWorkspaceSingleton !== null) return batteryWorkspaceSingleton;
  const base = fs20.mkdtempSync(path25.join(os2.tmpdir(), "a21x-integration-battery-"));
  const journalRoot = path25.join(base, "journal-root");
  const migrationJournalRoot = path25.join(base, "migration-journal-root");
  fs20.mkdirSync(journalRoot, { recursive: true });
  fs20.mkdirSync(migrationJournalRoot, { recursive: true });
  const moduleBase = path25.join(base, "module-workspace");
  buildBatteryModuleWorkspace(moduleBase);
  batteryWorkspaceSingleton = {
    journalRoot,
    migrationJournalRoot,
    pluginRoot: path25.join(moduleBase, "plugin"),
    consumerRoots: {
      website: path25.join(moduleBase, "website"),
      benchmark: path25.join(moduleBase, "benchmark")
    }
  };
...
function scanResumableLanes(runDir3, cwd, slug) {
  ...
  const lanesDir = path26.join(runDir3, "lanes");
  let entries;
  try {
    entries = fs21.readdirSync(lanesDir, { withFileTypes: true });
  } catch {
    return [];
  }
...
function manifestPathFor(cwd, slug) {
  return path27.join(cwd, ".guild", "programs", slug, "manifest.json");
}
function readRunManifest(cwd, slug) {
  try {
    const raw = fs22.readFileSync(manifestPathFor(cwd, slug), "utf8");
...
function runWriteRunManifestCli(argv = process.argv.slice(2)) {
  ...
  if (!fs22.existsSync(args.cwd) || !fs22.statSync(args.cwd).isDirectory()) {
...
var fs22, path27, WAVE_STATUSES, PROGRAM_STATUSES;
...
function taskRunPath(cwd, runId, taskId) {
  return path28.join(cwd, ".guild", "runs", runId, "task-runs", `${taskId}.yaml`);
}
function writeTaskRun(cwd, runId, taskId, params) {
  ...
  try {
    const _traceRunDir = path28.join(cwd, ".guild", "runs", runId);
...
function resolvedSettingsPath(root, runId) {
  return path29.join(runDir2(root, runId), "resolved-settings.json");
}
...
function assertContained(target, cwd, label) {
  ...
  if (isRefused(r)) {
    throw new Error(
      `[run-lifecycle] ${label}: resolved path "${path29.resolve(target)}" escapes the project root "${path29.resolve(cwd)}" [${r.code}] \u2014 ${r.detail}`
    );
  }
  const runsBase = path29.resolve(cwd, ".guild", "runs");
...
function readResolvedSettingsSnapshot(runId, opts) {
  ...
  const fs25 = fsSeam ?? realProvenanceFsSeam();
  ...
  const raw = fs25.readFile(filePath);
...
var fsNode, path29, HOST_FAMILY_TO_KIND, CANONICAL_PHASES;
...
function resolveActiveRunId(guildRoot) {
  ...
  const sentinel = path30.join(guildRoot, ".guild", "runs", "current-run-id");
  try {
    const value = fs23.readFileSync(sentinel, "utf8").trim();
    return value.length > 0 ? value : void 0;
  } catch {
    return void 0;
  }
...
function readRunYamlFacts(guildRoot, runId) {
  const runYamlPath = path30.join(guildRoot, ".guild", "runs", runId, "run.yaml");
  let raw;
  try {
    raw = fs23.readFileSync(runYamlPath, "utf8");
  } catch {
    return null;
  }
  let doc;
...
function newestRunSignalMs(guildRoot, runId) {
  const dir = path30.join(guildRoot, ".guild", "runs", runId);
  const candidates = [
    path30.join(dir, "run.yaml"),
    path30.join(dir, "events.ndjson"),
    path30.join(dir, "provenance.json")
  ];
  for (const sub of ["logs", "handoffs", "in-progress"]) {
    try {
      for (const name of fs23.readdirSync(path30.join(dir, sub))) {
        candidates.push(path30.join(dir, sub, name));
      }
    } catch {
    }
  }
  ...
  for (const p of candidates) {
    try {
      newest = Math.max(newest, fs23.statSync(p).mtimeMs);
    } catch {
    }
  }
  return newest;
...
`
  ...
  const runDir3 = process.env["GUILD_RUN_DIR"] ?? path31.join(guildRoot, ".guild", "runs", runId);
  const ts = (/* @__PURE__ */ new Date()).toISOString();
  const event = {
    ts,
    event: "hook_event",
    ...
    latency_ms: 0,
    status: "ok",
    ...laneId2 !== void 0 ? { lane_id: laneId2 } : {}
  };
  const traceV2 = resolveTraceV2Fields({
    runId,
    eventType: "hook_event",
    ts,
    actorId: laneId2 ?? "main"
  });
  try {
    appendEvent(runDir3, event, { traceV2 });
  } catch (err) {
    process.stderr.write(
