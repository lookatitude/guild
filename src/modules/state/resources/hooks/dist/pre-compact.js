...
function deriveNextGate(phase, passedGates) {
  const seq2 = phase ? GATE_SEQUENCE_BY_PHASE[phase] : void 0;
  if (!seq2 || seq2.length === 0) return null;
  for (const gate of seq2) {
    if (!passedGates.has(gate)) return gate;
  }
  return seq2[seq2.length - 1];
}
