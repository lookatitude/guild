...
async function readArchive(path14) {
  const chunks = [];
  const src = (0, import_node_fs2.createReadStream)(path14);
...
function readRunYamlFacts(guildRoot, runId) {
  const runYamlPath = path12.join(guildRoot, ".guild", "runs", runId, "run.yaml");
  let raw;
  try {
    raw = fs10.readFileSync(runYamlPath, "utf8");
  } catch {
    return null;
  }
  let doc;
...
function newestRunSignalMs(guildRoot, runId) {
  const dir = path12.join(guildRoot, ".guild", "runs", runId);
  const candidates = [
    path12.join(dir, "run.yaml"),
    path12.join(dir, "events.ndjson"),
    path12.join(dir, "provenance.json")
  ];
  for (const sub of ["logs", "handoffs", "in-progress"]) {
    try {
      for (const name of fs10.readdirSync(path12.join(dir, sub))) {
        candidates.push(path12.join(dir, sub, name));
      }
    } catch {
    }
  }
  ...
  for (const p of candidates) {
    try {
      newest = Math.max(newest, fs10.statSync(p).mtimeMs);
    } catch {
    }
  }
  return newest;
...
function cliMain(argv) {
  ...
  if (!(0, import_node_fs3.existsSync)(path14)) {
    process.stderr.write(`error: file not found: ${path14}
`);
    return 2;
  }
  const text = (0, import_node_fs3.readFileSync)(path14, "utf8");
  const summary = validateText(text);
  if (summary.invalid > 0) {
    for (const { line, result } of summary.perLine) {
      if (!result.ok) {
        for (const e of result.errors) {
          process.stderr.write(`${path14}:${line}: ${e}
`);
        }
      }
    }
...
function gateStatePath(runDir) {
  return path13.join(runDir, "lifecycle-gate-state.json");
}
function closeStatePath(runDir) {
  return path13.join(runDir, "lifecycle-close-state.json");
}
function loadGateState(runDir) {
  try {
    const raw = fs11.readFileSync(gateStatePath(runDir), "utf8");
    const parsed = JSON.parse(raw);
...
function loadCloseState(runDir) {
  ...
  try {
    const raw = fs11.readFileSync(closeStatePath(runDir), "utf8");
    const parsed = JSON.parse(raw);
...
function writeJsonAtomic(finalPath, value) {
  fs11.mkdirSync(path13.dirname(finalPath), { recursive: true });
  const tmpPath = `${finalPath}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  fs11.writeFileSync(tmpPath, JSON.stringify(value, null, 2) + "\n", "utf8");
  try {
...
async function resolveGateContext(guildRoot, runId, env) {
  ...
  const runDir = env["GUILD_RUN_DIR"] ?? path13.join(guildRoot, ".guild", "runs", safeRunId);
...
function hasContent(filePath) {
  try {
    ...
    return fs11.readFileSync(filePath, "utf8").trim().length > 0;
  } catch {
    return false;
  }
}
async function evaluateCloseGate(guildRoot, runId, env = process.env) {
  ...
  for (const ref of ctx.runState.receiptRefs) {
    ...
    const rel = path13.relative(handoffsDir, resolved);
    if (rel === "" || rel.startsWith("..") || path13.isAbsolute(rel)) return silent;
