...
function runStatePath(runDir3) {
  return path15.join(runDir3, "run-state.json");
}
function loadRunState(runDir3) {
  let raw;
  try {
    raw = fs12.readFileSync(runStatePath(runDir3), "utf8");
  } catch {
    return null;
  }
  let parsed;
...
function writeRunStateAtomic(runDir3, state) {
  fs12.mkdirSync(runDir3, { recursive: true });
...
function laneResumeCheckpointPath(runDir3, laneId2) {
  return path15.join(runDir3, "lanes", laneId2, "resume.json");
}
...
function loadLaneResumeCheckpoint(runDir3, laneId2) {
  try {
    const raw = fs12.readFileSync(laneResumeCheckpointPath(runDir3, laneId2), "utf8");
    const parsed = JSON.parse(raw);
...
function liveLogPath2(runDir3) {
  return path21.join(runDir3, "logs", "v1.4-events.jsonl");
}
...
function scanResumableLanes(runDir3, cwd, slug) {
  ...
  const lanesDir = path23.join(runDir3, "lanes");
  let entries;
  try {
...
function liveLogPath3(root, runId) {
  return path29.join(runDir2(root, runId), "logs", "v1.4-events.jsonl");
}
...
function appendTraceLine(file, event) {
  fs21.mkdirSync(path29.dirname(file), { recursive: true });
  fs21.appendFileSync(file, JSON.stringify(event) + "\n", "utf8");
}
function readTraceLines(file) {
  try {
    return fs21.readFileSync(file, "utf8").split("\n").filter((l) => l.trim().length > 0).map((l) => JSON.parse(l));
  } catch {
    return [];
  }
}
