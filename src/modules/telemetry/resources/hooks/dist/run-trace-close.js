...
var crypto4, fsNode, path14, CANONICAL_PHASES;
...
function readProvenance(issues, parent, path26) {
  ...
  const provenancePath3 = `${path26}.provenance`;
...
function initStableLockfile(runDir4) {
  const path26 = stableLockPath(runDir4);
...
function appendEvent(runDir4, event, opts = {}) {
  ...
  if (opts.forceFallback || process.platform === "win32") {
    ...
    const path26 = laneFallbackPath(runDir4, laneId2);
...
function appendKnowledgeLinksIndex(guildRoot, links) {
  ...
  const indexDir = path23.join(guildRoot, ".guild", "indexes");
  const indexPath = path23.join(indexDir, "knowledge-links.json");
  let existing = [];
...
function appendReflections(guildRoot, runId, phase, decisions) {
  ...
  const reflectionsDir = path23.join(guildRoot, ".guild", "reflections");
  fs17.mkdirSync(reflectionsDir, { recursive: true });
  const reflPath = path23.join(reflectionsDir, `${runId}.md`);
  const entry = `
## Phase: ${phase} (${runId})
...
function writeCheckpoint(opts) {
  ...
  const learningDir = path23.join(guildRoot, ".guild", "runs", opts.runId, "learning");
  fs17.mkdirSync(learningDir, { recursive: true });
  const checkpointFile = path23.join(learningDir, `${opts.phase}-${opts.runId}.yaml`);
  const reflectionsRelPath = `.guild/reflections/${opts.runId}.md`;
  const reflectionsAbsPath = path23.join(guildRoot, ".guild", "reflections", `${opts.runId}.md`);
  ...
  fs17.writeFileSync(checkpointFile, yaml2, "utf8");
...
function liveLogPath3(root, runId) {
  return path24.join(runDir3(root, runId), "logs", "v1.4-events.jsonl");
}
function provenancePath2(root, runId) {
  return path24.join(runDir3(root, runId), "provenance.json");
}
...
function appendTraceLine(file, event) {
  fs18.mkdirSync(path24.dirname(file), { recursive: true });
...
function newestPostCloseActivityMs(root, runId) {
  ...
  const candidates = [liveLogPath3(root, runId), path24.join(dir, "events.ndjson")];
  try {
    const inProgress = path24.join(dir, "in-progress");
    for (const name of fs18.readdirSync(inProgress)) {
      candidates.push(path24.join(inProgress, name));
    }
  } catch {
  }
  let newest = 0;
...
async function readHookStdin() {
  return new Promise((resolve11) => {
    ...
    process.stdin.on("end", () => resolve11(Buffer.concat(chunks).toString("utf8")));
    process.stdin.on("error", () => resolve11(""));
  });
}
...
function findTerminalCheckpoint(runDir4, runId) {
  const learningDir = path25.join(runDir4, "learning");
  if (!fs19.existsSync(learningDir)) return null;
  const reflectionFile = path25.join(learningDir, `reflection-${runId}.yaml`);
  ...
  const sorted = yamlFiles.map((f) => path25.join(learningDir, f)).sort((a, b) => {
    try {
...
async function main2() {
  ...
  const runDir4 = path25.join(root, ".guild", "runs", runId);
  if (!fs19.existsSync(path25.join(runDir4, "run.yaml"))) process.exit(0);
  const provenanceFile = path25.join(runDir4, "provenance.json");
