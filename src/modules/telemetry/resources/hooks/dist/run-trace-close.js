...
function runYamlPath(root, runId) {
  return path9.join(runDir(root, runId), "run.yaml");
}
function provenancePath(root, runId) {
  return path9.join(runDir(root, runId), "provenance.json");
}
function logsDir(root, runId) {
  return path9.join(runDir(root, runId), "logs");
}
function resolvedSettingsPath(root, runId) {
  return path9.join(runDir(root, runId), "resolved-settings.json");
}
function sentinelPath(root) {
  return path9.join(root, ".guild", "runs", "current-run-id");
}
  ...
  return {
    ...
    closeRun(runId, opts) {
      ...
      if (env.fs.scrubbedWriteDurable) {
        const runDir3 = path9.join(root, ".guild", "runs", runId);
...
function writeResolvedSettingsSnapshot(runId, snapshot, opts) {
  ...
  const fs11 = fsSeam ?? realProvenanceFsSeam();
  ...
  if (fs11.scrubbedWriteDurable) {
    const runDir3 = path9.join(cwd, ".guild", "runs", runId);
    const result = fs11.scrubbedWriteDurable(outPath, serialized, "config", runDir3, runId);
    if (result.blocked) {
      process.stderr.write(
...
`
    ...
    fs11.writeFile(outPath, serialized);
  }
  return outPath;
}
...
function writeCheckpoint(opts) {
  ...
  fs8.mkdirSync(learningDir, { recursive: true });
  ...
  fs8.writeFileSync(checkpointFile, yaml2, "utf8");
...
function main() {
  ...
  if (verdictPath) {
    try {
      const raw = fs8.readFileSync(verdictPath, "utf8");
...
`
  ...
  if (decisions === void 0 && artifactsJsonPath) {
    try {
      const rawArtifacts = fs8.readFileSync(artifactsJsonPath, "utf8");
...
`
  ...
  if (linksPath) {
    try {
      const raw = fs8.readFileSync(linksPath, "utf8");
      const parsed = JSON.parse(raw);
...
function liveLogPath(root, runId) {
  return path11.join(runDir2(root, runId), "logs", "v1.4-events.jsonl");
}
function provenancePath2(root, runId) {
  return path11.join(runDir2(root, runId), "provenance.json");
}
...
function emitRunClosed(root, runId, resolveHost, opts = {}) {
  try {
    ...
    const prov = JSON.parse(fs9.readFileSync(provenancePath2(root, runId), "utf8"));
...
function newestPostCloseActivityMs(root, runId) {
  ...
  const candidates = [liveLogPath(root, runId), path11.join(dir, "events.ndjson")];
  try {
    const inProgress = path11.join(dir, "in-progress");
    for (const name of fs9.readdirSync(inProgress)) {
      candidates.push(path11.join(inProgress, name));
    }
  } catch {
  }
  let newest = 0;
...
function findTerminalCheckpoint(runDir3, runId) {
  const learningDir = path12.join(runDir3, "learning");
  if (!fs10.existsSync(learningDir)) return null;
  const reflectionFile = path12.join(learningDir, `reflection-${runId}.yaml`);
  if (fs10.existsSync(reflectionFile)) return reflectionFile;
  let yamlFiles;
  try {
    yamlFiles = fs10.readdirSync(learningDir).filter((f) => f.endsWith(".yaml"));
  } catch {
    return null;
  }
  if (yamlFiles.length === 0) return null;
  const sorted = yamlFiles.map((f) => path12.join(learningDir, f)).sort((a, b) => {
    try {
      return fs10.statSync(b).mtimeMs - fs10.statSync(a).mtimeMs;
    } catch {
      return 0;
    }
  });
...
async function main2() {
  ...
  const runDir3 = path12.join(root, ".guild", "runs", runId);
  if (!fs10.existsSync(path12.join(runDir3, "run.yaml"))) process.exit(0);
  const provenanceFile = path12.join(runDir3, "provenance.json");
  if (fs10.existsSync(provenanceFile)) {
    let terminal = true;
    let closedAtMs = 0;
    try {
      const prov = JSON.parse(fs10.readFileSync(provenanceFile, "utf8"));
