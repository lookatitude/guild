...
var PROVIDER_REGISTRY, KNOWN_FAMILIES;
...
function buildSessionContext(input) {
  ...
  const verified = input.native_adapter ? { id: input.native_adapter, source: "native_adapter" } : input.handshake ? { id: input.handshake, source: "host_handshake" } : null;
  let host;
  let identity;
  if (asserted) {
    ...
    if (verified && verified.id.family !== assertedFamily) {
      ...
      identity = {
        ...
        trust: "verified",
        confidence: "high",
        evidence: `${verified.id.evidence}; asserted host "${asserted.value}" (${asserted.source}) REJECTED: contradicted by ${verified.source} evidence`
      };
    } else if (verified) {
      host = blockFromVerified(verified.id);
      identity = {
...
function sessionContextPath(root, runId) {
  return path4.join(root, ".guild", "runs", runId, "session-context.json");
}
function writeSessionContext(root, ctx, fs20) {
  const f = fs20 ?? realFs();
  ...
  f.mkdirp(path4.dirname(p));
  f.writeFile(p, serialized);
}
var fsReal, path4, UNKNOWN_TARGET;
...
function redactKeyValueSecrets(input) {
  return input.replace(
    KV_SECRET_PATTERN,
    (_match, key, sep4) => `${key}${sep4}${KV_REDACTED}`
  );
}
...
function appendSecurityEvent(runDir4, record) {
  try {
    const logsDir2 = path10.join(runDir4, "logs");
    fs8.mkdirSync(logsDir2, { recursive: true });
    fs8.appendFileSync(path10.join(logsDir2, "security-events.jsonl"), JSON.stringify(record) + "\n", "utf8");
    return true;
  } catch (err) {
...
var fs8, path10, SECURITY_EVENT_SCHEMA_VERSION, KNOWN_GUILD_HOST_KINDS, KNOWN_GUILD_HOST_ID_SET, LEGACY_HOST_ALIASES2;
...
function guildRootFromRunDir(runDir4) {
  return path11.resolve(runDir4, "../../..");
}
function writeScrubApprovalRequest(runDir4, runId, surface, outPath, laneId2) {
  try {
    const approvalDir = path11.join(runDir4, "agent-bus", "approvals");
    ...
    const record = {
      ...
      run_id: runId,
      tool: "scrubbedWrite",
      reason: `Secret scrub failed for durable surface "${surface}" \u2014 write blocked. Human review required. Path: ${path11.basename(outPath)}`,
    ...
    try {
      const secConfig = readSecurityConfig(guildRootFromRunDir(runDir4));
      const scrubResult = applySecretsPolicy(rawContent, secConfig.secrets_policy, { noTruncate: true });
      content = scrubResult.value;
    } catch {
    }
    fs9.writeFileSync(path11.join(approvalDir, fileName), content, "utf8");
  } catch {
  }
}
function scrubbedWrite(outPath, content, opts) {
  ...
  if (scrubResult.ok) {
    try {
      fs9.mkdirSync(path11.dirname(outPath), { recursive: true });
      fs9.writeFileSync(outPath, scrubResult.value, "utf8");
    } catch (err) {
      process.stderr.write(
...
`
  ...
  if (failMode === "open") {
    process.stderr.write(
      `[scrubbed-write] WARN: secret scrub custom-pattern failure for surface "${opts.surface}" at ${path11.basename(outPath)} \u2014 writing built-in-redacted content (fail-open). Failures: ${scrubResult.failures.join("; ")}
`
    );
    try {
      fs9.mkdirSync(path11.dirname(outPath), { recursive: true });
...
`
    ...
    try {
      const evt = buildSecurityEvent({
        ...
        decision: "degraded",
        tool: "scrubbedWrite",
        detail: `Secret scrub custom-pattern failure (fail-open) for surface "${opts.surface}" at ${path11.basename(outPath)}. Built-in-redacted content written.`,
...
`
  ...
  try {
    const evt = buildSecurityEvent({
      ...
      decision: "blocked",
      tool: "scrubbedWrite",
      detail: `Secret scrub failed for durable surface "${opts.surface}" at ${path11.basename(outPath)} \u2014 write blocked (fail-closed).`,
      permission_mode: "blocked"
    });
    appendSecurityEvent(opts.runDir, evt);
  } catch {
...
function liveLogPath(runDir4) {
  return path12.join(runDir4, "logs", "v1.4-events.jsonl");
}
...
`
  ...
  try {
    ...
    process.stderr.write(
      `[guild-trace-emit] WARN: could not write trace event to ${runDir4}/logs/v1.4-events.jsonl: ${err instanceof Error ? err.message : String(err)}
`
    );
    return false;
  }
...
function runBindingPath(root, runId) {
  return path13.join(root, ".guild", "runs", runId, "binding.json");
}
function mintRunBinding(opts) {
  const fs20 = opts.fs ?? realBindingFs();
  ...
  const record = {
    ...
    run_id: opts.run_id,
    binding_ref: `rb-${crypto3.randomBytes(16).toString("hex")}`,
    state: "open"
  };
  fs20.mkdirp(path13.dirname(p));
  fs20.writeFile(p, JSON.stringify(record, null, 2) + "\n");
  return record;
}
...
function readRunBindingRecord(opts) {
  const fs20 = opts.fs ?? realBindingFs();
  const raw = fs20.readFile(runBindingPath(opts.root, opts.run_id));
...
function reopenRunBinding(opts, binding_ref) {
  ...
  fs20.writeFile(runBindingPath(opts.root, opts.run_id), JSON.stringify(reopened, null, 2) + "\n");
  return reopened;
}
...
var crypto3, fsReal2, path13, BindingRejectedError, HOOK_BINDING_ENV_RUN_ID, HOOK_BINDING_ENV_BINDING_REF;
...
function runYamlPath(root, runId) {
  return path14.join(runDir2(root, runId), "run.yaml");
}
function provenancePath(root, runId) {
  return path14.join(runDir2(root, runId), "provenance.json");
}
function logsDir(root, runId) {
  return path14.join(runDir2(root, runId), "logs");
}
function resolvedSettingsPath(root, runId) {
  return path14.join(runDir2(root, runId), "resolved-settings.json");
}
function pluginConfigSnapshotPath(root, runId) {
  return path14.join(runDir2(root, runId), "plugin-config-snapshot.json");
}
...
function makeCanonicalRunId(nowIso, slugSource) {
  const slug = runSlug(slugSource) || crypto4.randomUUID();
...
function createRunLifecycle(env) {
  return {
    startRun(opts) {
      ...
      const runId = env.fs.exists(runDir2(root, preferredRunId)) ? makeCanonicalRunId(nowIso, `${deriveRunSlug(opts)}-${crypto4.randomUUID()}`) : preferredRunId;
    ...
    closeRun(runId, opts) {
      ...
      const terminalTraceEvent = {
        event_id: `evt-${crypto4.randomUUID()}`,
      ...
      if (env.fs.scrubbedWriteDurable) {
        const runDir4 = path14.join(root, ".guild", "runs", runId);
...
function createRealEnv(root, resolveHost) {
  const env = {
    ...
    fs: {
      ...
      scrubbedWriteDurable(outPath, contents, surface, runDir4, runId) {
        return scrubbedWrite(outPath, contents, { surface, runDir: runDir4, runId });
      }
    },
    resolveHost,
...
function writeResolvedSettingsSnapshot(runId, snapshot, opts) {
  ...
  const fs20 = fsSeam ?? realProvenanceFsSeam();
  ...
  if (fs20.scrubbedWriteDurable) {
    const runDir4 = path14.join(cwd, ".guild", "runs", runId);
    const result = fs20.scrubbedWriteDurable(outPath, serialized, "config", runDir4, runId);
    if (result.blocked) {
      process.stderr.write(
...
`
    ...
    fs20.writeFile(outPath, serialized);
  }
  return outPath;
}
function hashOptionalFile(env, file) {
  const raw = env.fs.readFile(file);
  return raw === null ? null : crypto4.createHash("sha256").update(raw).digest("hex");
}
function derivePluginIdentity(start, env) {
  const pluginManifestPath = path14.join(start.root, ".claude-plugin", "plugin.json");
  ...
  const manifestHash = pluginManifest === null ? null : crypto4.createHash("sha256").update(pluginManifest).digest("hex");
  const commandSurfaceHash = hashOptionalFile(env, path14.join(start.root, "command-src", "command-registry.json"));
  return {
...
function writePluginConfigSnapshot(runId, snapshot, start, env) {
  ...
  const record = {
    ...
    registry_hashes: {
      skills: hashOptionalFile(env, path14.join(start.root, ".guild", "skills", "registry.yaml")),
      agents: hashOptionalFile(env, path14.join(start.root, ".guild", "agents", "registry.yaml"))
    },
...
var crypto4, fsNode, path14, CANONICAL_RUN_ID_RE, CANONICAL_PHASES;
...
function issue(path27, code, message) {
  return { path: path27, code, message: `${DOCUMENTS_ERROR_NAMESPACE}: ${message}` };
}
function pushIssue(issues, path27, code, message) {
  if (issues.length >= MAX_ISSUES) return;
  issues.push(issue(path27, code, message));
}
...
function canonicalDocumentJson(value) {
  ...
  let nodes = 0;
  const walk = (node, path27, depth) => {
    if (errors.length >= MAX_ISSUES) return null;
    if (depth > MAX_CANONICAL_DEPTH) {
      pushIssue(errors, path27, "depth_exceeded", `value nests deeper than ${MAX_CANONICAL_DEPTH}`);
      return null;
    }
    nodes += 1;
    if (nodes > MAX_CANONICAL_NODES) {
      pushIssue(errors, path27, "size_exceeded", `value exceeds ${MAX_CANONICAL_NODES} nodes`);
      return null;
    }
    ...
    if (kind === "string") {
      ...
      if (text.length > MAX_STRING_LENGTH) {
        pushIssue(errors, path27, "string_too_long", `string exceeds ${MAX_STRING_LENGTH} characters`);
        return null;
      }
    ...
    if (kind === "number") {
      ...
      if (!Number.isFinite(num)) {
        pushIssue(errors, path27, "non_finite_number", "numbers must be finite");
        return null;
      }
      return Object.is(num, -0) ? "0" : String(num);
    }
    if (kind !== "object") {
      pushIssue(errors, path27, "unsupported_type", `${kind} has no canonical JSON form`);
      return null;
    }
    if (active.has(node)) {
      pushIssue(errors, path27, "cycle_detected", "value contains a cycle");
      return null;
    }
    active.add(node);
    try {
      if (safeIsArray(node)) {
        ...
        if (length.ok === false) {
          pushIssue(errors, path27, "array_length_unreadable", length.reason);
          return null;
        }
        if (length.length > MAX_ARRAY_ITEMS) {
          pushIssue(errors, path27, "array_too_long", `array exceeds ${MAX_ARRAY_ITEMS} items`);
          return null;
        }
        const parts2 = [];
        for (let index = 0; index < length.length; index += 1) {
          ...
          if (!safeHasOwn(node, key)) {
            pushIssue(errors, `${path27}[${index}]`, "sparse_array_hole", "array holes have no canonical JSON form");
            return null;
          }
          const read = safeGet(node, key);
          if (read.ok === false) {
            pushIssue(errors, `${path27}[${index}]`, "property_read_threw", read.reason);
            return null;
          }
          const encoded = walk(read.value, `${path27}[${index}]`, depth + 1);
      ...
      if (keys.ok === false) {
        pushIssue(errors, path27, "own_keys_threw", keys.reason);
        return null;
      }
      if (keys.keys.length > MAX_OBJECT_KEYS) {
        pushIssue(errors, path27, "object_too_wide", `object exceeds ${MAX_OBJECT_KEYS} keys`);
        return null;
      }
      ...
      for (const key of sorted) {
        const read = safeGet(node, key);
        if (read.ok === false) {
          pushIssue(errors, `${path27}.${key}`, "property_read_threw", read.reason);
          return null;
        }
        if (read.value === void 0) {
          pushIssue(errors, `${path27}.${key}`, "undefined_value", "undefined has no canonical JSON form");
          return null;
        }
        const encoded = walk(read.value, `${path27}.${key}`, depth + 1);
        if (encoded === null) return null;
...
function readShape(issues, value, path27, allowed) {
  if (value === null || typeof value !== "object") {
    pushIssue(issues, path27, "not_an_object", `${path27} must be an object`);
    return false;
  }
  if (safeIsArray(value)) {
    pushIssue(issues, path27, "not_an_object", `${path27} must be an object, not an array`);
    return false;
  }
  const keys = safeOwnKeys(value);
  if (keys.ok === false) {
    pushIssue(issues, path27, "own_keys_threw", `${path27}: ${keys.reason}`);
    return false;
  }
  ...
  for (const key of [...keys.keys].sort()) {
    if (!allowedSet.has(key)) {
      pushIssue(issues, `${path27}.${key}`, "unexpected_key", `${path27}.${key} is not part of the closed schema`);
      ok = false;
    }
  }
  for (const key of allowed) {
    if (!safeHasOwn(value, key)) {
      pushIssue(issues, `${path27}.${key}`, "missing_field", `${path27}.${key} is required`);
      ok = false;
    }
  }
  return ok;
}
function readString(issues, parent, path27, key, options = {}) {
  const fieldPath = `${path27}.${key}`;
...
function readArray(issues, parent, path27, key, options = {}) {
  const fieldPath = `${path27}.${key}`;
  const read = safeGet(parent, key);
  if (read.ok === false) {
    pushIssue(issues, fieldPath, "property_read_threw", `${fieldPath}: property read threw`);
...
function readStringArray(issues, parent, path27, key, options = {}) {
  const items = readArray(issues, parent, path27, key, options);
...
function readItemArray(issues, parent, path27, key, options, readItem) {
  const items = readArray(issues, parent, path27, key, options);
  if (items === null) return null;
  const fieldPath = `${path27}.${key}`;
  const out = [];
...
function readProvenance(issues, parent, path27) {
  const read = safeGet(parent, "provenance");
  if (read.ok === false) {
    pushIssue(issues, `${path27}.provenance`, "property_read_threw", `${path27}.provenance: property read threw`);
    return null;
  }
  const provenancePath3 = `${path27}.provenance`;
  ...
  const authorFamily = readString(issues, source, provenancePath3, "author_family", {
    pattern: DOCUMENT_ID_PATTERN
  });
  const hostId = readString(issues, source, provenancePath3, "host_id", {
    pattern: DOCUMENT_ID_PATTERN
  });
  const createdAt = readString(issues, source, provenancePath3, "created_at", {
    pattern: DOCUMENT_TIMESTAMP_PATTERN
  });
  const provenanceSource = readString(issues, source, provenancePath3, "source", {
  ...
  if (createdAt !== null) {
    ...
    if (Number.isNaN(parsed) || new Date(parsed).toISOString() !== createdAt) {
      pushIssue(
        issues,
        `${provenancePath3}.created_at`,
        "not_a_real_instant",
        `${provenancePath3}.created_at is not a real UTC instant`
      );
      return null;
    }
  }
...
function readPlanBody(issues, body, path27) {
  if (!readShape(issues, body, path27, ["objectives", "steps"])) return null;
  const objectives = readStringArray(issues, body, path27, "objectives", { min: 1, max: 64, itemMaxLength: 500 });
  const steps = readItemArray(issues, body, path27, "steps", { min: 1, max: 256 }, (itemIssues, item, itemPath) => {
...
function readSpecBody(issues, body, path27) {
  if (!readShape(issues, body, path27, ["requirements"])) return null;
...
function readHandoffBody(issues, body, path27) {
  if (!readShape(issues, body, path27, ["task_id", "status", "artifacts", "issues"])) return null;
  const taskId = readString(issues, body, path27, "task_id", { pattern: DOCUMENT_ITEM_ID_PATTERN });
  const status = readString(issues, body, path27, "status", { enumOf: HANDOFF_STATUSES });
  const artifacts = readStringArray(issues, body, path27, "artifacts", { max: 256, itemMaxLength: 1e3 });
  const handoffIssues = readStringArray(issues, body, path27, "issues", { max: 256, itemMaxLength: 1e3 });
  if (taskId === null || status === null || artifacts === null || handoffIssues === null) return null;
  return { task_id: taskId, status, artifacts, issues: handoffIssues };
}
function readReviewBody(issues, body, path27) {
  if (!readShape(issues, body, path27, ["verdict", "findings"])) return null;
  const verdict = readString(issues, body, path27, "verdict", { enumOf: REVIEW_VERDICTS });
...
function readVerifyBody(issues, body, path27) {
  if (!readShape(issues, body, path27, ["outcome", "checks"])) return null;
  const outcome = readString(issues, body, path27, "outcome", { enumOf: VERIFY_OUTCOMES });
...
function readRunStateLanes(runDir4) {
  const obj = readJsonObject(path15.join(runDir4, "run-state.json"));
...
function readHeartbeatAges(runDir4, now = Date.now()) {
  ...
  const dir = path15.join(runDir4, "in-progress");
  let names;
  try {
...
function readReceiptEvidence(runDir4) {
  const dir = path15.join(runDir4, "handoffs");
  let names;
  try {
    names = fs11.readdirSync(dir);
  } catch {
  ...
  for (const name of names.sort()) {
    ...
    let text = null;
    try {
      if (fs11.statSync(filePath).size <= MAX_RECEIPT_BYTES) {
        text = fs11.readFileSync(filePath, "utf8");
      }
    } catch {
      text = null;
    }
...
function sweepLaneLiveness(runDir4, timeoutMs = DEFAULT_HEARTBEAT_TIMEOUT_MS, now = Date.now()) {
  const lanes = readRunStateLanes(runDir4);
  const heartbeats = readHeartbeatAges(runDir4, now);
  const receiptEvidence = readReceiptEvidence(runDir4);
...
`);
  ...
  const report = sweepLaneLiveness(runDir4, resolveTimeoutMs());
  process.stdout.write(JSON.stringify(report, null, 2) + "\n");
  return 0;
}
var fs11, path15, DEFAULT_HEARTBEAT_TIMEOUT_MS, TERMINAL_STATUSES, MAX_RECEIPT_BYTES, NO_RECEIPT, USAGE;
...
function stableLockPath(runDir4) {
  return (0, import_node_path.join)(runDir4, "logs", ".lock");
}
function exclusionSentinelPath(runDir4) {
  return (0, import_node_path.join)(runDir4, "logs", ".lock.exclusion");
}
function initStableLockfile(runDir4) {
  const path27 = stableLockPath(runDir4);
  (0, import_node_fs.mkdirSync)((0, import_node_path.dirname)(path27), { recursive: true });
  if ((0, import_node_fs.existsSync)(path27)) return;
  try {
    const fd = (0, import_node_fs.openSync)(path27, "wx");
...
function withStableLock(runDir4, fn, opts = {}) {
  initStableLockfile(runDir4);
  const sentinel = exclusionSentinelPath(runDir4);
...
function liveLogPath2(runDir4) {
  return (0, import_node_path2.join)(runDir4, "logs", "v1.4-events.jsonl");
}
function archiveDir(runDir4) {
  return (0, import_node_path2.join)(runDir4, "logs", "archive");
}
...
function appendEvent(runDir4, event, opts = {}) {
  ...
  if (opts.forceFallback || process.platform === "win32") {
    ...
    const path27 = laneFallbackPath(runDir4, laneId2);
    (0, import_node_fs2.mkdirSync)((0, import_node_path2.dirname)(path27), { recursive: true });
    const fd = (0, import_node_fs2.openSync)(path27, "a");
    try {
  ...
  withStableLock(runDir4, () => {
    ...
    maybeRotateLocked(runDir4, opts.rotationThresholdBytes ?? ROTATION_THRESHOLD_BYTES);
  });
}
function nextRotationIndex(runDir4) {
...
function maybeRotateLocked(runDir4, thresholdBytes) {
  const live = liveLogPath2(runDir4);
...
function rotateLocked(runDir4) {
  const live = liveLogPath2(runDir4);
  const archive = archiveDir(runDir4);
  (0, import_node_fs2.mkdirSync)(archive, { recursive: true });
  const n = nextRotationIndex(runDir4);
  const stagingPath = (0, import_node_path2.join)(archive, `v1.4-events.${n}.jsonl`);
  const finalArchive = archivePath(runDir4, n);
...
function readSentinel(cwd) {
  const sentinelPath2 = path16.join(cwd, ".guild", "runs", "current-run-id");
  try {
    const value = fs12.readFileSync(sentinelPath2, "utf8").trim();
...
var fs12, path16, VALID_EVENTS, VALID_LAYERS, VALID_TERMINATED;
...
function runStatePath(runDir4) {
  return path17.join(runDir4, "run-state.json");
}
function loadRunState(runDir4) {
  let raw;
  try {
    raw = fs13.readFileSync(runStatePath(runDir4), "utf8");
  } catch {
    return null;
  }
  let parsed;
...
function writeRunStateAtomic(runDir4, state) {
  fs13.mkdirSync(runDir4, { recursive: true });
  const finalPath = runStatePath(runDir4);
  const tmpPath = `${finalPath}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  fs13.writeFileSync(tmpPath, JSON.stringify(state, null, 2) + "\n", "utf8");
  try {
    fs13.renameSync(tmpPath, finalPath);
  } catch (err) {
    try {
...
function markLaneInProgress(runDir4, init2, laneId2, opts = {}) {
  return upsertLane(runDir4, init2, laneId2, {
    status: "in_progress",
    tier: opts.tier,
...
function laneResumeCheckpointPath(runDir4, laneId2) {
  return path17.join(runDir4, "lanes", laneId2, "resume.json");
}
function readResumeEnabled(cwd) {
  const settingsPath = path17.join(resolveGuildRoot2(cwd), ".guild", "settings.json");
  try {
    const raw = fs13.readFileSync(settingsPath, "utf8");
    const parsed = JSON.parse(raw);
    const defs = parsed["defaults"];
...
function loadLaneResumeCheckpoint(runDir4, laneId2) {
  try {
    const raw = fs13.readFileSync(laneResumeCheckpointPath(runDir4, laneId2), "utf8");
    const parsed = JSON.parse(raw);
...
function markLaneDead(runDir4, init2, laneId2, signal, cwd) {
  const state = upsertLane(runDir4, init2, laneId2, {
    status: "dead",
  ...
  if (readResumeEnabled(cwd)) {
    ...
    fs13.mkdirSync(path17.dirname(checkpointPath), { recursive: true });
    fs13.writeFileSync(checkpointPath, JSON.stringify(checkpoint, null, 2) + "\n", "utf8");
  }
  return state;
}
var fs13, path17, RUN_STATE_SCHEMA_VERSION, LANE_RESUME_SCHEMA_VERSION;
...
function parseMarkLaneDeadArgs(argv) {
  ...
  const [runDir4, laneId2] = positionals;
  if (!runDir4 || !laneId2) {
    return {
...
function scanResumableLanes(runDir4, cwd, slug) {
  ...
  const lanesDir = path19.join(runDir4, "lanes");
  let entries;
  try {
    entries = fs14.readdirSync(lanesDir, { withFileTypes: true });
  } catch {
    return [];
  }
  const runState = loadRunState(runDir4);
  ...
  for (const entry of entries) {
    ...
    const cp = loadLaneResumeCheckpoint(runDir4, laneId2);
    if (!cp) continue;
...
function renderHuman(runDir4, lanes) {
  if (lanes.length === 0) {
    return `[resume-lanes] no resumable dead lanes in ${runDir4}
`;
  }
  const rows = [
    `[resume-lanes] ${lanes.length} resumable dead lane(s) in ${runDir4}:`,
...
function manifestPathFor(cwd, slug) {
  return path20.join(cwd, ".guild", "programs", slug, "manifest.json");
}
function readRunManifest(cwd, slug) {
  try {
    const raw = fs15.readFileSync(manifestPathFor(cwd, slug), "utf8");
...
function runWriteRunManifestCli(argv = process.argv.slice(2)) {
  ...
  if (!fs15.existsSync(args.cwd) || !fs15.statSync(args.cwd).isDirectory()) {
...
var fs15, path20, WAVE_STATUSES, PROGRAM_STATUSES;
...
function taskRunPath(cwd, runId, taskId) {
  return path21.join(cwd, ".guild", "runs", runId, "task-runs", `${taskId}.yaml`);
}
function writeTaskRun(cwd, runId, taskId, params) {
  ...
  try {
    const _traceRunDir = path21.join(cwd, ".guild", "runs", runId);
...
function readPlanOwnerTaskIds(guildRoot, slug) {
  ...
  let raw;
  try {
    raw = fs16.readFileSync(path22.join(guildRoot, ".guild", "plan", `${slug}.md`), "utf8");
  } catch {
    return map;
  }
...
function appendKnowledgeLinksIndex(guildRoot, links) {
  ...
  const indexDir = path24.join(guildRoot, ".guild", "indexes");
  const indexPath = path24.join(indexDir, "knowledge-links.json");
  let existing = [];
  if (fs17.existsSync(indexPath)) {
    try {
      const raw = fs17.readFileSync(indexPath, "utf8");
      const parsed = JSON.parse(raw);
...
`
  ...
  try {
    fs17.mkdirSync(indexDir, { recursive: true });
    fs17.writeFileSync(
      indexPath,
      JSON.stringify(
...
function appendReflections(guildRoot, runId, phase, decisions) {
  ...
  const reflectionsDir = path24.join(guildRoot, ".guild", "reflections");
  fs17.mkdirSync(reflectionsDir, { recursive: true });
  const reflPath = path24.join(reflectionsDir, `${runId}.md`);
  const entry = `
## Phase: ${phase} (${runId})

` + nonNone.map((k) => `- ${k}: ${decisions[k]}`).join("\n") + "\n";
  fs17.appendFileSync(reflPath, entry, "utf8");
}
function writeCheckpoint(opts) {
  ...
  const learningDir = path24.join(guildRoot, ".guild", "runs", opts.runId, "learning");
  fs17.mkdirSync(learningDir, { recursive: true });
  const checkpointFile = path24.join(learningDir, `${opts.phase}-${opts.runId}.yaml`);
  const reflectionsRelPath = `.guild/reflections/${opts.runId}.md`;
  const reflectionsAbsPath = path24.join(guildRoot, ".guild", "reflections", `${opts.runId}.md`);
  ...
  fs17.writeFileSync(checkpointFile, yaml4, "utf8");
...
function main() {
  ...
  if (verdictPath) {
    try {
      const raw = fs17.readFileSync(verdictPath, "utf8");
...
`
  ...
  if (decisions === void 0 && artifactsJsonPath) {
    try {
      const rawArtifacts = fs17.readFileSync(artifactsJsonPath, "utf8");
...
`
  ...
  if (linksPath) {
    try {
      const raw = fs17.readFileSync(linksPath, "utf8");
      const parsed = JSON.parse(raw);
...
function liveLogPath3(root, runId) {
  return path25.join(runDir3(root, runId), "logs", "v1.4-events.jsonl");
}
function provenancePath2(root, runId) {
  return path25.join(runDir3(root, runId), "provenance.json");
}
...
function defaultResolveHost(requested) {
  ...
  const resolved = KNOWN_HOST_KINDS.includes(raw) ? raw : "unknown";
  return { requested, resolved };
}
function appendTraceLine(file, event) {
  fs18.mkdirSync(path25.dirname(file), { recursive: true });
  fs18.appendFileSync(file, JSON.stringify(event) + "\n", "utf8");
}
function emitRunClosed(root, runId, resolveHost, opts = {}) {
  try {
    ...
    const prov = JSON.parse(fs18.readFileSync(provenancePath2(root, runId), "utf8"));
...
`
    ...
    appendTraceLine(liveLogPath3(root, runId), {
...
function newestPostCloseActivityMs(root, runId) {
  ...
  const candidates = [liveLogPath3(root, runId), path25.join(dir, "events.ndjson")];
  try {
    const inProgress = path25.join(dir, "in-progress");
    for (const name of fs18.readdirSync(inProgress)) {
      candidates.push(path25.join(inProgress, name));
    }
  } catch {
  }
  let newest = 0;
  for (const p of candidates) {
    try {
      newest = Math.max(newest, fs18.statSync(p).mtimeMs);
    } catch {
    }
  }
  return newest;
...
async function readHookStdin() {
  return new Promise((resolve13) => {
    ...
    process.stdin.on("end", () => resolve13(Buffer.concat(chunks).toString("utf8")));
    process.stdin.on("error", () => resolve13(""));
  });
}
...
function findActiveWork(runDir4) {
  try {
    const raw = fs19.readFileSync(path26.join(runDir4, "run-state.json"), "utf8");
    const parsed = JSON.parse(raw);
    if (parsed?.schema_version === "guild.run_state.v1" && parsed.lanes) {
      for (const [laneId2, lane] of Object.entries(parsed.lanes)) {
        if (lane?.status === "in_progress") {
          return `run-state lane "${laneId2}" is in_progress`;
        }
      }
    }
  } catch {
  }
  const cellsRoot = path26.join(runDir4, "task-cells");
  try {
    for (const cell of fs19.readdirSync(cellsRoot, { withFileTypes: true })) {
      if (!cell.isDirectory()) continue;
      const attemptsDir = path26.join(cellsRoot, cell.name, "attempts");
      let attempts;
      try {
        attempts = fs19.readdirSync(attemptsDir, { withFileTypes: true });
      } catch {
        continue;
      }
      for (const attempt of attempts) {
        ...
        try {
          const record = JSON.parse(
            fs19.readFileSync(path26.join(attemptsDir, attempt.name, "attempt.json"), "utf8")
          );
...
function recoverPrematureClose(runDir4, activeWork) {
  const provenanceFile = path26.join(runDir4, "provenance.json");
  let prov;
  try {
    prov = JSON.parse(fs19.readFileSync(provenanceFile, "utf8"));
  } catch {
    return null;
  }
  const status = prov["status"];
  if (status !== "closed" && status !== "failed") return null;
  let runYamlOpen = false;
  try {
    runYamlOpen = /^status:\s*open\s*$/m.test(
      fs19.readFileSync(path26.join(runDir4, "run.yaml"), "utf8")
    );
  } catch {
    return null;
  }
  if (!runYamlOpen) return null;
  const recovered = {
    ...prov,
    status: "resumable",
    premature_close_previous_status: status,
    premature_close_recovered_at: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  fs19.writeFileSync(tmpPath, JSON.stringify(recovered, null, 2) + "\n", "utf8");
  try {
    fs19.renameSync(tmpPath, provenanceFile);
  } catch (err) {
    try {
...
function findTerminalCheckpoint(runDir4, runId) {
  const learningDir = path26.join(runDir4, "learning");
  if (!fs19.existsSync(learningDir)) return null;
  const reflectionFile = path26.join(learningDir, `reflection-${runId}.yaml`);
  if (fs19.existsSync(reflectionFile)) return reflectionFile;
  let yamlFiles;
  try {
    yamlFiles = fs19.readdirSync(learningDir).filter((f) => f.endsWith(".yaml"));
  } catch {
    return null;
  }
  if (yamlFiles.length === 0) return null;
  const sorted = yamlFiles.map((f) => path26.join(learningDir, f)).sort((a, b) => {
    try {
      return fs19.statSync(b).mtimeMs - fs19.statSync(a).mtimeMs;
    } catch {
      return 0;
    }
  });
...
async function main2() {
  ...
  const runDir4 = path26.join(root, ".guild", "runs", runId);
  if (!fs19.existsSync(path26.join(runDir4, "run.yaml"))) process.exit(0);
  const activeWork = findActiveWork(runDir4);
  if (activeWork !== null) {
    try {
      const recovered = recoverPrematureClose(runDir4, activeWork);
      if (recovered !== null) {
...
`);
      ...
      process.stderr.write(
        `[run-trace-close] WARN: premature-close recovery failed for ${runId}: ${err instanceof Error ? err.message : String(err)} \u2014 close still refused.
`
      );
    }
    process.stderr.write(
      `[run-trace-close] refusing to close ${runId}: ${activeWork} \u2014 workers are still active; the close is deferred until they terminate.
`
    );
    process.exit(0);
  }
  const provenanceFile = path26.join(runDir4, "provenance.json");
  if (fs19.existsSync(provenanceFile)) {
    let terminal = true;
    let closedAtMs = 0;
    try {
      const prov = JSON.parse(fs19.readFileSync(provenanceFile, "utf8"));
...
`
      ...
      const recloseRef = envelope && envelope.run_id === runId ? envelope.binding_ref : void 0;
      if (recloseRef === void 0) {
        process.stderr.write(
          `[run-trace-close] WARN: binding_rejected \u2014 cannot re-close ${runId}: the GUILD_RUN_BINDING_REF envelope does not name this run. No write performed.
`
        );
        process.exit(0);
      }
      try {
        ...
        process.stderr.write(
          `[run-trace-close] WARN: ${err instanceof Error ? err.message : String(err)} \u2014 re-close refused (fail closed).
`
        );
        process.exit(0);
      }
