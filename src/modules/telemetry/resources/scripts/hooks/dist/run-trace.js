...
var fs4, path5, yaml, DEFAULTS2, VALID_TIER_HOST_KEYS, KNOWN_HOST_IDS2, VALID_LOOPS, VALID_RIGOR, VALID_REVIEW, DISPATCH_HOST_IDS, VALID_AGENT_MODE, VALID_CACHE_TTL, DEFAULTS_ALLOWED_KEYS;
...
function loadLocalOverride(cwd, fileConfig, selfBuild) {
  const localPath = path13.join(cwd, ".guild", "settings.local.json");
...
function loadFileConfig(cwd, selfBuild) {
  const settingsPath = path13.join(cwd, ".guild", "settings.json");
...
function readSecurityConfig(cwd) {
  const settingsPath = path9.join(resolveGuildRoot(cwd), ".guild", "settings.json");
  let raw;
  try {
...
function appendSecurityEvent(runDir3, record) {
  try {
    const logsDir2 = path10.join(runDir3, "logs");
...
function guildRootFromRunDir(runDir3) {
  return path11.resolve(runDir3, "../../..");
}
function writeScrubApprovalRequest(runDir3, runId, surface, outPath, laneId) {
  try {
    const approvalDir = path11.join(runDir3, "agent-bus", "approvals");
...
function runYamlPath(root, runId) {
  return path12.join(runDir(root, runId), "run.yaml");
}
function provenancePath(root, runId) {
  return path12.join(runDir(root, runId), "provenance.json");
}
function logsDir(root, runId) {
  return path12.join(runDir(root, runId), "logs");
}
function resolvedSettingsPath(root, runId) {
  return path12.join(runDir(root, runId), "resolved-settings.json");
}
function sentinelPath(root) {
  return path12.join(root, ".guild", "runs", "current-run-id");
}
  ...
  const env = {
    ...
    fs: {
      ...
      writeFile(absPath, contents) {
        fsNode.mkdirSync(path12.dirname(absPath), { recursive: true });
...
function assertContained(target, base, label) {
  ...
  const resolvedBase = path12.resolve(base);
  if (!resolvedTarget.startsWith(resolvedBase + path12.sep)) {
    throw new Error(
...
function writeResolvedSettingsSnapshot(runId, snapshot, opts) {
  ...
  const runsBase = path12.resolve(cwd, ".guild", "runs");
  ...
  if (fs14.scrubbedWriteDurable) {
    const runDir3 = path12.join(cwd, ".guild", "runs", runId);
