...
var import_child_process, fs2, os, path2, PROVIDER_REGISTRY, KNOWN_FAMILIES;
...
function sessionContextPath(root, runId) {
  return path3.join(root, ".guild", "runs", runId, "session-context.json");
}
function writeSessionContext(root, ctx, fs14) {
  const f = fs14 ?? realFs();
  ...
  f.mkdirp(path3.dirname(p));
  f.writeFile(p, serialized);
}
var fsReal, path3, UNKNOWN_TARGET;
...
function discoverWorkspace(startDir) {
  let current = path5.dirname(startDir);
  const fsRoot = path5.parse(current).root;
  while (current !== fsRoot) {
    const manifestPath = path5.join(current, ".guild", "workspace.json");
...
function parseLocalFile(guildDir) {
  const localPath = path6.join(guildDir, "settings.local.json");
...
function isContainedIn(candidatePath, baseDir) {
  ...
  return resolved.startsWith(resolvedBase + path6.sep);
}
function initiativeIsWorkspaceScoped(workspaceRoot, id) {
  try {
    ...
    const initiativesBase = path6.join(workspaceRoot, ".guild", "initiatives");
    const activePath = path6.join(
      initiativesBase,
      "active",
      id,
      "initiative.yaml"
    );
    const archivedPath = path6.join(
      initiativesBase,
      "archived",
      id,
      "initiative.yaml"
    );
    const activeBase = path6.join(initiativesBase, "active");
    const archivedBase = path6.join(initiativesBase, "archived");
...
function resolveSettings(opts) {
  ...
  let wsLocalSettings = {};
  if (ws !== null) {
    const wsGuildDir = path6.join(ws.rootDir, ".guild");
    const rawWsSettings = parseSettingsFile(path6.join(wsGuildDir, "settings.json"));
    const wsInheritable = {};
  ...
  const projectGuildDir = path6.join(cwd, ".guild");
  const projectSettings = parseSettingsFile(path6.join(projectGuildDir, "settings.json"));
...
var fs4, path6, yaml, DEFAULTS2, VALID_TIER_HOST_KEYS, KNOWN_HOST_IDS2, VALID_LOOPS, VALID_RIGOR, VALID_REVIEW, DISPATCH_HOST_IDS, VALID_AGENT_MODE, VALID_CACHE_TTL, DEFAULTS_ALLOWED_KEYS;
...
function resolveSettings2(opts) {
  ...
  try {
    ...
    const _traceRunDir = _traceRunId && cwd ? path8.join(cwd, ".guild", "runs", _traceRunId) : void 0;
    if (_traceRunDir) {
...
function loadLocalOverride(cwd, fileConfig, selfBuild) {
  const localPath = path15.join(cwd, ".guild", "settings.local.json");
  if (!fs10.existsSync(localPath)) return fileConfig;
  let localParsed;
  try {
...
function loadFileConfig(cwd, selfBuild) {
  const settingsPath = path15.join(cwd, ".guild", "settings.json");
  if (fs10.existsSync(settingsPath)) {
    let parsed;
    try {
      parsed = JSON.parse(fs10.readFileSync(settingsPath, "utf8"));
    } catch (e) {
...
`);
  ...
  if (fs10.existsSync(localPath)) {
    try {
      const rawLocal = JSON.parse(fs10.readFileSync(localPath, "utf8"));
...
var fsReal2 = __toESM(require("fs"));
var path10 = __toESM(require("path"));
var BindingRejectedError = class extends Error {
...
function runBindingPath(root, runId) {
  return path10.join(root, ".guild", "runs", runId, "binding.json");
}
function mintRunBinding(opts) {
  const fs14 = opts.fs ?? realBindingFs();
  ...
  const record = {
    ...
    run_id: opts.run_id,
    binding_ref: `rb-${crypto2.randomBytes(16).toString("hex")}`,
    state: "open"
  };
  fs14.mkdirp(path10.dirname(p));
  fs14.writeFile(p, JSON.stringify(record, null, 2) + "\n");
  return record;
}
...
function readRunBindingRecord(opts) {
  const fs14 = opts.fs ?? realBindingFs();
  const raw = fs14.readFile(runBindingPath(opts.root, opts.run_id));
...
function locateCandidateRunId(root, fs14) {
  const f = fs14 ?? realBindingFs();
  const candidates = [
    [path10.join(root, ".guild", "runs", "current-run-id"), "sentinel-legacy"],
    [path10.join(root, ".guild", "current-run-id"), "sentinel-b2"]
  ];
...
function appendSecurityEvent(runDir3, record) {
  try {
    const logsDir2 = path12.join(runDir3, "logs");
    fs8.mkdirSync(logsDir2, { recursive: true });
    fs8.appendFileSync(path12.join(logsDir2, "security-events.jsonl"), JSON.stringify(record) + "\n", "utf8");
    return true;
  } catch (err) {
    process.stderr.write(
...
function writeScrubApprovalRequest(runDir3, runId, surface, outPath, laneId) {
  try {
    const approvalDir = path13.join(runDir3, "agent-bus", "approvals");
    ...
    const record = {
      ...
      run_id: runId,
      tool: "scrubbedWrite",
      reason: `Secret scrub failed for durable surface "${surface}" \u2014 write blocked. Human review required. Path: ${path13.basename(outPath)}`,
    ...
    fs9.writeFileSync(path13.join(approvalDir, fileName), content, "utf8");
  } catch {
  }
}
function scrubbedWrite(outPath, content, opts) {
  ...
  if (scrubResult.ok) {
    try {
      fs9.mkdirSync(path13.dirname(outPath), { recursive: true });
...
`
  ...
  if (failMode === "open") {
    process.stderr.write(
      `[scrubbed-write] WARN: secret scrub custom-pattern failure for surface "${opts.surface}" at ${path13.basename(outPath)} \u2014 writing built-in-redacted content (fail-open). Failures: ${scrubResult.failures.join("; ")}
`
    );
    try {
      fs9.mkdirSync(path13.dirname(outPath), { recursive: true });
...
`
    ...
    try {
      const evt = buildSecurityEvent({
        ...
        decision: "degraded",
        tool: "scrubbedWrite",
        detail: `Secret scrub custom-pattern failure (fail-open) for surface "${opts.surface}" at ${path13.basename(outPath)}. Built-in-redacted content written.`,
...
`
  ...
  try {
    const evt = buildSecurityEvent({
      ...
      decision: "blocked",
      tool: "scrubbedWrite",
      detail: `Secret scrub failed for durable surface "${opts.surface}" at ${path13.basename(outPath)} \u2014 write blocked (fail-closed).`,
      permission_mode: "blocked"
    });
    appendSecurityEvent(opts.runDir, evt);
  } catch {
...
function assertContained(target, base, label) {
  ...
  const resolvedBase = path14.resolve(base);
  if (!resolvedTarget.startsWith(resolvedBase + path14.sep)) {
    throw new Error(
...
function appendKnowledgeLinksIndex(guildRoot, links) {
  ...
  const indexDir = path16.join(guildRoot, ".guild", "indexes");
  const indexPath = path16.join(indexDir, "knowledge-links.json");
  let existing = [];
...
function appendReflections(guildRoot, runId, phase, decisions) {
  ...
  const reflectionsDir = path16.join(guildRoot, ".guild", "reflections");
  fs11.mkdirSync(reflectionsDir, { recursive: true });
  const reflPath = path16.join(reflectionsDir, `${runId}.md`);
  const entry = `
## Phase: ${phase} (${runId})
...
function writeCheckpoint(opts) {
  ...
  const learningDir = path16.join(guildRoot, ".guild", "runs", opts.runId, "learning");
  fs11.mkdirSync(learningDir, { recursive: true });
  const checkpointFile = path16.join(learningDir, `${opts.phase}-${opts.runId}.yaml`);
  const reflectionsRelPath = `.guild/reflections/${opts.runId}.md`;
  const reflectionsAbsPath = path16.join(guildRoot, ".guild", "reflections", `${opts.runId}.md`);
...
function provenancePath2(root, runId) {
  return path18.join(runDir2(root, runId), "provenance.json");
}
function skippedFilesPath(root, runId) {
  return path18.join(runDir2(root, runId), "learn", "skipped-files.json");
}
...
function appendTraceLine(file, event) {
  fs13.mkdirSync(path18.dirname(file), { recursive: true });
...
function newestRunActivityMs(root, runId) {
  ...
  const candidates = [
    ...
    path18.join(dir, "events.ndjson"),
    path18.join(dir, "run.yaml")
  ];
  try {
    const inProgress = path18.join(dir, "in-progress");
    for (const name of fs13.readdirSync(inProgress)) {
      candidates.push(path18.join(inProgress, name));
    }
  } catch {
  }
  let newest = 0;
...
function closeStalePriorOpenRun(root, resolveHost, now = Date.now) {
  try {
    ...
    let runYaml;
    try {
      runYaml = fs13.readFileSync(path18.join(runDir2(root, priorId), "run.yaml"), "utf8");
    } catch {
      return;
    }
...
function buildPhaseArtifacts(root, runId) {
  ...
  try {
    const hDir = path18.join(runDir2(root, runId), "handoffs");
    const blocks = [];
    const texts = [];
    for (const f of fs13.readdirSync(hDir)) {
      if (!f.endsWith(".md")) continue;
      const content = fs13.readFileSync(path18.join(hDir, f), "utf8");
      texts.push(content);
...
function emitPhaseCheckpoint(root, runId, phase) {
  try {
    ...
    const checkpointFile = path18.join(
      root,
      ".guild",
      "runs",
      runId,
...
function writeSkippedFiles(root, runId, entries, opts = {}) {
  ...
  fs13.mkdirSync(path18.dirname(out), { recursive: true });
