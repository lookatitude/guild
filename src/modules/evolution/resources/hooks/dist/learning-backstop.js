...
function appendKnowledgeLinksIndex(guildRoot, links) {
  ...
  const indexDir = path26.join(guildRoot, ".guild", "indexes");
  const indexPath = path26.join(indexDir, "knowledge-links.json");
  let existing = [];
...
`
  ...
  try {
    fs19.mkdirSync(indexDir, { recursive: true });
    fs19.writeFileSync(
      indexPath,
      JSON.stringify(
...
function appendReflections(guildRoot, runId, phase, decisions) {
  ...
  const reflectionsDir = path26.join(guildRoot, ".guild", "reflections");
  fs19.mkdirSync(reflectionsDir, { recursive: true });
  const reflPath = path26.join(reflectionsDir, `${runId}.md`);
  const entry = `
## Phase: ${phase} (${runId})

` + nonNone.map((k) => `- ${k}: ${decisions[k]}`).join("\n") + "\n";
  fs19.appendFileSync(reflPath, entry, "utf8");
}
function writeCheckpoint(opts) {
  ...
  const learningDir = path26.join(guildRoot, ".guild", "runs", opts.runId, "learning");
  fs19.mkdirSync(learningDir, { recursive: true });
  const checkpointFile = path26.join(learningDir, `${opts.phase}-${opts.runId}.yaml`);
  const reflectionsRelPath = `.guild/reflections/${opts.runId}.md`;
  const reflectionsAbsPath = path26.join(guildRoot, ".guild", "reflections", `${opts.runId}.md`);
  ...
  fs19.writeFileSync(checkpointFile, yaml4, "utf8");
...
function main() {
  ...
  if (verdictPath) {
    try {
      const raw = fs19.readFileSync(verdictPath, "utf8");
...
`
  ...
  if (decisions === void 0 && artifactsJsonPath) {
    try {
      const rawArtifacts = fs19.readFileSync(artifactsJsonPath, "utf8");
...
`
  ...
  if (linksPath) {
    try {
      const raw = fs19.readFileSync(linksPath, "utf8");
      const parsed = JSON.parse(raw);
...
function parsePhasesLogPhases(runYamlRaw) {
  const tokens = [];
  const seen = /* @__PURE__ */ new Set();
...
function runLearningBackstop(opts) {
  try {
    const runDir2 = opts.runDir ?? path27.join(opts.guildRoot, ".guild", "runs", opts.runId);
    let raw;
    try {
      raw = fs20.readFileSync(path27.join(runDir2, "run.yaml"), "utf8");
    } catch {
      return { ...EMPTY_RESULT };
    }
    const result = {
    ...
    const phasesSeen = /* @__PURE__ */ new Set();
    for (const token of tokens) {
      ...
      const checkpointFile = path27.join(
        runDir2,
        "learning",
        `${phase}-${opts.runId}.yaml`
      );
      if (fs20.existsSync(checkpointFile)) {
...
async function main2() {
  ...
  if (result.written.length > 0) {
    process.stderr.write(
      `[learning-backstop] wrote ${result.written.length} backstop checkpoint(s) for run ${runId}: [${result.written.join(", ")}]
`
    );
  }
  if (result.unknownTokens.length > 0) {
    process.stderr.write(
      `[learning-backstop] WARN: unknown phases_log token(s) skipped: [${result.unknownTokens.join(", ")}]
`
    );
  }
