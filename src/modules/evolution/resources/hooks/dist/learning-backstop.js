...
function initStableLockfile(runDir2) {
  const path19 = stableLockPath(runDir2);
...
function appendEvent(runDir2, event, opts = {}) {
  ...
  if (opts.forceFallback || process.platform === "win32") {
    ...
    const path19 = laneFallbackPath(runDir2, laneId2);
...
function runLearningBackstop(opts) {
  try {
    const runDir2 = opts.runDir ?? path18.join(opts.guildRoot, ".guild", "runs", opts.runId);
    let raw;
    try {
      raw = fs14.readFileSync(path18.join(runDir2, "run.yaml"), "utf8");
    } catch {
      return { ...EMPTY_RESULT };
    }
    const result = {
