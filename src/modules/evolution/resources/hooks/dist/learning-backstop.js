...
function runLearningBackstop(opts) {
  try {
    const runDir = opts.runDir ?? path11.join(opts.guildRoot, ".guild", "runs", opts.runId);
    let raw;
    try {
      raw = fs8.readFileSync(path11.join(runDir, "run.yaml"), "utf8");
    } catch {
      return { ...EMPTY_RESULT };
    }
    const result = {
    ...
    for (const token of tokens) {
      ...
      const checkpointFile = path11.join(
        runDir,
        "learning",
        `${phase}-${opts.runId}.yaml`
      );
      if (fs8.existsSync(checkpointFile)) {
