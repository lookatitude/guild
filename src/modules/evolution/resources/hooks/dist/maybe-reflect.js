...
function fileContainsMarker(p) {
  try {
    ...
    return fs5.readFileSync(p, "utf8").includes(SELF_BUILD_MARKER);
  } catch {
    return false;
  }
}
function detectSelfBuild(root) {
  const directPath = path6.join(root, "AGENTS.md");
  if (fileContainsMarker(directPath)) return { armed: true, path: directPath };
  const nestedPath = path6.join(root, "plugin", "AGENTS.md");
...
function devteamSubagentGateCheck(events, cwd) {
  ...
  const specDir = path7.join(resolveGuildRoot(cwd), ".guild", "spec");
  const slug = process.env["GUILD_SPEC_SLUG"];
  if (slug && slug.trim().length > 0) {
    const specPath = path7.join(specDir, `${slug}.md`);
...
function writeStubSummary(runDir, runId, events) {
  ...
  const summaryPath = path7.join(runDir, "summary.md");
...
function tryRealSummarizer(cwd, runId) {
  const summarizerPath = path7.join(cwd, "scripts", "trace-summarize.ts");
...
function evaluateCodexSkipGuard(guildRoot) {
  try {
    ...
    const reflectionsDir = path7.join(guildRoot, ".guild", "reflections");
    if (!fs6.existsSync(reflectionsDir)) return { armed: true, streak: 0 };
    const files = fs6.readdirSync(reflectionsDir).filter((f) => f.endsWith(".md")).map((f) => path7.join(reflectionsDir, f)).map((p) => {
      let mtime = 0;
      try {
...
function writeCodexSkipSentinel(guildRoot, streak) {
  try {
    const guildDir = path7.join(guildRoot, ".guild");
    fs6.mkdirSync(guildDir, { recursive: true });
    const sentinel = path7.join(guildDir, "codex-skip-streak.json");
    const data = {
...
`
  ...
  const runId = resolveRunIdForTrace(guildRoot, { GUILD_RUN_ID: process.env["GUILD_RUN_ID"] }) ?? (sessionId ? `run-${sessionId}` : `run-session-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}`);
  const eventsRunDir = path7.join(guildRoot, ".guild", "runs", runId);
  const canonicalEventsFile = path7.join(eventsRunDir, "logs", "v1.4-events.jsonl");
  const legacyEventsFile = path7.join(eventsRunDir, "events.ndjson");
