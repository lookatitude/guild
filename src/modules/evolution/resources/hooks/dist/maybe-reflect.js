...
function fileContainsMarker(p) {
  try {
    ...
    return fs9.readFileSync(p, "utf8").includes(SELF_BUILD_MARKER);
  } catch {
    return false;
  }
}
function detectSelfBuild(root) {
  const directPath = path11.join(root, "AGENTS.md");
  if (fileContainsMarker(directPath)) return { armed: true, path: directPath };
  const nestedPath = path11.join(root, "plugin", "AGENTS.md");
...
function loadEvents(eventsFile) {
  if (!fs10.existsSync(eventsFile)) return [];
  const content = fs10.readFileSync(eventsFile, "utf8");
  const events = [];
...
function writeStubSummary(runDir, runId, events) {
  ...
  const summaryPath = path12.join(runDir, "summary.md");
  fs10.writeFileSync(summaryPath, lines.join("\n") + "\n", "utf8");
  process.stderr.write(`[maybe-reflect] wrote fallback summary to ${summaryPath}
`);
}
function tryRealSummarizer(cwd, runId) {
  const summarizerPath = path12.join(cwd, "scripts", "trace-summarize.ts");
  if (!fs10.existsSync(summarizerPath)) return false;
...
function evaluateCodexSkipGuard(guildRoot) {
  try {
    ...
    if (!fs10.existsSync(reflectionsDir)) return { armed: true, streak: 0 };
    const files = fs10.readdirSync(reflectionsDir).filter((f) => f.endsWith(".md")).map((f) => path12.join(reflectionsDir, f)).map((p) => {
      let mtime = 0;
      try {
        mtime = fs10.statSync(p).mtimeMs;
      } catch {
        mtime = 0;
      }
      return { path: p, mtime };
...
function writeCodexSkipSentinel(guildRoot, streak) {
  try {
    const guildDir = path12.join(guildRoot, ".guild");
    fs10.mkdirSync(guildDir, { recursive: true });
    const sentinel = path12.join(guildDir, "codex-skip-streak.json");
    const data = {
    ...
    fs10.writeFileSync(sentinel, JSON.stringify(data, null, 2) + "\n", "utf8");
    process.stderr.write(
...
`
  ...
  const eventsRunDir = path12.join(guildRoot, ".guild", "runs", runId);
  const canonicalEventsFile = path12.join(eventsRunDir, "logs", "v1.4-events.jsonl");
  const legacyEventsFile = path12.join(eventsRunDir, "events.ndjson");
  const eventsFile = fs10.existsSync(canonicalEventsFile) ? canonicalEventsFile : legacyEventsFile;
...
`
  ...
  const runDir = path12.join(guildRoot, ".guild", "runs", runId);
