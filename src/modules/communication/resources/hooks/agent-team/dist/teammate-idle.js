...
function runStatePath(runDir) {
  return path11.join(runDir, "run-state.json");
}
...
function heartbeatPath(runDir, specialist) {
  return path12.join(runDir, "in-progress", `${specialist}.json`);
}
function legacyLogPath(runDir, specialist) {
  return path12.join(runDir, "in-progress", `${specialist}.log`);
}
function readHeartbeat(runDir, specialist) {
  let raw;
  try {
    raw = fs9.readFileSync(heartbeatPath(runDir, specialist), "utf8");
  } catch {
    return null;
  }
  let parsed;
...
function emitBusEvent(runDir, input) {
  try {
    const busDir = path13.join(runDir, "agent-bus");
    fs10.mkdirSync(busDir, { recursive: true });
    const record = buildBusEvent(input);
    fs10.appendFileSync(
      path13.join(busDir, "events.ndjson"),
...
function readAcceptanceForInstance(cwd, ids) {
  ...
  try {
    const raw = fs11.readFileSync(absUnderCwd(cwd, paths.acceptance_path), "utf8");
...
function readAssignmentForInstance(cwd, ids) {
  const paths = taskCellPaths(ids);
  try {
    const raw = fs11.readFileSync(absUnderCwd(cwd, paths.assignment_path), "utf8");
    const obj = JSON.parse(raw);
...
function findRunAcceptances(cwd, runId) {
  ...
  let logicalTaskDirs;
  try {
    logicalTaskDirs = fs11.readdirSync(cellsRoot);
  } catch {
    return out;
  }
  for (const logical_task_id of logicalTaskDirs) {
    ...
    let attemptDirs;
    try {
      attemptDirs = fs11.readdirSync(attemptsRoot);
    } catch {
      continue;
    }
    for (const attemptStr of attemptDirs) {
      ...
      let instanceDirs;
      try {
        instanceDirs = fs11.readdirSync(instancesRoot);
      } catch {
        continue;
      }
...
function findAssignedTaskIds(cwd, teammate) {
  ...
  const ids = [];
  for (const file of files) {
    const content = fs12.readFileSync(path16.join(planDir, file), "utf8");
...
`
  ...
  if (writeAuth.ok === true) {
    const boundRunDir = path16.join(guildRootForRun, ".guild", "runs", writeAuth.run_id);
