...
function assertNotUnderPluginInstall(absPath, pluginInstallRoot) {
  ...
  const rel = path2.relative(resolvedRoot, path2.resolve(absPath));
  if (rel === "" || !rel.startsWith("..") && !path2.isAbsolute(rel)) {
    const underOwnDotGuild = rel === ".guild" || rel.startsWith(`.guild${path2.sep}`);
...
function countWorkflowFiles(moduleDir) {
  const workflowsDir = path4.join(moduleDir, "workflows");
  return walkTsFiles(workflowsDir).length;
}
function countModuleResources(root, moduleId) {
  const manifestAbs = path4.join(root, "src", "modules", moduleId, "resources", "module-resources.json");
...
function validateModuleHealth(root, manifests) {
  ...
  for (const manifest of manifests) {
    ...
    const resourcesManifest = path4.join(moduleDir, "resources", "module-resources.json");
    const resourcesMarker = path4.join(moduleDir, "resources", ".generated-by-guild-module-resources");
...
function moduleIdForPath(modulesDir, filePath, moduleIds) {
  const rel = path4.relative(modulesDir, filePath);
  if (rel.startsWith("..") || path4.isAbsolute(rel)) return null;
  const [moduleId] = rel.split(path4.sep);
...
var path10, SHARED_SCRUBBED_NAMES, CANONICAL_RUN_LOG;
var init_share_set = __esm({
  "../src/modules/security/workflows/share-set.ts"() {
    ...
    CANONICAL_RUN_LOG = path10.join("logs", "v1.4-events.jsonl");
  }
});
...
var fs9, path11, VALID_EVENTS, VALID_LAYERS, VALID_TERMINATED;
  ...
  "../src/modules/telemetry/workflows/receipt-journal.ts"() {
    ...
    defaultJournalIo = {
      appendLine(journalPath2, text, bound) {
        ...
        fs11.mkdirSync(path15.dirname(journalPath2), { recursive: true });
        const fd = fs11.openSync(journalPath2, "a");
        try {
      ...
      readAll(journalPath2, bound) {
        ...
        try {
          return fs11.readFileSync(journalPath2, "utf8");
        } catch {
          return null;
        }
      },
      ...
      truncate(journalPath2, size, bound) {
        ...
        fs11.truncateSync(journalPath2, size);
      },
      ...
      acquireLock(lockPath2) {
        fs11.mkdirSync(path15.dirname(lockPath2), { recursive: true });
        try {
          fs11.mkdirSync(lockPath2);
        } catch (err) {
...
function isExistingDirectory(target) {
  try {
    return fs12.statSync(target).isDirectory();
  } catch {
    return false;
  }
}
...
function evaluateReceiptJournalConformance(request) {
  ...
  const identity = request.evidence_identity;
  const workspace = fs12.mkdtempSync(path16.join(root, PROBE_WORKSPACE_PREFIX));
  const verdicts = {};
  try {
    for (const stableId of MH06_SCENARIO_IDS) {
...
function readRunBindingRecord(opts) {
  const fs21 = opts.fs ?? realBindingFs();
  const raw = fs21.readFile(runBindingPath(opts.root, opts.run_id));
...
var fs16, path21, WAVE_STATUSES, PROGRAM_STATUSES;
...
function nearestProjectRoot(startDir) {
  ...
  let nearestGuildDir = null;
  for (; ; ) {
    if (fs17.existsSync(path23.join(current, ".git"))) return current;
    if (nearestGuildDir === null) {
...
function emitBusEvent(runDir2, input) {
  try {
    const busDir = path25.join(runDir2, "agent-bus");
    fs19.mkdirSync(busDir, { recursive: true });
    const record = buildBusEvent(input);
    fs19.appendFileSync(
      path25.join(busDir, "events.ndjson"),
...
function loadPlanTaskIds(cwd) {
  const planDir = path26.join(resolveGuildRoot(cwd), ".guild", "plan");
  if (!fs20.existsSync(planDir)) return null;
  const files = fs20.readdirSync(planDir).filter((f) => f.endsWith(".md"));
  if (files.length === 0) return null;
  const ids = /* @__PURE__ */ new Set();
  for (const file of files) {
    const content = fs20.readFileSync(path26.join(planDir, file), "utf8");
    const patterns = [
      /\bid:\s*(task-[\w-]+)/gi,
      /^\s*[-*]\s*(task-[\w-]+):/gim,
      /task_id:\s*(task-[\w-]+)/gi,
...
async function main() {
  ...
  if (deps.length > 0) {
    ...
    if (planIds === null) {
      warn(
        `Task "${taskId}" has depends-on references [${deps.join(", ")}] but no plan file found at ${path26.join(resolveGuildRoot(cwd), ".guild/plan/")}. Skipping dependency check.`
      );
    } else {
  ...
  if (runStateAuth.ok === false) {
    ...
    const runDir2 = path26.join(guildRootForRun, ".guild", "runs", runId);
    try {
      markLaneInProgress(runDir2, { runId }, taskId);
      process.stderr.write(
        `[task-created] run-state: lane "${taskId}" \u2192 in_progress (${path26.join(runDir2, "run-state.json")}).
`
      );
    } catch (err) {
      process.stderr.write(
