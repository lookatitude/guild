...
import { casVerify, publish, readBusLog, type BusPublisher } from "../../communication";
import {
  isRefused,
  isWithin,
...
export interface EmitOptions {
  ...
  guildDir?: string;
  /** Deterministic clock/publisher overrides for receipts and tests. */
  now?: () => string;
  publisher?: BusPublisher;
}

function stationTopic(kind: typeof TEAM_PLAN_DIR | typeof TEAM_RESULT_DIR, station: StationId): string {
  return kind === TEAM_PLAN_DIR
    ? `context/station/${station}/team-plan`
...
function publishTeamArtifact(
  ...
  const runDir = path.join(cwd, opts.guildDir ?? ".guild", "runs", runId);
  ...
  if (!event?.sha256) {
    throw new Error(`writeTeamArtifact: canonical bus publish failed for ${kind}/${station}`);
  }
}

function teamArtifactMatchesBus(
  cwd: string,
  runId: string,
  kind: typeof TEAM_PLAN_DIR | typeof TEAM_RESULT_DIR,
  station: StationId,
  ...
  const runDir = path.join(cwd, opts.guildDir ?? ".guild", "runs", runId);
  const relativePath = path.relative(cwd, filePath);
  const event = readBusLog(runDir)
    .filter((item) => item.topic === stationTopic(kind, station) && item.path === relativePath)
    .at(-1);
  return Boolean(
    event &&
    event.event !== "artifact.retracted" &&
    event.sha256 &&
    casVerify(runDir, event.sha256, bytes),
  );
}

function readContainedTeamArtifact(cwd: string, filePath: string): Buffer | null {
  try {
...
export function readTeamPlan(
  ...
  const bytes = readContainedTeamArtifact(cwd, p);
  if (!bytes) return null;
  const raw = bytes.toString("utf8");
  if (!teamArtifactMatchesBus(cwd, runId, TEAM_PLAN_DIR, station, p, bytes, opts)) return null;
  let parsed: unknown;
  try {
...
export function readTeamResult(
  ...
  if (!bytes) return null;
  const raw = bytes.toString("utf8");
  if (!teamArtifactMatchesBus(cwd, runId, TEAM_RESULT_DIR, station, p, bytes, opts)) return null;
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
