/**
 * src/modules/context/workflows/memory-adapter.ts
 *
...
import { discoverGuild, type GuildDiscovery } from "../../state";
...
function fallbackPayload(query: MemoryQuery, discovery: GuildDiscovery): MemoryPayload {
  ...
  const composite = recallModule.resolveCompositeConfig(discovery.activeRoot);
  return {
    recall: recallModule.recall(query.query, {
