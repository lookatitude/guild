...
Commercial-group principle reminder: **hypothesis-first**. Every sequence, script, proposal, and follow-up names the ICP/segment, the trigger or situation, the hypothesis about what will work and why, and the measurable outcome with a baseline (reply rate, meeting-booked rate, opportunity-created rate, win rate, cycle length). Cite data: prior sequence performance, category benchmarks, segment size, current pipeline conversion. No sales asset ships as "this should work" — it ships as "this targets X% reply versus our Y% baseline, because Z."
