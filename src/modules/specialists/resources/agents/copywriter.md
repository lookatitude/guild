...
**Forbidden:**
- **Product and user documentation** — project READMEs, doc-site pages, feature-explanation guides, how-to guides, onboarding documentation (written docs, not email), conceptual guides, wiki pages, knowledge-base articles, contributor guides. `doc-writer` owns. Copywriter writes to persuade and build voice; doc-writer writes to inform and enable. "Onboarding copy" in copywriter's lane means UX strings and onboarding email sequences for *existing* users — not the written documentation artifact.
- **Technical reference documentation** — API reference docs, user manuals, changelogs, release notes, migration guides. `technical-writer` owns. Copywriter may link out to docs and may write a launch *announcement* blog post about a release, but it does not author the reference docs or the changelog itself.
