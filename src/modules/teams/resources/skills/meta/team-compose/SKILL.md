...
have_P    := roster-resolve(cwd)                 # CODE-BACKED enumeration (ADR §4 / D4): run
                                                 #   npx tsx ${GUILD_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/roster-resolve.ts --cwd .
                                                 # and consume its guild.roster.v1 JSON — the deterministic union of
                                                 # plugin/agents/*.md ∪ .guild/agents/*.md (+ project .guild/skills/*),
...
1. **Match.** (Non-self-build.) Build the **available-specialist set** by running the roster resolver — `npx tsx ${GUILD_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/roster-resolve.ts --cwd .` — and consuming its `guild.roster.v1` JSON (`roster` = merged union, `shipped` + `project` broken out, per-entry `name`, `definition`, `source`, `default_tier`, `skills`, `description`, `augmenting`). This is the code-backed D4 enumeration; never re-derive the set by walking directories yourself. **Filter out entries with `augmenting: true`** (`advisor`/`developer`/`doc-writer` — they augment lanes and stay dispatchable, but are never product-roster candidates and never count toward the cap). The union is **(a)** the shipped roster from `plugin/agents/*.md` (the canonical source per the specialist-roster ADR, D4 — never a hand-maintained list). As of this writing that is the **14 shipping specialists** (`§6.1–§6.3`): architect, researcher, backend, frontend, devops, qa, mobile, security, copywriter, technical-writer, social-media, seo, marketing, sales — but treat this as a derived snapshot of the files, not the authority. **And (b)** the consuming repo's project-local specialists already minted under `.guild/agents/*.md` (the live tree only — the `.guild/agents/proposed/*` incubation tree is **never** a candidate, per `guild:create-specialist`'s incubation contract). A domain either source covers is a candidate match; a project-local specialist minted on a previous task is **reused here, never re-created**.
...
specialists:
  - name: architect        # exact roster slug
    ...
    definition: agents/architect.md   # agent-definition path from roster-resolve (`definition` field).
                            #   Shipped: plugin-relative, informational. Project-local: `.guild/agents/<role>.md`,
                            #   LOAD-BEARING — dispatch embeds the definition-adoption instruction and the
                            #   in-process backend swaps subagent_type to the host-generic type.
    definition_source: shipped        # shipped | project — from roster-resolve (`source` field)
