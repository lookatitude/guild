...
${GUILD_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-$HOME/.local/share/guild/dist/claude-code}}/scripts/workspace/write-manifest.ts --cwd <workspace-root>`. A workspace builds
...
2a. **Detect repo kind (workspace vs regular).** Run `npx tsx
   ${GUILD_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-$HOME/.local/share/guild/dist/claude-code}}/scripts/workspace/detect.ts --cwd <root>` — it classifies the target
...
4. **Workspace path (federation, not duplication):** write the federation
   manifest with `npx tsx ${GUILD_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-$HOME/.local/share/guild/dist/claude-code}}/scripts/workspace/write-manifest.ts --cwd
