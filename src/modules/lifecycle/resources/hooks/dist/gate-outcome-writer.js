...
function initStableLockfile(runDir3) {
  const path27 = stableLockPath(runDir3);
...
function appendEvent(runDir3, event, opts = {}) {
  ...
  if (opts.forceFallback || process.platform === "win32") {
    ...
    const path27 = laneFallbackPath(runDir3, laneId2);
...
function runStatePath(runDir3) {
  return path14.join(runDir3, "run-state.json");
}
...
function laneResumeCheckpointPath(runDir3, laneId2) {
  return path14.join(runDir3, "lanes", laneId2, "resume.json");
}
...
var path25, HOST_FAMILY_TO_KIND, CANONICAL_PHASES, GATE_TOKEN;
...
function readCurrentRunId(cwd) {
  const sentinelPath = path26.join(resolveGuildRoot(cwd), ".guild", "runs", "current-run-id");
  try {
...
async function main() {
  ...
  const basename7 = path26.basename(filePath);
  ...
  const expectedDir = path26.join(root, ".guild", "runs", runId);
  const abs = path26.isAbsolute(filePath) ? filePath : path26.resolve(cwd, filePath);
  if (path26.dirname(abs) !== expectedDir) return;
  let content;
  try {
