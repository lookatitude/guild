...
/**
 * Compute the per-host tier→model map from the registry rows. For each host:
 *   - If the row's `capabilities.models.<tier>.model` is non-null, use it.
 *   - Otherwise fall back to `CLAUDE_TIER_FALLBACK` (the historical default for
 *     any host that has no model configured — preserves the prior "every host
 *     gets claudeDefaults" invariant that all three duplication sites had).
 *
...
): Record<HostKind, Record<Tier, string>> {
  const result = {} as Record<HostKind, Record<Tier, string>>;
 ...
 * CLAUDE_TIER_FALLBACK — same behavior as the prior exhaustive switch.
 */
