...
describe("write-run-manifest — initRunManifest (RE-6)", () => {
  ...
  it("idempotent: re-init does NOT clobber existing wave progress", () => {
    ...
    upsertWave(root, "alpha", { wave: 1, status: "completed", run_id: "run-x" });
...
describe("write-run-manifest — upsertWave (RE-6)", () => {
  ...
  it("appends a new wave with defaults when wave is unknown", () => {
    const root = mkRoot();
    const m = upsertWave(root, "alpha", { wave: 2 });
    const w = m.waves.find((x) => x.wave === 2)!;
  ...
  it("merges fields onto an existing wave (does not duplicate)", () => {
    const root = mkRoot();
    upsertWave(root, "alpha", { wave: 1, name: "foundation", status: "pending" });
    const m = upsertWave(root, "alpha", { wave: 1, status: "active", run_id: "run-1" });
    expect(m.waves.filter((w) => w.wave === 1)).toHaveLength(1);
    const w = m.waves.find((x) => x.wave === 1)!;
  ...
  it("auto-stamps started_at when a wave becomes active", () => {
    const root = mkRoot();
    const m = upsertWave(root, "alpha", { wave: 1, status: "active" });
    const w = m.waves.find((x) => x.wave === 1)!;
  ...
  it("auto-stamps completed_at when a wave completes or fails", () => {
    const root = mkRoot();
    const done = upsertWave(root, "alpha", { wave: 1, status: "completed" });
    expect(done.waves[0].completed_at).not.toBeNull();
    const failed = upsertWave(root, "alpha", { wave: 2, status: "failed" });
    expect(failed.waves.find((w) => w.wave === 2)!.completed_at).not.toBeNull();
  });

  it("current_wave is the lowest non-completed wave (the resume point)", () => {
    const root = mkRoot();
    upsertWave(root, "alpha", { wave: 1, status: "completed" });
    upsertWave(root, "alpha", { wave: 3, status: "pending" });
    const m = upsertWave(root, "alpha", { wave: 2, status: "active" });
  ...
  it("current_wave points at the last wave when all are completed", () => {
    const root = mkRoot();
    upsertWave(root, "alpha", { wave: 1, status: "completed" });
    const m = upsertWave(root, "alpha", { wave: 2, status: "completed" });
    expect(m.current_wave).toBe(2);
  });

  it("keeps waves sorted by wave number", () => {
    const root = mkRoot();
    upsertWave(root, "alpha", { wave: 3 });
    upsertWave(root, "alpha", { wave: 1 });
    const m = upsertWave(root, "alpha", { wave: 2 });
    expect(m.waves.map((w) => w.wave)).toEqual([1, 2, 3]);
  });
});
...
describe("write-run-manifest — setProgramStatus + read (RE-6)", () => {
  ...
  it("never creates .guild/wiki/", () => {
    const root = mkRoot();
    initRunManifest(root, "alpha");
    upsertWave(root, "alpha", { wave: 1, status: "active" });
...
describe("write-run-manifest — CLI (RE-6)", () => {
  ...
  it("--wave upserts a wave; --show prints JSON with the wave recorded", () => {
    const root = mkRoot();
    run(["--cwd", root, "--slug", "alpha", "--init"]);
    run(["--cwd", root, "--slug", "alpha", "--wave", "1", "--wave-name", "foundation", "--wave-status", "completed", "--run-id", "run-9"]);
