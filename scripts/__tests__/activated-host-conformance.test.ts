...
function packageFixtureWithInstallSubset(host: HostId) {
  ...
  return {
    ...fixture,
    runtimePackage: snapshotMigrationRuntimePackage(fixture.root, host, supportedInstallRoot),
  };
}
...
function workerCommand(host: HostId, runtimePackage: ReturnType<typeof snapshotMigrationRuntimePackage>): string {
  return [
    ...
    `--install-surface-hash ${runtimePackage.install_surface_sha256}`,
...
function receipt(host: HostId, runtimePackage: ReturnType<typeof snapshotMigrationRuntimePackage>, packageRoot: string, rawTranscript: string, rawWorker: string) {
  const command = workerCommand(host, runtimePackage);
  const withoutId = {
...
function verify(host: HostId) {
  const fixture = packageFixture(host);
  const command = workerCommand(host, fixture.runtimePackage);
...
describe("activated-host conformance capture", () => {
  it.each(["claude-code-cli", "codex-cli"] as const)("verifies exact %s generated bytes when the attested install surface is a proper subset", (host) => {
    const fixture = packageFixtureWithInstallSubset(host);
    const command = workerCommand(host, fixture.runtimePackage);
    const rawWorker = workerBytes(host, fixture.runtimePackage);
    const rawTranscript = transcript(host, fixture.root, command, fixture.runtimePackage, rawWorker);
    const value = receipt(host, fixture.runtimePackage, fixture.root, rawTranscript, rawWorker);
    const verifyFixture = () => verifyActivatedHostConformanceReceipt({
      receipt: value,
      transcript: rawTranscript,
      workerResult: rawWorker,
      packageRoot: fixture.root,
    ...
    expect(fixture.runtimePackage.install_files.length).toBeLessThan(fixture.runtimePackage.files.length);
    expect(verifyFixture()).toEqual(expect.objectContaining({ ok: true, code: "verified" }));

    fs.appendFileSync(path.join(fixture.root, "hooks", "dist", "runtime.cjs"), "// planted full-tree drift\n");
    expect(verifyFixture()).toEqual(expect.objectContaining({ ok: false, code: "runtime_package_mismatch" }));
  });

  it("refuses install-surface drift even when the attestation uses a proper subset", () => {
    const fixture = packageFixtureWithInstallSubset("claude-code-cli");
    const command = workerCommand("claude-code-cli", fixture.runtimePackage);
  ...
  it("executes the shipped worker bundle from both freshly generated host packages without an ambient TypeScript loader", () => {
    ...
    for (const [host, packageName] of [["claude-code-cli", "claude-code"], ["codex-cli", "codex"]] as const) {
      ...
      const result = spawnSync(process.execPath, [
        ...
        "--package-hash", runtimePackage.tree_sha256, "--install-surface-hash", runtimePackage.install_surface_sha256, "--runtime-version", RUNTIME_VERSION,
        "--host-version", host === "claude-code-cli" ? "2.1.236" : "0.147.0",
  ...
  it("consumes a Codex SessionStart receipt before exact-tree self-snapshotting", () => {
    ...
    const result = runActivatedConformanceWorker({
      ...
      installSurfaceHash: attested.install_surface_sha256,
  ...
  it("evaluates Codex module boundaries from an exact package twin whose consumers are true siblings", () => {
    ...
    const result = runActivatedConformanceWorker({
      ...
      installSurfaceHash: fixture.runtimePackage.install_surface_sha256,
      runtimeVersion: RUNTIME_VERSION,
      hostVersion: "0.147.0",
      platform: PLATFORM,
      release: RELEASE,
  ...
  it("refuses a module-boundary package twin whose bytes differ from the activated runtime", () => {
    ...
    const result = runActivatedConformanceWorker({
      ...
      packageHash: fixture.runtimePackage.tree_sha256, installSurfaceHash: fixture.runtimePackage.install_surface_sha256, runtimeVersion: RUNTIME_VERSION,
  ...
  it("drives the six production owners into one canonical 31-result worker payload", () => {
    ...
    const result = runActivatedConformanceWorker({
      ...
      installSurfaceHash: runtimePackage.install_surface_sha256,
      runtimeVersion: RUNTIME_VERSION,
      hostVersion: "2.1.236",
      platform: PLATFORM,
      release: RELEASE,
