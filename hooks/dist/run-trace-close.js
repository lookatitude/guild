...
function loadLocalOverride(cwd, fileConfig, selfBuild) {
  const localPath = path12.join(cwd, ".guild", "settings.local.json");
...
function loadFileConfig(cwd, selfBuild) {
  const settingsPath = path12.join(cwd, ".guild", "settings.json");
...
function appendKnowledgeLinksIndex(guildRoot, links) {
  ...
  const indexDir = path13.join(guildRoot, ".guild", "indexes");
  const indexPath = path13.join(indexDir, "knowledge-links.json");
  let existing = [];
...
function appendReflections(guildRoot, runId, phase, decisions) {
  ...
  const reflectionsDir = path13.join(guildRoot, ".guild", "reflections");
  fs10.mkdirSync(reflectionsDir, { recursive: true });
  const reflPath = path13.join(reflectionsDir, `${runId}.md`);
  const entry = `
## Phase: ${phase} (${runId})
...
function writeCheckpoint(opts) {
  ...
  const learningDir = path13.join(guildRoot, ".guild", "runs", opts.runId, "learning");
  fs10.mkdirSync(learningDir, { recursive: true });
  const checkpointFile = path13.join(learningDir, `${opts.phase}-${opts.runId}.yaml`);
  const reflectionsRelPath = `.guild/reflections/${opts.runId}.md`;
  const reflectionsAbsPath = path13.join(guildRoot, ".guild", "reflections", `${opts.runId}.md`);
...
function newestPostCloseActivityMs(root, runId) {
  ...
  const candidates = [liveLogPath2(root, runId), path14.join(dir, "events.ndjson")];
  try {
...
function findTerminalCheckpoint(runDir3, runId) {
  const learningDir = path15.join(runDir3, "learning");
  if (!fs12.existsSync(learningDir)) return null;
  const reflectionFile = path15.join(learningDir, `reflection-${runId}.yaml`);
  ...
  const sorted = yamlFiles.map((f) => path15.join(learningDir, f)).sort((a, b) => {
    try {
...
async function main3() {
  ...
  const runDir3 = path15.join(root, ".guild", "runs", runId);
  if (!fs12.existsSync(path15.join(runDir3, "run.yaml"))) process.exit(0);
  const provenanceFile = path15.join(runDir3, "provenance.json");
