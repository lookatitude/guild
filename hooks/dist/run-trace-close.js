...
function sessionContextPath(root, runId) {
  return path2.join(root, ".guild", "runs", runId, "session-context.json");
}
function writeSessionContext(root, ctx, fs13) {
  const f = fs13 ?? realFs();
  ...
  f.mkdirp(path2.dirname(p));
  f.writeFile(p, serialized);
}
var fsReal, path2, UNKNOWN_TARGET;
...
var fs3, path5, yaml, DEFAULTS2, VALID_TIER_HOST_KEYS, KNOWN_HOST_IDS2, VALID_LOOPS, VALID_RIGOR, VALID_REVIEW, DISPATCH_HOST_IDS, VALID_AGENT_MODE, VALID_CACHE_TTL, DEFAULTS_ALLOWED_KEYS;
...
function loadLocalOverride(cwd, fileConfig, selfBuild) {
  ...
  if (!fs9.existsSync(localPath)) return fileConfig;
  let localParsed;
  try {
...
function loadFileConfig(cwd, selfBuild) {
  ...
  if (fs9.existsSync(settingsPath)) {
    let parsed;
    try {
      parsed = JSON.parse(fs9.readFileSync(settingsPath, "utf8"));
    } catch (e) {
...
`);
  ...
  if (fs9.existsSync(localPath)) {
    try {
      const rawLocal = JSON.parse(fs9.readFileSync(localPath, "utf8"));
...
function mintRunBinding(opts) {
  const fs13 = opts.fs ?? realBindingFs();
  ...
  fs13.mkdirp(path9.dirname(p));
  fs13.writeFile(p, JSON.stringify(record, null, 2) + "\n");
  return record;
}
...
function readRunBindingRecord(opts) {
  const fs13 = opts.fs ?? realBindingFs();
  const raw = fs13.readFile(runBindingPath(opts.root, opts.run_id));
...
function reopenRunBinding(opts, binding_ref) {
  ...
  fs13.writeFile(runBindingPath(opts.root, opts.run_id), JSON.stringify(reopened, null, 2) + "\n");
  return reopened;
}
...
function appendSecurityEvent(runDir3, record) {
  try {
    ...
    fs7.appendFileSync(path11.join(logsDir2, "security-events.jsonl"), JSON.stringify(record) + "\n", "utf8");
    return true;
  } catch (err) {
    process.stderr.write(
...
function writeScrubApprovalRequest(runDir3, runId, surface, outPath, laneId) {
  try {
    ...
    fs8.writeFileSync(path12.join(approvalDir, fileName), content, "utf8");
  } catch {
  }
}
function scrubbedWrite(outPath, content, opts) {
  ...
  if (scrubResult.ok) {
    try {
      fs8.mkdirSync(path12.dirname(outPath), { recursive: true });
...
function writeResolvedSettingsSnapshot(runId, snapshot, opts) {
  ...
  const fs13 = fsSeam ?? realProvenanceFsSeam();
  ...
  if (fs13.scrubbedWriteDurable) {
    const runDir3 = path13.join(cwd, ".guild", "runs", runId);
    const result = fs13.scrubbedWriteDurable(outPath, serialized, "config", runDir3, runId);
    if (result.blocked) {
      process.stderr.write(
...
`
    ...
    fs13.writeFile(outPath, serialized);
  }
  return outPath;
}
...
function writeCheckpoint(opts) {
  ...
  fs10.writeFileSync(checkpointFile, yaml2, "utf8");
...
function newestPostCloseActivityMs(root, runId) {
  ...
  const candidates = [liveLogPath2(root, runId), path16.join(dir, "events.ndjson")];
  try {
...
function findTerminalCheckpoint(runDir3, runId) {
  const learningDir = path17.join(runDir3, "learning");
  if (!fs12.existsSync(learningDir)) return null;
  const reflectionFile = path17.join(learningDir, `reflection-${runId}.yaml`);
  if (fs12.existsSync(reflectionFile)) return reflectionFile;
  let yamlFiles;
  try {
    yamlFiles = fs12.readdirSync(learningDir).filter((f) => f.endsWith(".yaml"));
  } catch {
    return null;
  }
  if (yamlFiles.length === 0) return null;
  const sorted = yamlFiles.map((f) => path17.join(learningDir, f)).sort((a, b) => {
    try {
      return fs12.statSync(b).mtimeMs - fs12.statSync(a).mtimeMs;
    } catch {
      return 0;
    }
  });
...
async function main3() {
  ...
  const runDir3 = path17.join(root, ".guild", "runs", runId);
  if (!fs12.existsSync(path17.join(runDir3, "run.yaml"))) process.exit(0);
  const provenanceFile = path17.join(runDir3, "provenance.json");
  if (fs12.existsSync(provenanceFile)) {
    let terminal = true;
    let closedAtMs = 0;
    try {
      const prov = JSON.parse(fs12.readFileSync(provenanceFile, "utf8"));
