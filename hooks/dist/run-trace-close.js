...
function newestPostCloseActivityMs(root, runId) {
  ...
  const candidates = [liveLogPath(root, runId), path10.join(dir, "events.ndjson")];
  try {
    const inProgress = path10.join(dir, "in-progress");
    for (const name of fs8.readdirSync(inProgress)) {
      candidates.push(path10.join(inProgress, name));
    }
  } catch {
  }
  let newest = 0;
...
function findTerminalCheckpoint(runDir3, runId) {
  const learningDir = path11.join(runDir3, "learning");
  if (!fs9.existsSync(learningDir)) return null;
  const reflectionFile = path11.join(learningDir, `reflection-${runId}.yaml`);
  if (fs9.existsSync(reflectionFile)) return reflectionFile;
  let yamlFiles;
  try {
    yamlFiles = fs9.readdirSync(learningDir).filter((f) => f.endsWith(".yaml"));
  } catch {
    return null;
  }
  if (yamlFiles.length === 0) return null;
  const sorted = yamlFiles.map((f) => path11.join(learningDir, f)).sort((a, b) => {
    try {
      return fs9.statSync(b).mtimeMs - fs9.statSync(a).mtimeMs;
    } catch {
      return 0;
    }
  });
...
async function main2() {
  ...
  if (!fs9.existsSync(path11.join(runDir3, "run.yaml"))) process.exit(0);
  const provenanceFile = path11.join(runDir3, "provenance.json");
  if (fs9.existsSync(provenanceFile)) {
    let terminal = true;
    let closedAtMs = 0;
    try {
      const prov = JSON.parse(fs9.readFileSync(provenanceFile, "utf8"));
