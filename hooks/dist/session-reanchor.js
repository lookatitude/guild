...
function readRunStateLanes(runDir3) {
  const obj = readJsonObject(path10.join(runDir3, "run-state.json"));
...
function readHeartbeatAges(runDir3, now = Date.now()) {
  ...
  const dir = path10.join(runDir3, "in-progress");
  let names;
  try {
...
function readReceiptEvidence(runDir3) {
  const dir = path10.join(runDir3, "handoffs");
  let names;
  try {
    names = fs9.readdirSync(dir);
  } catch {
...
function laneResumeCheckpointPath(runDir3, laneId2) {
  return path12.join(runDir3, "lanes", laneId2, "resume.json");
}
...
function liveLogPath(runDir3) {
  return path18.join(runDir3, "logs", "v1.4-events.jsonl");
}
...
function scanResumableLanes(runDir3, cwd, slug) {
  ...
  const lanesDir = path20.join(runDir3, "lanes");
  let entries;
  try {
...
function runDir2(root, runId) {
  return path21.join(root, ".guild", "runs", runId);
}
function resolvedSettingsPath(root, runId) {
  return path21.join(runDir2(root, runId), "resolved-settings.json");
}
...
function assertContained(target, cwd, label) {
  ...
  if (isRefused(r)) {
    throw new Error(
      `[run-lifecycle] ${label}: resolved path "${path21.resolve(target)}" escapes the project root "${path21.resolve(cwd)}" [${r.code}] \u2014 ${r.detail}`
    );
  }
  const runsBase = path21.resolve(cwd, ".guild", "runs");
  const resolvedTarget = path21.resolve(target);
...
function realProvenanceFsSeam() {
  return {
    writeFile(absPath, contents) {
      fsNode.mkdirSync(path21.dirname(absPath), { recursive: true });
...
function readResolvedSettingsSnapshot(runId, opts) {
  ...
  const fs24 = fsSeam ?? realProvenanceFsSeam();
  ...
  const raw = fs24.readFile(filePath);
...
var fsNode, path21, HOST_FAMILY_TO_KIND, CANONICAL_PHASES;
...
function appendSecurityEvent(runDir3, record) {
  try {
    const logsDir = path27.join(runDir3, "logs");
    fs21.mkdirSync(logsDir, { recursive: true });
    fs21.appendFileSync(path27.join(logsDir, "security-events.jsonl"), JSON.stringify(record) + "\n", "utf8");
    return true;
  } catch (err) {
    process.stderr.write(
...
var fs21, path27, SECURITY_EVENT_SCHEMA_VERSION, KNOWN_GUILD_HOST_KINDS, KNOWN_GUILD_HOST_ID_SET, LEGACY_HOST_ALIASES2;
...
function writeScrubApprovalRequest(runDir3, runId, surface, outPath, laneId2) {
  try {
    ...
    fs22.writeFileSync(path28.join(approvalDir, fileName), content, "utf8");
  } catch {
  }
}
function scrubbedWrite(outPath, content, opts) {
  ...
  if (scrubResult.ok) {
    try {
      fs22.mkdirSync(path28.dirname(outPath), { recursive: true });
