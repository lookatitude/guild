...
function readProvenance(issues, parent, path39) {
  ...
  const provenancePath2 = `${path39}.provenance`;
...
function initStableLockfile(runDir3) {
  const path39 = stableLockPath(runDir3);
...
function appendEvent(runDir3, event, opts = {}) {
  ...
  if (opts.forceFallback || process.platform === "win32") {
    ...
    const path39 = laneFallbackPath(runDir3, laneId2);
...
async function readArchive(path39) {
  const chunks = [];
  const src = (0, import_node_fs2.createReadStream)(path39);
...
function readRunYamlFacts(guildRoot, runId) {
  const runYamlPath2 = path37.join(guildRoot, ".guild", "runs", runId, "run.yaml");
  let raw;
  try {
...
function newestRunSignalMs(guildRoot, runId) {
  ...
  for (const sub of ["logs", "handoffs", "in-progress"]) {
    try {
      for (const name of fs30.readdirSync(path37.join(dir, sub))) {
...
function cliMain(argv) {
  ...
  if (!(0, import_node_fs3.existsSync)(path39)) {
    process.stderr.write(`error: file not found: ${path39}
`);
    return 2;
  }
  const text = (0, import_node_fs3.readFileSync)(path39, "utf8");
  const summary = validateText(text);
  if (summary.invalid > 0) {
    for (const { line, result } of summary.perLine) {
      if (!result.ok) {
        for (const e of result.errors) {
          process.stderr.write(`${path39}:${line}: ${e}
`);
        }
      }
    }
...
function isGateDecisionReply(promptText) {
  if (typeof promptText !== "string") return false;
  const normalized = promptText.replace(/\r\n?/g, "\n").trim();
  const isExactDecision = (value) => /^(?:approve|approved)$/i.test(value.trim());
...
${close}`)) return false;
  ...
  return !body.includes("<in-app-browser-context") && !body.includes("</in-app-browser-context>");
}
...
function gateStatePath(runDir3) {
  return path38.join(runDir3, "lifecycle-gate-state.json");
}
function closeStatePath(runDir3) {
  return path38.join(runDir3, "lifecycle-close-state.json");
}
...
function writeJsonAtomic(finalPath, value) {
  fs31.mkdirSync(path38.dirname(finalPath), { recursive: true });
  const tmpPath = `${finalPath}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
async function resolveGateContext(guildRoot, runId, env) {
  ...
  const runDir3 = env["GUILD_RUN_DIR"] ?? path38.join(guildRoot, ".guild", "runs", safeRunId);
...
function validateReceiptEnvelope(content) {
  ...
  if (validateHandoffV22(block).valid) return { ok: true, reason: null };
  const obj = block;
...
async function evaluateCloseGate(guildRoot, runId, env = process.env) {
  ...
  const handoffsDir = path38.resolve(ctx.runDir, "handoffs");
  const seenReceipts = /* @__PURE__ */ new Set();
  const malformed = [];
  for (const ref of ctx.runState.receiptRefs) {
    if (ref === null) return silent;
    const resolved = path38.resolve(ctx.runDir, ref);
    const rel = path38.relative(handoffsDir, resolved);
    if (rel === "" || rel.startsWith("..") || path38.isAbsolute(rel)) return silent;
    ...
    if (!validateReceiptEnvelope(receiptText).ok) malformed.push(path38.basename(resolved));
  }
  malformed.sort();
  const missing = [];
  for (const artifact of ["review.md", "verify.md"]) {
    if (!hasContent(path38.join(ctx.runDir, artifact))) missing.push(artifact);
  }
  missing.sort();
