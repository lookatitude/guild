...
function loadInventoryAllowList() {
  const inventoryPath = path.resolve(
    __dirname,
    // comms/ → scripts/ → plugin/ → guild-root/
    "../../..",
...
function lintCommsFormat(opts = {}) {
  ...
  if (opts.runsDir) {
    try {
      ...
      const runDirs = fs.readdirSync(opts.runsDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => path.join(opts.runsDir, d.name));
      for (const runDir of runDirs) {
        if (!isRunInScope(runDir)) continue;
        const handoffsDir = path.join(runDir, "handoffs");
        if (!fs.existsSync(handoffsDir)) continue;
        const receiptFiles = fs.readdirSync(handoffsDir, { withFileTypes: true }).filter((f) => f.isFile() && f.name.endsWith(".md")).map((f) => path.join(handoffsDir, f.name));
