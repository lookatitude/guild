...
async function readArchive(path37) {
  const chunks = [];
  const src = (0, import_node_fs2.createReadStream)(path37);
...
function advisoryStatePath(runDir3) {
  return path36.join(runDir3, "lean-lead-advisory-state.json");
}
...
async function evaluateLeanLeadGuard(guildRoot, runId, env = process.env) {
  ...
  const runDir3 = env["GUILD_RUN_DIR"] ?? path36.join(guildRoot, ".guild", "runs", safeRunId);
