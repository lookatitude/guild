...
function writeSessionContext(root, ctx, fs31) {
  const f = fs31 ?? realFs();
...
function loadSessionContext(root, runId, fs31) {
  const f = fs31 ?? realFs();
...
function mintRunBinding(opts) {
  const fs31 = opts.fs ?? realBindingFs();
  ...
  fs31.mkdirp(path23.dirname(p));
  fs31.writeFile(p, JSON.stringify(record, null, 2) + "\n");
  return record;
}
...
function readRunBindingRecord(opts) {
  const fs31 = opts.fs ?? realBindingFs();
  const raw = fs31.readFile(runBindingPath(opts.root, opts.run_id));
...
function reopenRunBinding(opts, binding_ref) {
  ...
  fs31.writeFile(runBindingPath(opts.root, opts.run_id), JSON.stringify(reopened, null, 2) + "\n");
  return reopened;
}
...
function locateCandidateRunId(root, fs31) {
  const f = fs31 ?? realBindingFs();
  const candidates = [
  ...
  return {
    ...
    closeRun(runId, opts) {
      ...
      if (env.fs.scrubbedWriteDurable) {
        const runDir3 = path35.join(root, ".guild", "runs", runId);
...
function writeResolvedSettingsSnapshot(runId, snapshot, opts) {
  ...
  const fs31 = fsSeam ?? realProvenanceFsSeam();
  ...
  if (fs31.scrubbedWriteDurable) {
    const runDir3 = path35.join(cwd, ".guild", "runs", runId);
    const result = fs31.scrubbedWriteDurable(outPath, serialized, "config", runDir3, runId);
    if (result.blocked) {
      process.stderr.write(
...
`
    ...
    fs31.writeFile(outPath, serialized);
  }
  return outPath;
}
...
function readResolvedSettingsSnapshot(runId, opts) {
  ...
  const raw = fs31.readFile(filePath);
  if (raw === null) return null;
  try {
    return JSON.parse(raw);
  } catch {
...
function appendGateOutcome(fs31, root, runId, gate, record) {
  if (!GATE_TOKEN.test(gate)) return false;
  const p = runYamlPath(root, runId);
  const raw = fs31.readFile(p);
  if (raw === null) return false;
  const next = writeGateBlock(raw, gate, record);
  if (next === null) return false;
  fs31.writeFile(p, next);
  return true;
}
function readRunStartedAt(runDir3, readFile = (p) => {
  ...
  const p = path35.join(runDir3, "run.yaml");
...
function writeCheckpoint(opts) {
  ...
  fs29.writeFileSync(checkpointFile, yaml5, "utf8");
...
function countOpenLanes(runDir3) {
  const state = loadRunState(runDir3);
...
async function countHandsOnEdits(runDir3, runId) {
  const events = await readAllEvents(runDir3);
  let anchorIndex = -1;
  let anchorTs = null;
...
function advisoryStatePath(runDir3) {
  return path37.join(runDir3, "lean-lead-advisory-state.json");
}
function loadAdvisoryState(runDir3) {
  try {
    const raw = fs30.readFileSync(advisoryStatePath(runDir3), "utf8");
    const parsed = JSON.parse(raw);
...
function writeAdvisoryState(runDir3, state) {
  fs30.mkdirSync(runDir3, { recursive: true });
  const finalPath = advisoryStatePath(runDir3);
  const tmpPath = `${finalPath}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
function decideCrossing(count, threshold, anchorTs, priorState) {
  const sameCycle = priorState !== null && priorState.anchor_ts === anchorTs && priorState.last_fired_at_count <= count;
  const priorCount = sameCycle ? priorState.last_fired_at_count : 0;
  const isFreshPostCompactionCycle = anchorTs !== null && !sameCycle;
  const crossed = isFreshPostCompactionCycle ? count >= 1 : count >= threshold && Math.floor(count / threshold) > Math.floor(priorCount / threshold);
  if (!crossed) {
    return {
      shouldFire: false,
      nextState: sameCycle ? priorState : { schema_version: ADVISORY_STATE_SCHEMA, anchor_ts: anchorTs, last_fired_at_count: priorCount }
    };
  }
  return {
    shouldFire: true,
...
function renderLeanLeadAdvisory(runId, count, threshold, openLanes) {
  return [
    `${LEAN_LEAD_MARKER} inline shortcut expired on run ${runId} \u2014 ${count} direct lead Edit/Write ops (threshold ${threshold}) while ${openLanes} lane(s) are still open.`,
...
async function evaluateLeanLeadGuard(guildRoot, runId, env = process.env) {
  ...
  const openLanes = countOpenLanes(runDir3);
  if (openLanes === 0) return { advisory: null };
  const { count, anchorTs } = await countHandsOnEdits(runDir3, safeRunId);
  if (count === 0) return { advisory: null };
  const priorState = loadAdvisoryState(runDir3);
  const decision = decideCrossing(count, config.threshold, anchorTs, priorState);
  if (!decision.shouldFire) return { advisory: null };
  writeAdvisoryState(runDir3, decision.nextState);
...
async function main2() {
  ...
  try {
    ...
    process.stderr.write(
      `warn: [lean-lead-guard] evaluation failed: ${err instanceof Error ? err.message : String(err)}
`
    );
  }
}
