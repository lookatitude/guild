...
function runLearningBackstop(opts) {
  try {
    const runDir = opts.runDir ?? path5.join(opts.guildRoot, ".guild", "runs", opts.runId);
    let raw;
    try {
      raw = fs4.readFileSync(path5.join(runDir, "run.yaml"), "utf8");
    } catch {
      return { ...EMPTY_RESULT };
    }
    const result = {
    ...
    for (const token of tokens) {
      ...
      const checkpointFile = path5.join(
        runDir,
        "learning",
        `${phase}-${opts.runId}.yaml`
      );
      if (fs4.existsSync(checkpointFile)) {
