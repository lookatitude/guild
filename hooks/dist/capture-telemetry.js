...
`
  ...
  const runsDir = path30.join(resolveGuildRoot(cwd), ".guild", "runs", runId);
  ...
  const logsDir = path30.join(runsDir, "logs");
  const canonicalFile = path30.join(logsDir, "v1.4-events.jsonl");
  const legacyFile = path30.join(runsDir, "events.ndjson");
  ...
  if (semanticClass) {
    emitTraceEvent(
      makeAnalysisTraceEvent({
        ...
        duration_ms: ms,
        tokens,
        config_snapshot_ref: fs23.existsSync(path30.join(runsDir, "plugin-config-snapshot.json")) ? "plugin-config-snapshot.json" : void 0,
