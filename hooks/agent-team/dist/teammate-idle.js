...
function assessReceipts(runDir, teammate) {
  const handoffsDir = path9.join(runDir, "handoffs");
  if (!fs8.existsSync(handoffsDir)) return [];
  const prefix = `${teammate}-`;
  const results = [];
  const files = fs8.readdirSync(handoffsDir).filter((f) => f.startsWith(prefix) && f.endsWith(".md"));
  for (const file of files) {
    const taskId = file.slice(prefix.length, -".md".length);
    const rPath = path9.join(handoffsDir, file);
...
function findAssignedTaskIds(cwd, teammate) {
  const planDir = path9.join(resolveGuildRoot(cwd), ".guild", "plan");
  if (!fs8.existsSync(planDir)) return [];
  const files = fs8.readdirSync(planDir).filter((f) => f.endsWith(".md"));
  const ids = [];
  for (const file of files) {
    const content = fs8.readFileSync(path9.join(planDir, file), "utf8");
...
`);
  ...
  const runDir = path9.join(guildRootForRun, ".guild", "runs", runId);
