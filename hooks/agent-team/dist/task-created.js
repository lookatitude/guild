...
function parseWorkspaceManifest(manifestPath) {
  let raw;
  try {
    if (!fs4.existsSync(manifestPath)) return { status: "absent" };
    raw = fs4.readFileSync(manifestPath, "utf8");
  } catch (e) {
...
function parseSettingsFile(filePath) {
  ...
  let parsed;
  try {
    parsed = JSON.parse(fs5.readFileSync(filePath, "utf8"));
  } catch {
    return {};
  }
...
function parseLocalFile(guildDir) {
  ...
  if (!fs5.existsSync(localPath)) return {};
  let localParsed;
  try {
    localParsed = JSON.parse(fs5.readFileSync(localPath, "utf8"));
  } catch {
    return {};
  }
...
function initiativeIsWorkspaceScoped(workspaceRoot, id) {
  try {
    ...
    let yamlPath = null;
    if (isContainedIn(activePath, activeBase) && fs5.existsSync(activePath)) {
      yamlPath = activePath;
    } else if (isContainedIn(archivedPath, archivedBase) && fs5.existsSync(archivedPath)) {
      yamlPath = archivedPath;
    }
    if (yamlPath !== null) {
      try {
        const raw = fs5.readFileSync(yamlPath, "utf8");
        const parsed = yaml.load(raw);
        if (isPlainObject2(parsed)) {
...
function loadLocalOverride(cwd, fileConfig, selfBuild) {
  const localPath = path10.join(cwd, ".guild", "settings.local.json");
  if (!fs8.existsSync(localPath)) return fileConfig;
  let localParsed;
  try {
    localParsed = JSON.parse(fs8.readFileSync(localPath, "utf8"));
  } catch (e) {
    process.stderr.write(
...
function loadFileConfig(cwd, selfBuild) {
  const settingsPath = path10.join(cwd, ".guild", "settings.json");
  if (fs8.existsSync(settingsPath)) {
    let parsed;
    try {
      parsed = JSON.parse(fs8.readFileSync(settingsPath, "utf8"));
    } catch (e) {
...
`);
  ...
  if (fs8.existsSync(localPath)) {
    try {
      const rawLocal = JSON.parse(fs8.readFileSync(localPath, "utf8"));
...
function initStableLockfile(runDir) {
  const path14 = stableLockPath(runDir);
  (0, import_node_fs.mkdirSync)((0, import_node_path.dirname)(path14), { recursive: true });
  if ((0, import_node_fs.existsSync)(path14)) return;
  try {
    const fd = (0, import_node_fs.openSync)(path14, "wx");
...
function loadPlanTaskIds(cwd) {
  const planDir = path13.join(resolveGuildRoot(cwd), ".guild", "plan");
  if (!fs11.existsSync(planDir)) return null;
  const files = fs11.readdirSync(planDir).filter((f) => f.endsWith(".md"));
  if (files.length === 0) return null;
  const ids = /* @__PURE__ */ new Set();
  for (const file of files) {
    const content = fs11.readFileSync(path13.join(planDir, file), "utf8");
    const patterns = [
      /\bid:\s*(task-[\w-]+)/gi,
      /^\s*[-*]\s*(task-[\w-]+):/gim,
      /task_id:\s*(task-[\w-]+)/gi,
...
async function main3() {
  ...
  if (deps.length > 0) {
    ...
    if (planIds === null) {
      warn(
        `Task "${taskId}" has depends-on references [${deps.join(", ")}] but no plan file found at ${path13.join(resolveGuildRoot(cwd), ".guild/plan/")}. Skipping dependency check.`
      );
    } else {
  ...
  const runDir = path13.join(guildRootForRun, ".guild", "runs", runId);
  try {
    markLaneInProgress(runDir, { runId }, taskId);
    process.stderr.write(
      `[task-created] run-state: lane "${taskId}" \u2192 in_progress (${path13.join(runDir, "run-state.json")}).
`
    );
  } catch (err) {
    process.stderr.write(
