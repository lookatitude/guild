...
function discoverWorkspace(startDir) {
  let current = path11.dirname(startDir);
  const fsRoot = path11.parse(current).root;
  while (current !== fsRoot) {
    const manifestPath = path11.join(current, ".guild", "workspace.json");
...
function parseLocalFile(guildDir) {
  const localPath = path12.join(guildDir, "settings.local.json");
...
function isContainedIn(candidatePath, baseDir) {
  const resolved = path12.resolve(candidatePath);
  const resolvedBase = path12.resolve(baseDir);
  return resolved.startsWith(resolvedBase + path12.sep);
}
function initiativeIsWorkspaceScoped(workspaceRoot, id) {
  try {
    if (!isValidInitiativeId(id)) return false;
    const registryPath = path12.join(
      workspaceRoot,
      ".guild",
      "indexes",
    ...
    const initiativesBase = path12.join(workspaceRoot, ".guild", "initiatives");
    const activePath = path12.join(
      initiativesBase,
      "active",
      id,
      "initiative.yaml"
    );
    const archivedPath = path12.join(
      initiativesBase,
      "archived",
      id,
      "initiative.yaml"
    );
    const activeBase = path12.join(initiativesBase, "active");
    const archivedBase = path12.join(initiativesBase, "archived");
...
function resolveSettings(opts) {
  ...
  let wsLocalSettings = {};
  if (ws !== null) {
    const wsGuildDir = path12.join(ws.rootDir, ".guild");
    const rawWsSettings = parseSettingsFile(path12.join(wsGuildDir, "settings.json"));
    const wsInheritable = {};
  ...
  const projectGuildDir = path12.join(cwd, ".guild");
  const projectSettings = parseSettingsFile(path12.join(projectGuildDir, "settings.json"));
...
var fs10, path12, yaml, DEFAULTS2, VALID_TIER_HOST_KEYS, KNOWN_HOST_IDS2, VALID_LOOPS, VALID_RIGOR, VALID_REVIEW, DISPATCH_HOST_IDS, VALID_AGENT_MODE, VALID_CACHE_TTL, DEFAULTS_ALLOWED_KEYS;
...
function liveLogPath(runDir) {
  return path13.join(runDir, "logs", "v1.4-events.jsonl");
}
...
function resolveSettings2(opts) {
  ...
  try {
    ...
    const _traceRunDir = _traceRunId && cwd ? path14.join(cwd, ".guild", "runs", _traceRunId) : void 0;
    if (_traceRunDir) {
...
function loadLocalOverride(cwd, fileConfig, selfBuild) {
  const localPath = path16.join(cwd, ".guild", "settings.local.json");
  if (!fs13.existsSync(localPath)) return fileConfig;
  let localParsed;
  try {
...
function loadFileConfig(cwd, selfBuild) {
  const settingsPath = path16.join(cwd, ".guild", "settings.json");
  if (fs13.existsSync(settingsPath)) {
    let parsed;
    try {
      parsed = JSON.parse(fs13.readFileSync(settingsPath, "utf8"));
    } catch (e) {
...
`);
  ...
  if (fs13.existsSync(localPath)) {
    try {
      const rawLocal = JSON.parse(fs13.readFileSync(localPath, "utf8"));
...
function readRunBindingRecord(opts) {
  const fs18 = opts.fs ?? realBindingFs();
  const raw = fs18.readFile(runBindingPath(opts.root, opts.run_id));
...
function initStableLockfile(runDir) {
  const path21 = stableLockPath(runDir);
  (0, import_node_fs.mkdirSync)((0, import_node_path.dirname)(path21), { recursive: true });
  if ((0, import_node_fs.existsSync)(path21)) return;
  try {
    const fd = (0, import_node_fs.openSync)(path21, "wx");
...
function independenceDirForRunDir(runDir) {
  return path5.join(runDir, INDEPENDENCE_DIR);
}
function loadWrittenAdjudications(runDir) {
  const dir = independenceDirForRunDir(runDir);
  let names;
  try {
    names = fs3.readdirSync(dir).filter((n) => n.endsWith(".json")).sort();
  } catch {
    return [];
  }
  const out = [];
  for (const name of names) {
    let parsed;
    try {
      parsed = JSON.parse(fs3.readFileSync(path5.join(dir, name), "utf8"));
    } catch {
      continue;
    }
...
function loadRunState(runDir) {
  let raw;
  try {
    raw = fs4.readFileSync(runStatePath(runDir), "utf8");
  } catch {
    return null;
  }
  let parsed;
...
function appendSecurityEvent(runDir, record) {
  try {
    ...
    fs5.appendFileSync(path7.join(logsDir, "security-events.jsonl"), JSON.stringify(record) + "\n", "utf8");
    return true;
  } catch (err) {
    process.stderr.write(
...
function readSecurityConfig(cwd) {
  const settingsPath = path8.join(resolveGuildRoot(cwd), ".guild", "settings.json");
  let raw;
  try {
...
function writeScrubApprovalRequest(runDir, runId, surface, outPath, laneId) {
  try {
    ...
    fs7.writeFileSync(path9.join(approvalDir, fileName), content, "utf8");
  } catch {
  }
}
function scrubbedWrite(outPath, content, opts) {
  ...
  if (scrubResult.ok) {
    try {
      fs7.mkdirSync(path9.dirname(outPath), { recursive: true });
...
function emitBusEvent(runDir, input) {
  try {
    ...
    fs8.mkdirSync(busDir2, { recursive: true });
...
function runMigrations(dbPath) {
  ...
  let fromVersion = 0;
  try {
    fs12.mkdirSync(path15.dirname(dbPath), { recursive: true });
...
`
  ...
  try {
    fs14.mkdirSync(indexDir, { recursive: true });
    fs14.writeFileSync(
      indexPath,
      JSON.stringify(
...
function appendReflections(guildRoot, runId, phase, decisions) {
  ...
  fs14.mkdirSync(reflectionsDir, { recursive: true });
  const reflPath = path17.join(reflectionsDir, `${runId}.md`);
  const entry = `
## Phase: ${phase} (${runId})

` + nonNone.map((k) => `- ${k}: ${decisions[k]}`).join("\n") + "\n";
  fs14.appendFileSync(reflPath, entry, "utf8");
}
function writeCheckpoint(opts) {
  ...
  fs14.mkdirSync(learningDir, { recursive: true });
  ...
  fs14.writeFileSync(checkpointFile, yaml2, "utf8");
...
function main2() {
  ...
  if (verdictPath) {
    try {
      const raw = fs14.readFileSync(verdictPath, "utf8");
...
`
  ...
  if (decisions === void 0 && artifactsJsonPath) {
    try {
      const rawArtifacts = fs14.readFileSync(artifactsJsonPath, "utf8");
...
`
  ...
  if (linksPath) {
    try {
      const raw = fs14.readFileSync(linksPath, "utf8");
      const parsed = JSON.parse(raw);
...
function busDir(runDir) {
  return path18.join(runDir, "bus");
}
function busLogPath(runDir) {
  return path18.join(busDir(runDir), "log.jsonl");
}
...
var BusLockTimeout = class extends Error {
  constructor(runDir) {
    super(`artifact-bus: could not acquire ${path18.join(busDir(runDir), ".lock")} within timeout`);
...
function withLock(runDir, fn) {
  const lock = path18.join(busDir(runDir), ".lock");
...
function readSubscribers(runDir) {
  const dir = path18.join(busDir(runDir), "subscribers");
  let entries;
  try {
  ...
  for (const name of entries) {
    ...
    try {
      const sub = validateBusSubscriberV1(load(fs15.readFileSync(path18.join(dir, name), "utf8")));
      if (sub) out.push(sub);
    } catch {
    }
  }
...
function processFanout(runDir, now) {
  ...
  try {
    return withLock(runDir, () => {
      const cursorPath = path18.join(busDir(runDir), FANOUT_CURSOR);
      let cursor = -1;
      try {
      ...
      if (lines.length > 0) fs15.appendFileSync(path18.join(busDir(runDir), FANOUT_LOG), lines.join("\n") + "\n");
...
function appendEvent(runDir, event, opts = {}) {
  ...
  if (opts.forceFallback || process.platform === "win32") {
    ...
    const path21 = laneFallbackPath(runDir, laneId);
    (0, import_node_fs2.mkdirSync)((0, import_node_path2.dirname)(path21), { recursive: true });
    const fd = (0, import_node_fs2.openSync)(path21, "a");
    try {
...
function bundleRelPath(runId, specialist, taskId) {
  return path19.join(".guild", "context", runId, `${specialist}-${taskId}.md`);
}
function bundleAbsPath(guildRoot, runId, specialist, taskId) {
  return path19.join(guildRoot, bundleRelPath(runId, specialist, taskId));
}
function dispatchTraceAbsPath(runDir) {
  return path19.join(runDir, "dispatch-trace.md");
}
...
function appendComplianceLog(runDir, runId, specialist, taskId, result) {
  try {
    ...
    fs16.appendFileSync(
      path19.join(runDir, "context-compliance.jsonl"),
      JSON.stringify(rec) + "\n",
      "utf8"
    );
    return true;
...
function receiptPath(guildRoot, runId, specialist, taskId) {
  return path20.join(guildRoot, ".guild", "runs", runId, "handoffs", `${specialist}-${taskId}.md`);
}
function learningsPath(guildRoot, runId, specialist, taskId) {
  return path20.join(guildRoot, ".guild", "runs", runId, "learnings", `${specialist}-${taskId}.json`);
}
...
function persistRunState(runDir, runId, specialist, taskId, status, tier, dependsOn) {
  try {
    const patch = {
      status,
      receipt_ref: path20.join("handoffs", `${specialist}-${taskId}.md`)
    };
...
function runStatePathHint(runDir) {
  return path20.join(runDir, "run-state.json");
}
...
`
  ...
  if (!quarantineDone) {
    ...
    if (!canonicalRemoved) {
      try {
        const evt = buildSecurityEvent({
          ...
          detail: `CRITICAL: Cannot remove raw handoff receipt "${path20.basename(rPath)}" from canonical path \u2014 quarantine AND overwrite/unlink both failed. Raw secret may persist. Lane blocked. Manual remediation required.`,
          permission_mode: "blocked"
        });
    ...
    process.stderr.write(
      `[task-completed] WARN: HK-06: quarantine rename failed but canonical path overwritten/unlinked for ${path20.basename(rPath)}.
`
    );
  }
  try {
...
function persistInjectionAudit(runDir, taskId, specialist, injectionClean) {
  try {
    const logsDir = path20.join(runDir, "logs");
    ...
    fs17.appendFileSync(
      path20.join(logsDir, "injection-audit.jsonl"),
      JSON.stringify(record) + "\n",
      "utf8"
    );
  } catch (err) {
...
async function main3() {
  ...
  const runDir = path20.join(guildRoot, ".guild", "runs", runId);
