...
function makeCanonicalRunId(nowIso, slugSource) {
  const slug = runSlug(slugSource) || crypto3.randomUUID();
  return assertCanonicalRunId(`run-${utcCompact(nowIso)}-${slug}`);
}
var crypto3, CANONICAL_RUN_ID_RE, CANONICAL_PHASES;
...
function writeCheckpoint(opts) {
  ...
  fs18.writeFileSync(checkpointFile, yaml4, "utf8");
...
var TOPIC_TYPES2 = Object.freeze([
  "handoff",
  "status",
  "context",
  "review",
...
var BUS_EVENT_KINDS2 = Object.freeze([
  "artifact.published",
  "artifact.streaming",
  "artifact.closed",
  "artifact.retracted"
...
function busDir(runDir2) {
  return path24.join(runDir2, "bus");
}
function busLogPath(runDir2) {
  return path24.join(busDir(runDir2), "log.jsonl");
}
...
function isValidTopic(topic) {
  ...
  if (!TOPIC_TYPES2.includes(parts[0])) return false;
...
function validateBusEventV1(obj) {
  ...
  if (!BUS_EVENT_KINDS2.includes(o.event)) return null;
...
function atomicWrite2(target, data) {
  fs19.mkdirSync(path24.dirname(target), { recursive: true });
...
var BusLockTimeout = class extends Error {
  constructor(runDir2) {
    super(`artifact-bus: could not acquire ${path24.join(busDir(runDir2), ".lock")} within timeout`);
...
function withLock(runDir2, fn) {
  const lock = path24.join(busDir(runDir2), ".lock");
...
function readSubscribers(runDir2) {
  const dir = path24.join(busDir(runDir2), "subscribers");
  let entries;
  try {
  ...
  for (const name of entries) {
    ...
    try {
      const sub = validateBusSubscriberV1(decodeYamlCompatible(fs19.readFileSync(path24.join(dir, name), "utf8")));
      if (sub) out.push(sub);
    } catch {
    }
  }
...
function processFanout(runDir2, now) {
  ...
  try {
    return withLock(runDir2, () => {
      const cursorPath = path24.join(busDir(runDir2), FANOUT_CURSOR);
      let cursor = -1;
      try {
      ...
      const fresh = readBusLog2(runDir2).filter((e) => e.seq > cursor);
      let delivered = 0;
      let deferred = 0;
      let maxSeq = cursor;
      const lines = [];
      ...
      if (lines.length > 0) fs19.appendFileSync(path24.join(busDir(runDir2), FANOUT_LOG), lines.join("\n") + "\n");
...
function bundleRelPath(runId, specialist, taskId) {
  return path25.join(".guild", "context", runId, `${specialist}-${taskId}.md`);
}
function bundleAbsPath(guildRoot, runId, specialist, taskId) {
  return path25.join(guildRoot, bundleRelPath(runId, specialist, taskId));
}
function dispatchTraceAbsPath(runDir2) {
  return path25.join(runDir2, "dispatch-trace.md");
}
...
function appendComplianceLog(runDir2, runId, specialist, taskId, result) {
  try {
    ...
    fs20.appendFileSync(
      path25.join(runDir2, "context-compliance.jsonl"),
      JSON.stringify(rec) + "\n",
      "utf8"
    );
    return true;
...
function deriveRunId(sessionId, guildRoot) {
  ...
  if (fs21.existsSync(path26.join(guildRoot, ".guild", "runs", legacy))) {
    process.stderr.write(
...
`
  ...
  process.stderr.write(
    `[task-completed] WARN: read-only fallback run id used: ${fallback}; no run-record write is authorized without the lifecycle binding envelope.
`
  );
  return fallback;
}
function receiptPath(guildRoot, runId, specialist, taskId) {
  return path26.join(guildRoot, ".guild", "runs", runId, "handoffs", `${specialist}-${taskId}.md`);
}
function learningsPath(guildRoot, runId, specialist, taskId) {
  return path26.join(guildRoot, ".guild", "runs", runId, "learnings", `${specialist}-${taskId}.json`);
}
...
function persistRunState(runDir2, runId, specialist, taskId, status, tier, dependsOn) {
  try {
    const patch2 = {
      status,
      receipt_ref: path26.join("handoffs", `${specialist}-${taskId}.md`)
    };
...
function runStatePathHint(runDir2) {
  return path26.join(runDir2, "run-state.json");
}
...
`
  ...
  if (!quarantineDone) {
    ...
    if (!canonicalRemoved) {
      try {
        const evt = buildSecurityEvent({
          ...
          detail: `CRITICAL: Cannot remove raw handoff receipt "${path26.basename(rPath)}" from canonical path \u2014 quarantine AND overwrite/unlink both failed. Raw secret may persist. Lane blocked. Manual remediation required.`,
          permission_mode: "blocked"
        });
    ...
    process.stderr.write(
      `[task-completed] WARN: HK-06: quarantine rename failed but canonical path overwritten/unlinked for ${path26.basename(rPath)}.
`
    );
  }
  try {
...
function persistInjectionAudit(runDir2, taskId, specialist, injectionClean) {
  try {
    ...
    fs21.appendFileSync(
      path26.join(logsDir, "injection-audit.jsonl"),
      JSON.stringify(record) + "\n",
      "utf8"
    );
  } catch (err) {
...
async function main2() {
  ...
  const runDir2 = path26.join(guildRoot, ".guild", "runs", runId);
