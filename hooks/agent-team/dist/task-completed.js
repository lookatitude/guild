...
var fs9, path10, yaml, HOST_MODES, DEFAULTS2, VALID_TIER_HOST_KEYS, KNOWN_HOST_IDS2, VALID_LOOPS, VALID_RIGOR, VALID_REVIEW, DISPATCH_HOST_IDS, VALID_AGENT_MODE, VALID_CACHE_TTL, DEFAULTS_ALLOWED_KEYS;
...
function validateDefaults(d, selfBuild) {
  ...
  if (d["lifecycle_gate"] !== void 0 && !isPlainObject4(d["lifecycle_gate"])) {
  ...
  } else if (isPlainObject4(d["lifecycle_gate"])) {
...
function writeCheckpoint(opts) {
  ...
  fs13.writeFileSync(checkpointFile, yaml3, "utf8");
...
function readSubscribers(runDir) {
  ...
  for (const name of entries) {
    ...
    try {
      const sub = validateBusSubscriberV1(yaml2.load(fs15.readFileSync(path17.join(dir, name), "utf8")));
      if (sub) out.push(sub);
    } catch {
    }
  }
