...
function checkContained(root, target, options = {}) {
  ...
  const rel = path5.relative(realRoot, realProbe);
  if (rel !== "" && escapes(rel)) {
...
function controlCommitment(sequence) {
  let hex2 = (sequence >>> 0).toString(16);
...
function prepareEmitDir(cwd, runId, filePath, guildDir, label) {
  ...
  const rel = path19.relative(path19.join(prepared.realRoot, guildDir, "runs"), prepared.realDir);
  if (rel === "" || rel.startsWith("..") || path19.isAbsolute(rel) || rel.split(path19.sep)[0] !== runId) {
    throw new Error(`${label}: refusing to write outside the run tree (symlinked path)`);
  }
  try {
    if (fs16.lstatSync(filePath).isSymbolicLink()) {
...
function writeRunArtifact(cwd, runId, kind, filename, content, opts = {}) {
  assertSafeRunId2(runId);
  ...
  const runDir2 = path19.join(cwd, guildDir, "runs", runId);
  const p = path19.join(runDir2, kind, filename);
  if (!isWithin(path19.resolve(p), path19.resolve(runDir2))) {
  ...
  if (opts.immutable && fs16.existsSync(p)) {
    throw new Error(
      `writeRunArtifact: ${kind}/${filename} already exists \u2014 artifacts are immutable (\xA71); write a NEW version instead of editing`
    );
  }
  fs16.writeFileSync(p, content, "utf8");
  return p;
}
var fs16, path19, STATION_SIGNAL_KEYS, SIGNAL_KEY_SET, TEAM_PLAN_DIR, TEAM_RESULT_DIR, SAFE_SEGMENT;
var init_station_signals = __esm({
  "src/modules/teams/workflows/station-signals.ts"() {
    ...
    TEAM_PLAN_DIR = "team-plan";
    TEAM_RESULT_DIR = "team-result";
...
var fs18, path21, WAVE_STATUSES, PROGRAM_STATUSES;
...
var team_decide_exports = {};
__export(team_decide_exports, {
  main: () => main
});
...
var isNonEmptyStr = (v) => typeof v === "string" && v.trim().length > 0;
...
function validateProposal(obj) {
  ...
  for (const refKey of ["spec_ref", "plan_ref"]) {
    ...
    if (ref === null || typeof ref !== "object" || Array.isArray(ref) || !isNonEmptyStr(ref["path"]) || !isSha256Hex(ref["sha256"]) || Object.keys(ref).some((k) => k !== "path" && k !== "sha256")) {
      reasons.push(`${refKey} must be {path: non-empty string, sha256: sha256 hex} (closed shape)`);
    }
  }
  if (o["provenance"] !== void 0 && !isNonEmptyStr(o["provenance"])) {
  ...
  if (!Array.isArray(o["obligations"])) {
    ...
    for (const [i, raw] of o["obligations"].entries()) {
      ...
      if (!ownedByPresent) {
        ...
        } else if (exclusion === null || typeof exclusion !== "object" || !isNonEmptyStr(exclusion["rationale"]) || typeof exclusion["user_accepted"] !== "boolean") {
          reasons.push(
  ...
  if (!Array.isArray(o["excluded_roles"])) {
    ...
    for (const [i, raw] of o["excluded_roles"].entries()) {
      if (raw === null || typeof raw !== "object" || !isNonEmptyStr(raw["role_ref"]) || !isNonEmptyStr(raw["why_unnecessary"])) {
  ...
  if (o["backend_capacity_evidence"] !== void 0) {
    ...
    if (e === null || typeof e !== "object" || !isNonEmptyStr(e["backend"]) || typeof e["verified_capacity"] !== "number" || !isNonEmptyStr(e["method"]) || !isNonEmptyStr(e["as_of"])) {
...
function writeProposal(cwd, proposal, opts = {}) {
...
function validateDecisionActor(actor) {
  const reasons = [];
  if (actor === null || typeof actor !== "object" || Array.isArray(actor)) {
    return {
      valid: false,
      reasons: [
        'decided_by must be a typed actor object {kind: "user" | "operator", id?} \u2014 bare identity strings are rejected (\xA74 user-actor allowlist)'
      ]
    };
  }
  const a = actor;
  for (const key of Object.keys(a)) {
    if (key !== "kind" && key !== "id") {
      reasons.push(`unknown actor key "${key}" \u2014 the \xA74 actor shape is closed`);
    }
  }
  if (typeof a["kind"] !== "string" || !ACTOR_KIND_SET.has(a["kind"])) {
    reasons.push(
      `actor kind must be one of {${DECISION_ACTOR_KINDS.join(", ")}} \u2014 any other kind (including every agent-side identity) is rejected`
    );
  }
  if (a["id"] !== void 0 && (typeof a["id"] !== "string" || !ACTOR_ID_RE.test(a["id"]))) {
    reasons.push("actor id, when present, must match /^[a-z0-9][a-z0-9._-]{0,63}$/");
  }
  return { valid: reasons.length === 0, reasons };
}
function formatDecidedBy(actor, channel) {
  return `${actor.kind}${actor.id !== void 0 ? `:${actor.id}` : ""}@${channel}`;
}
...
function recordDecision(proposal, input) {
  ...
  if (!actorVerdict.valid) {
    throw new Error(
      `recordDecision: decided_by ${JSON.stringify(input.decided_by)} is not an allowed user/operator actor \u2014 ${actorVerdict.reasons.join("; ")} (\xA74 user-actor allowlist: approval is a USER act; an agent identity can never decide)`
    );
  }
  if (typeof input.decision_channel !== "string" || !CHANNEL_SET.has(input.decision_channel)) {
    throw new Error(
      `recordDecision: decision_channel is required and must be one of {${DECISION_CHANNELS.join(", ")}} \u2014 a decision without user-confirmation provenance is never recorded (\xA74 user-actor allowlist)`
    );
  }
  ...
  if (input.decided_at !== void 0 && !RFC3339_RE.test(input.decided_at)) {
  ...
  const decision = {
    ...
    decided_at: input.decided_at ?? (/* @__PURE__ */ new Date()).toISOString(),
    ...input.decision === "restructure" ? { restructure_edits: cloneArtifact(input.restructure_edits ?? []) } : {},
...
function applyRestructure(proposal, input) {
  ...
  for (const edit of edits) {
    if ("add" in edit) {
      ...
      next.participants = next.participants.map(
        (p) => p.participant_id === edit.substitute.participant_id ? cloneArtifact(edit.substitute.participant) : p
      );
    } else if ("edit_dependencies" in edit) {
      next.participants = next.participants.map(
        (p) => p.participant_id === edit.edit_dependencies.participant_id ? { ...p, depends_on: [...edit.edit_dependencies.depends_on] } : p
      );
    } else {
  ...
  if (next.proposed_schedule) {
    const seen = /* @__PURE__ */ new Set();
    next.proposed_schedule.waves = next.proposed_schedule.waves.map((w) => w.filter((id) => nextIds.has(id) && !seen.has(id) && (seen.add(id), true))).filter((w) => w.length > 0);
  ...
  const restamped = { ...restructure_decision };
...
function writeDecision(cwd, decision, opts = {}) {
...
init_canonical_hash();
init_station_signals();
var TEAM_SCHEDULE_SCHEMA = "guild.team_schedule.v1";
function validateScheduleSets(proposal, schedule, decision) {
  const errors = [];
  const p = proposal;
  const proposalVerdict = validateProposal(proposal);
  if (!proposalVerdict.valid) {
  ...
  if (!isSha256Hex(schedule.proposal_hash) || schedule.proposal_hash !== recomputedProposalHash) {
    return {
      valid: false,
      reason: "schedule.proposal_hash does not equal the \xA71 hash recomputed from the proposal \u2014 the schedule is not bound to this approved proposal; fails closed"
    };
  }
  const decisionVerdict = validateDecision(decision);
  if (!decisionVerdict.valid) {
  ...
  if (!isSha256Hex(schedule.decision_hash) || schedule.decision_hash !== decision.decision_hash) {
    return {
      valid: false,
      reason: "schedule.decision_hash does not equal the verified approve decision's decision_hash \u2014 the schedule is not approval-bound; fails closed"
    };
  }
  ...
  if ([...new Set(waveUnion)].sort().join() !== expectedKey) {
    errors.push(
      "base-wave union != approved participant set (an unapproved or omitted id breaks \xA75 set equality)"
    );
  }
  const receiptKey = [...new Set(schedule.dispatch_receipts.map((r) => r.participant_id))].sort().join();
  if (receiptKey !== expectedKey) {
    errors.push(
      "dispatch-receipt id set != approved participant set (an unapproved or omitted id breaks \xA75 set equality)"
    );
  }
...
function realTrailFs() {
  return {
    readdir: (dir) => fs20.existsSync(dir) ? fs20.readdirSync(dir).sort() : [],
    readFile: (p) => fs20.existsSync(p) ? fs20.readFileSync(p, "utf8") : null
  };
}
function teamPlanDir(cwd, runId, guildDir = ".guild") {
  return path24.join(cwd, guildDir, "runs", runId, "team-plan");
}
...
function preDispatchGate(proposal, decisions) {
  ...
  const refusal2 = sawMismatch ? "stale_or_hash_mismatch" : sawNonApprove ? "not_an_approve" : "no_persisted_decision";
  return {
    allowed: false,
    refusal: refusal2,
...
function scanCapReintroduction(value, trail = [], out = []) {
  if (Array.isArray(value)) {
...
function planRestructure(proposal, input) {
  const edits = input.restructure_edits ?? [];
  ...
  edits.forEach((e, i) => {
    ...
    } else if (edit["edit_dependencies"] && !parentIds.has(String(edit["edit_dependencies"].participant_id))) {
      no_op_edits.push(
        "edits[" + i + "] edit_dependencies " + edit["edit_dependencies"].participant_id + ": not in the roster"
      );
    }
  });
  if (edits.length === 0) {
  ...
  if (contentEquivalent(parent, result.new_proposal)) {
    return convergedPlan(
      parent,
      no_op_edits.length > 0 ? no_op_edits : ["all edits were already satisfied by the parent proposal"]
    );
  }
  const nextIds = idsOf(result.new_proposal);
  const cited = citedIds(edits);
  const uncited_roster_delta = [];
  ...
  return {
    ...
    cap_reintroduction,
    valid: uncited_roster_delta.length === 0 && result.coverage_reasons.length === 0 && cap_reintroduction.length === 0,
    reasons: result.coverage_reasons
  };
}
var ITERATION_FIELDS = ["proposal_version", "parent_proposal_hash", "proposal_hash"];
function contentEquivalent(a, b) {
  const strip = (p) => {
    const copy = JSON.parse(JSON.stringify(p));
    for (const f of ITERATION_FIELDS) delete copy[f];
    return canonicalYaml(copy);
  };
  return strip(a) === strip(b);
}
function convergedPlan(parent, no_op_edits) {
...
function scopeLabel(scope) {
  return scope === null || scope === void 0 ? "unscoped" : scope.join(",") || "unscoped";
}
function buildProposalReview(input) {
  ...
  if (input.previous) {
    ...
    const changed = participants.filter((x) => {
      const before = prevById.get(x.participant_id);
      return before !== void 0 && JSON.stringify(before) !== JSON.stringify(x);
    }).map((x) => x.participant_id).sort();
    edits = {
  ...
  const state = gate?.allowed ? "approved" : input.decision?.decision === "restructure" ? "restructured" : gate === null ? "pending" : "refused";
  let schedule = null;
  if (input.schedule) {
    ...
    if (input.decision) {
      ...
      invariants = verdict.valid ? "approved set = wave union = receipt ids = terminal ids (set equality holds)" : "VIOLATION - " + (verdict.reason ?? "unspecified");
    }
    schedule = {
      backend: s.backend,
...
function renderProposalReview(view) {
  const L = [];
  L.push("guild team decision - run " + view.run_id + " | phase " + view.phase);
  L.push(
    "proposal v" + view.proposal.version + " (" + view.proposal.hash_short + ")" + (view.proposal.parent_hash_short ? " | parent " + view.proposal.parent_hash_short : "") + " | " + view.proposal.participant_count + " participants (UNCAPPED - task-derived)"
  );
  L.push("");
  ...
  L.push("");
  L.push(
    "COVERAGE  obligations=" + view.coverage.obligations + " owned=" + view.coverage.owned + " excluded-with-user-acceptance=" + view.coverage.excluded_with_acceptance + " unowned=" + view.coverage.unowned.length
  );
  ...
  if (view.edits) {
    ...
    L.push(
      "  COVERAGE IMPACT: " + (view.edits.obligations_lost.length === 0 ? "no obligation lost its owner" : "obligations left unowned - " + view.edits.obligations_lost.join(", "))
    );
  }
  L.push("");
  ...
  if (view.schedule) {
    L.push("");
    L.push(
      "BACKEND SCHEDULE  backend=" + view.schedule.backend + " verified_capacity=" + view.schedule.verified_backend_capacity + " hint=" + (view.schedule.selected_hint === null ? "none" : view.schedule.selected_hint) + " effective=" + view.schedule.effective
    );
...
function renderRestructurePlan(plan) {
  const L = [];
  if (plan.result === null) {
    L.push(
      "restructure CONVERGED - proposal v" + (plan.proposal.proposal_version ?? 1) + " (" + shortHash(plan.proposal.proposal_hash) + ") is UNCHANGED; no new version, hash or decision was minted"
    );
  ...
  L.push(
    "restructure -> proposal v" + np.proposal_version + " (" + shortHash(np.proposal_hash) + ") parent " + shortHash(np.parent_proposal_hash)
  );
  ...
  L.push(
    "  coverage impact: " + (plan.result.lost_obligations.length === 0 ? "no obligation lost its owner" : "LOST OWNERS - " + plan.result.lost_obligations.join(", "))
  );
...
function parseFlags(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (argv[i + 1] !== void 0 && !argv[i + 1].startsWith("--")) out[key] = argv[++i];
    else out[key] = true;
  }
  return out;
}
function readArtifact(p) {
  ...
  const abs = path25.resolve(p);
  const raw = fs21.readFileSync(abs, "utf8");
...
function main(argv = process.argv.slice(2)) {
  ...
  try {
    if (verb === "review") {
      ...
      const previous = f["previous"] ? readArtifact(f["previous"]) : null;
      const schedule = f["schedule"] ? readArtifact(f["schedule"]) : null;
      const decision = f["decision"] ? readArtifact(f["decision"]) : null;
    ...
    if (verb === "restructure") {
      ...
      if (typeof decidedBy !== "string" || typeof channel !== "string") {
        process.stderr.write(
          "--decided-by and --channel are required: a restructure is a USER act and is recorded with its confirmation provenance\n"
        );
        return 1;
      }
      const [actorKind, actorId] = decidedBy.split(":");
      const actor = actorId === void 0 ? { kind: actorKind } : { kind: actorKind, id: actorId };
      ...
      process.stdout.write(
        json ? JSON.stringify(
          ...
          null,
          2
        ) + "\n" : renderRestructurePlan(plan) + renderProposalReview(
          buildProposalReview({
            proposal: plan.proposal,
    ...
    if (verb === "gate") {
      const proposal = readArtifact(f["proposal"]);
      const cwd = typeof f["cwd"] === "string" ? path25.resolve(f["cwd"]) : process.cwd();
      ...
      process.stdout.write(
        json ? JSON.stringify({ verdict, view }, null, 2) + "\n" : renderProposalReview(view) + "\nTRAIL (" + decisions.length + " persisted decision artifact(s) considered)\n" + verdict.considered.map((c) => "  " + c.source_path + ": " + c.verdict).join("\n") + "\n"
      );
      if (!verdict.allowed) {
        process.stderr.write(
    ...
    if (verb === "record") {
      ...
      const decidedBy = f["decided_by"];
      const channel = f["channel"];
      if (typeof decision !== "string" || typeof decidedBy !== "string" || typeof channel !== "string") {
        process.stderr.write(
          "--decision, --decided-by and --channel are required: a team decision is a USER act and is recorded with its confirmation provenance\n"
        );
        return 1;
      }
      const [actorKind, actorId] = decidedBy.split(":");
      ...
      process.stdout.write(
        json ? JSON.stringify({ decision_path: written, decision: built }, null, 2) + "\n" : "recorded " + built.decision + " by " + built.decided_by + "\npersisted decision: " + written + "\n"
      );
      return 0;
    }
