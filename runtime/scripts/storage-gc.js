...
function assertNotUnderPluginInstall(absPath, pluginInstallRoot) {
  ...
  if (rel === "" || !rel.startsWith("..") && !path3.isAbsolute(rel)) {
    ...
    if (underOwnDotGuild && fs3.existsSync(path3.join(resolvedRoot, ".git"))) return;
...
function countModuleResources(root, moduleId) {
  ...
  if (!fs5.existsSync(manifestAbs)) return 0;
  try {
    const parsed = JSON.parse(fs5.readFileSync(manifestAbs, "utf8"));
...
function validateModuleHealth(root, manifests) {
  ...
  for (const manifest of manifests) {
    ...
    if (!fs5.existsSync(resourcesManifest) || !fs5.statSync(resourcesManifest).isFile()) {
      findings.push({
        module_id: manifest.id,
    ...
    if (!fs5.existsSync(resourcesMarker) || !fs5.statSync(resourcesMarker).isFile()) {
      findings.push({
        module_id: manifest.id,
...
function readWorkspaceMode(root) {
  const raw = readJson(path7.join(root, ".guild", "settings.json"));
...
function readWorkspaceManifest(root) {
  const raw = readJson(path7.join(root, ".guild", "workspace.json"));
...
function immediateMarkedChildren(root) {
  ...
  for (const ent of entries) {
    ...
    const childRoot = path7.join(root, ent.name);
    const hasGit = fs6.existsSync(path7.join(childRoot, ".git"));
    const hasGuild = fs6.existsSync(path7.join(childRoot, ".guild"));
    if (!hasGit && !hasGuild) continue;
    children.push({
      name: ent.name,
      path: ent.name,
      kind: hasGuild ? "sub-guild" : "sub-project",
      has_wiki: fs6.existsSync(path7.join(childRoot, ".guild", "wiki")),
      has_indexes: fs6.existsSync(path7.join(childRoot, ".guild", "indexes"))
    });
  }
  return children;
}
...
function findParentWorkspace(activeRoot) {
  const parent = path7.dirname(activeRoot);
  if (parent === activeRoot) return null;
  const activeResolved = path7.resolve(activeRoot);
  for (const entry of workspaceEntries(parent)) {
    const childRoot = path7.resolve(parent, entry.path);
...
function canonicalMemory(activeRoot) {
  return {
    wiki: path7.join(activeRoot, ".guild", "wiki"),
    raw: path7.join(activeRoot, ".guild", "raw"),
    indexes: path7.join(activeRoot, ".guild", "indexes"),
    indexSqlite: path7.join(activeRoot, ".guild", "index.sqlite")
  };
}
function discoverGuild(startCwd) {
  ...
  return {
    startCwd: path7.resolve(startCwd),
    activeRoot,
    guildDir: path7.join(activeRoot, ".guild"),
    level,
...
function batteryWorkspace() {
  ...
  const base = fs16.mkdtempSync(path20.join(os.tmpdir(), "a21x-integration-battery-"));
...
function isUnderDurable(abs, guildDir) {
  const rel = path25.relative(path25.resolve(guildDir), path25.resolve(abs));
...
function resolveContainedRealDir(abs, root) {
  ...
  const rel = path26.relative(realRoot, real);
  if (rel === "" || rel.startsWith("..") || path26.isAbsolute(rel)) return null;
  return real;
}
...
function createGuildStorage(cwd = process.cwd(), opts = {}) {
  ...
  const project = profile === "workspace-only" ? void 0 : scopedPaths(guildDir, "project", "settings.json");
  const workspace = profile === "workspace-only" || profile === "hybrid" ? scopedPaths(guildDir, "workspace", "settings.json") : void 0;
  ...
  const storage = {
    ...
    ensureDir(absPath) {
      fs21.mkdirSync(absPath, { recursive: true });
      return absPath;
    },
    closeRun(runId) {
      assertSafeSegments([runId]);
...
var storage_gc_exports = {};
__export(storage_gc_exports, {
  formatGcReport: () => formatGcReport,
  runStorageGc: () => runStorageGc
});
module.exports = __toCommonJS(storage_gc_exports);
...
if (require.main === module) {
  ...
  const ttlHours = ttlRaw === void 0 ? void 0 : Number(ttlRaw);
  if (ttlHours !== void 0 && (!Number.isFinite(ttlHours) || ttlHours < 0)) {
