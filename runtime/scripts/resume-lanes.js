...
function moduleIdOfPath(root, target, moduleIds) {
  const rel2 = relativeTo(path7.join(root, "src", "modules"), target);
  if (rel2.indexOf("..") === 0 || path7.isAbsolute(rel2)) return null;
...
function classifyDestination(root, importer, specifier, fromModule, moduleIds) {
  ...
  if (rel2.indexOf("..") === 0 || path7.isAbsolute(rel2)) {
    return { destination_class: "unclassified", imported: rel2, to_module: null };
  }
  const head = rel2.split("/")[0];
  if (MH07_HOST_FACING_ROOTS.indexOf(head) !== -1) {
    return { destination_class: "host_facing_mirror", imported: rel2, to_module: head };
  }
...
function mh07ScanDependencyGraph(root, extract = extractNeutralImportEdges, moduleIdsOverride) {
  ...
  const ambient = [];
  for (const file of files) {
    const rel2 = relativeTo(root, file);
    nodes.push(rel2);
    ...
    for (const edge of extracted) {
      if (edge.kind === "unresolved") {
        ...
        if (edge.form === "require" && detail.indexOf("require used as a value") === 0) {
          ambient.push({ importer: rel2, detail });
        } else {
          unresolved.push({ importer: rel2, form: edge.form, detail });
        }
        continue;
      }
      const specifier = edge.specifier ?? "";
...
function canonicalJournalPath(journalPath2) {
  ...
  for (let hop = 0; hop < CANONICAL_PATH_MAX_LINK_HOPS; hop += 1) {
    ...
    return path12.join(canonicalJournalPath(parent), path12.basename(current));
  }
  return current;
}
...
function acquireJournalAuthority(journalPath2, io = defaultJournalIo, lockOptions = {}, access = "read", checkpointPath = null) {
  ...
  const journalParent = path12.dirname(identity.path);
  ...
  const checkpointParentPath = checkpointPath === null ? null : canonicalJournalPath(path12.dirname(checkpointPath));
  const checkpointCanonical = checkpointParentPath === null || checkpointPath === null ? null : path12.join(checkpointParentPath, path12.basename(checkpointPath));
  ...
  const verify = (stage) => {
    ...
    if (parentInode !== null && parentDevice !== null) {
      const parentNow = statOrNull(path12.dirname(identity.path));
    ...
    if (checkpointPath !== null) {
      ...
      const parentNamedNow = canonicalJournalPath(path12.dirname(checkpointPath));
  ...
  "src/modules/telemetry/workflows/receipt-journal.ts"() {
    ...
    defaultJournalIo = {
      appendLine(journalPath2, text, bound) {
        ...
        fs9.mkdirSync(path12.dirname(journalPath2), { recursive: true });
      ...
      acquireLock(lockPath2) {
        fs9.mkdirSync(path12.dirname(lockPath2), { recursive: true });
        try {
...
function makeProbePaths(parent, name) {
  const dir = path13.join(parent, name);
  fs10.mkdirSync(dir, { recursive: true });
  return { dir, journal: path13.join(dir, JOURNAL_LEAF), checkpoint: path13.join(dir, CHECKPOINT_LEAF) };
}
...
function evaluateReceiptJournalConformance(request) {
  ...
  const usableRoot = typeof root === "string" && root.length > 0 && path13.isAbsolute(root) && isExistingDirectory(root);
  if (!usableRoot) {
    return refuseEvaluation2(MH06_REFUSAL_CONTROLS.journalRootUnusable, "scenario_evidence_incomplete", [
  ...
  const identity = request.evidence_identity;
  const workspace = fs10.mkdtempSync(path13.join(root, PROBE_WORKSPACE_PREFIX));
  const verdicts = {};
  try {
    for (const stableId of MH06_SCENARIO_IDS) {
...
var fs10, path13, OUTCOME_ENVELOPE_SCHEMA, OUTCOME_CONTRACT_VERSION, MH06_SUITE_ID, MH06_SUITE_VERSION, MH06_OWNER_KEY, MH06_PACKET_SCHEMA, MH06_SCENARIO_IDS, MH06_CATEGORY, MH06_EVIDENCE_PROFILE, MH06_EXPECTED_OUTCOMES, SCENARIO_RESULT_MISMATCH, EVIDENCE_FRESHNESS_VERDICTS, EVIDENCE_IDENTITY_FIELDS, MH06_WAVE_OWNER, MH06_SCENARIOS, MH06_PRODUCTION_JOURNAL, MH06_REFUSAL_CONTROLS, MH06_SOURCE_VERSION, PROBE_RECORDED_AT, PROBE_RECONCILED_AT, PROBE_WORKSPACE_PREFIX, TORN_APPEND_CUT_BYTES, PROBE_DIRS, JOURNAL_LEAF, CHECKPOINT_LEAF, CHECKPOINT_SCHEMA, PROBES;
...
`;
  ...
  const coreWorkflows = {};
  for (const member of NEUTRAL_CORE_MEMBERS) {
    coreWorkflows[member] = fs13.readFileSync(path17.join(coreDir, member), "utf8");
  }
...
function readRunManifest(cwd, slug) {
  try {
    const raw = fs14.readFileSync(manifestPathFor(cwd, slug), "utf8");
...
function runWriteRunManifestCli(argv = process.argv.slice(2)) {
  ...
  if (!fs14.existsSync(args.cwd) || !fs14.statSync(args.cwd).isDirectory()) {
...
var fs14, path18, WAVE_STATUSES, PROGRAM_STATUSES;
...
function nearestProjectRoot(startDir) {
  ...
  let nearestGuildDir = null;
  for (; ; ) {
    if (fs15.existsSync(path20.join(current, ".git"))) return current;
    if (nearestGuildDir === null) {
...
var path21, NON_DURABLE_CLASSES, DURABLE_CLASSES, KTD16_FROZEN_PREFIXES, DURABLE_SUBTREES;
...
function resolveContainedRealDir(abs, root) {
  ...
  let realRoot;
  try {
    real = fs16.realpathSync(abs);
    realRoot = fs16.realpathSync(root);
  } catch {
    return null;
  }
  const rel2 = path22.relative(realRoot, real);
...
function removeContainedTree(abs, root) {
  ...
  if (!real) return null;
  fs16.rmSync(real, { recursive: true, force: true });
  return real;
}
var fs16, path22;
...
function classifyPath(relToGuild) {
  const rel2 = relToGuild.split(path23.sep).join("/");
  return DERIVED_PATTERNS.some((re) => re.test(rel2)) ? "derived" : "durable";
}
function rel(ctx, abs) {
  return path23.relative(ctx.root, abs).split(path23.sep).join("/");
}
...
function writeFile(ctx, abs, text) {
  ...
  fs17.mkdirSync(path23.dirname(abs), { recursive: true });
...
function filesUnder(dir, out = []) {
  for (const name of readdirSafe(dir)) {
    const abs = path23.join(dir, name);
    const st = lstatSafe(abs);
    if (!st) continue;
...
var init_upgrade_steps = __esm({
  "src/modules/state/workflows/upgrade-steps.ts"() {
    ...
    settingsPolicySplit = {
      ...
      apply(ctx) {
        const legacy = readIfFile(path23.join(ctx.guildDir, "settings.json"));
...
`);
    ...
    ktd22Strip = {
      ...
      apply(ctx) {
        ...
        const cfg = ctx.policy;
        const roots = ["initiatives", "team", "teams"].map((d) => path23.join(ctx.guildDir, d));
        const touched = [];
        let removedTotal = 0;
    ...
    cachesOut = {
      ...
      apply(ctx) {
        ...
        for (const name of CACHE_TARGETS) {
          const abs = path23.join(ctx.guildDir, name);
          const st = lstatSafe(abs);
          if (!st) continue;
    ...
    TERMINAL_STATUSES2 = Object.freeze([
      "closed",
      "complete",
      "completed",
      "done",
    ...
    closedRunReceipts = {
      ...
      affects: [".guild/runs"],
      apply(ctx) {
        const runsDir = path23.join(ctx.guildDir, "runs");
        ...
        for (const name of readdirSafe(runsDir)) {
          ...
          const runDir2 = path23.join(runsDir, name);
          if (!isContainedRealDir(runDir2, ctx.guildDir)) continue;
          const receipt = path23.join(runDir2, "receipt.json");
          if (lstatSafe(receipt)) continue;
          const runYaml = readIfFile(path23.join(runDir2, "run.yaml"));
...
`
    ...
    currentRunIdRetire = {
      ...
      affects: [],
      apply(ctx) {
        const abs = path23.join(ctx.guildDir, "current-run-id");
        const text = readIfFile(abs);
    ...
    skillVersionsDelete = {
      ...
      affects: [],
      apply(ctx) {
        const dir = path23.join(ctx.guildDir, LEGACY_VERSION_TREE);
        ...
        const skillsDir = path23.join(ctx.guildDir, "skills");
        if (isContainedRealDir(skillsDir, ctx.guildDir)) {
          for (const abs of filesUnder(skillsDir)) {
            const text = readIfFile(abs);
            if (text !== null) live.add(sha2563(text));
          }
        }
        const unique = snapshots.filter((abs) => {
          const text = readIfFile(abs);
          return text === null || !live.has(sha2563(text));
        });
        if (unique.length > 0) {
          return {
            status: "blocked_confirm",
    ...
    registryYamlRetire = {
      ...
      apply(ctx) {
        ...
        for (const relPath of DERIVED_REGISTRIES) {
          const abs = path23.join(ctx.guildDir, relPath);
        ...
        for (const relPath of AUTHORED_REGISTRIES) {
          ...
          const text = readIfFile(abs);
          if (text === null) continue;
          const target = path23.join(ctx.guildDir, "artifacts", "legacy", relPath.replace("/", "-"));
...
function runStatePath(runDir2) {
  return path24.join(runDir2, "run-state.json");
}
function loadRunState(runDir2) {
  let raw;
  try {
    raw = fs18.readFileSync(runStatePath(runDir2), "utf8");
  } catch {
    return null;
  }
  let parsed;
...
function writeRunStateAtomic(runDir2, state) {
  fs18.mkdirSync(runDir2, { recursive: true });
  const finalPath = runStatePath(runDir2);
  const tmpPath = `${finalPath}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  fs18.writeFileSync(tmpPath, JSON.stringify(state, null, 2) + "\n", "utf8");
  try {
    fs18.renameSync(tmpPath, finalPath);
  } catch (err) {
    try {
...
function laneResumeCheckpointPath(runDir2, laneId2) {
  return path24.join(runDir2, "lanes", laneId2, "resume.json");
}
...
function loadLaneResumeCheckpoint(runDir2, laneId2) {
  try {
    const raw = fs18.readFileSync(laneResumeCheckpointPath(runDir2, laneId2), "utf8");
    const parsed = JSON.parse(raw);
...
function markLaneDead(runDir2, init2, laneId2, signal, cwd) {
  ...
  if (readResumeEnabled(cwd)) {
    ...
    fs18.mkdirSync(path24.dirname(checkpointPath), { recursive: true });
    fs18.writeFileSync(checkpointPath, JSON.stringify(checkpoint, null, 2) + "\n", "utf8");
  }
  return state;
}
var fs18, path24, RUN_STATE_SCHEMA_VERSION, LANE_RESUME_SCHEMA_VERSION;
...
function scanResumableLanes(runDir2, cwd, slug) {
  ...
  const lanesDir = path25.join(runDir2, "lanes");
  let entries;
  try {
    entries = fs19.readdirSync(lanesDir, { withFileTypes: true });
  } catch {
    return [];
  }
