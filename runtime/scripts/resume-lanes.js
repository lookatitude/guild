...
function initStableLockfile(runDir2) {
  const path22 = stableLockPath(runDir2);
  (0, import_node_fs.mkdirSync)((0, import_node_path.dirname)(path22), { recursive: true });
  if ((0, import_node_fs.existsSync)(path22)) return;
  try {
    const fd = (0, import_node_fs.openSync)(path22, "wx");
...
function countWorkflowFiles(moduleDir) {
  const workflowsDir = path3.join(moduleDir, "workflows");
  return walkTsFiles(workflowsDir).length;
}
function countModuleResources(root, moduleId) {
  const manifestAbs = path3.join(root, "src", "modules", moduleId, "resources", "module-resources.json");
  if (!fs3.existsSync(manifestAbs)) return 0;
  try {
    const parsed = JSON.parse(fs3.readFileSync(manifestAbs, "utf8"));
...
function validateModuleHealth(root, manifests) {
  ...
  for (const manifest of manifests) {
    ...
    const resourcesManifest = path3.join(moduleDir, "resources", "module-resources.json");
    const resourcesMarker = path3.join(moduleDir, "resources", ".generated-by-guild-module-resources");
    ...
    if (!fs3.existsSync(resourcesManifest) || !fs3.statSync(resourcesManifest).isFile()) {
      findings.push({
        module_id: manifest.id,
    ...
    if (!fs3.existsSync(resourcesMarker) || !fs3.statSync(resourcesMarker).isFile()) {
      findings.push({
        module_id: manifest.id,
...
function moduleIdForPath(modulesDir, filePath, moduleIds) {
  const rel = path3.relative(modulesDir, filePath);
  if (rel.startsWith("..") || path3.isAbsolute(rel)) return null;
  const [moduleId] = rel.split(path3.sep);
...
function mh07RequiredConsumerRoots(pluginRoot) {
  ...
  const roots = {};
  for (const name of MH07_REQUIRED_CONSUMERS) roots[name] = path7.join(workspace, name);
  return roots;
}
...
function resolveRelative(importer, specifier) {
  const base = path7.resolve(path7.dirname(importer), specifier);
  const candidates = [base, `${base}.ts`, path7.join(base, "index.ts")];
  return candidates.find((candidate) => fs6.existsSync(candidate) && fs6.statSync(candidate).isFile()) ?? base;
}
function moduleIdOfPath(root, target, moduleIds) {
  const rel = relativeTo(path7.join(root, "src", "modules"), target);
  if (rel.indexOf("..") === 0 || path7.isAbsolute(rel)) return null;
...
function classifyDestination(root, importer, specifier, fromModule, moduleIds) {
  ...
  if (rel.indexOf("..") === 0 || path7.isAbsolute(rel)) {
  ...
  const publicIndex = path7.join(root, "src", "modules", toModule, "index.ts");
  const isPublic = path7.resolve(resolved) === path7.resolve(publicIndex);
  return {
...
function mh07ScanDependencyGraph(root, extract = extractNeutralImportEdges, moduleIdsOverride) {
  const modulesDir = path7.join(root, "src", "modules");
  const moduleIds = moduleIdsOverride ?? (fs6.existsSync(modulesDir) ? fs6.readdirSync(modulesDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name).sort() : []);
  ...
  for (const file of files) {
    ...
    const extracted = extract(fs6.readFileSync(file, "utf8"));
...
function consumerSourceReaches(consumerRoot, pluginRoot, extract) {
  const found = [];
  const pluginSource = path7.resolve(path7.join(pluginRoot, "src"));
  for (const file of walkTypeScript(consumerRoot, MH07_CONSUMER_SCAN_EXCLUDED_DIRECTORIES)) {
    for (const edge of extract(fs6.readFileSync(file, "utf8"))) {
      if (edge.kind !== "resolved" || edge.specifier === void 0) continue;
      if (edge.specifier.indexOf(".") !== 0) continue;
      const resolved = path7.resolve(resolveRelative(file, edge.specifier));
      if (resolved === pluginSource || resolved.indexOf(`${pluginSource}${path7.sep}`) === 0) {
        found.push({
...
function evaluateNeutralModuleBoundaries(request) {
  ...
  for (const name of consumerNames) {
    ...
    if (typeof consumerRoot !== "string" || !fs6.existsSync(consumerRoot)) {
      return refuse3(
        MH07_REFUSAL_CONTROLS.evidenceBindingMissing,
        "scenario_evidence_incomplete",
  ...
  for (const name of MH07_REQUIRED_CONSUMERS) {
    ...
    const bound = typeof suppliedRoot === "string" && path7.resolve(suppliedRoot) === path7.resolve(requiredRoot);
    if (!bound) {
      return refuse3(
        MH07_REFUSAL_CONTROLS.evidenceBindingMissing,
        "scenario_evidence_incomplete",
    ...
    try {
      position = fs6.lstatSync(suppliedRoot);
    } catch (error) {
  ...
  const sourcesOf = (moduleId) => walkTypeScript(
    path7.join(root, "src", "modules", moduleId),
    MH07_GRAPH_SCOPE.excluded_directories
  ).map((file) => ({ path: relativeTo(root, file), source: fs6.readFileSync(file, "utf8") }));
  ...
  for (const member of NEUTRAL_CORE_MEMBERS) {
    const memberPath = coreHostModule === null ? null : path7.join(root, "src", "modules", coreHostModule, "workflows", member);
    if (memberPath === null || !fs6.existsSync(memberPath)) {
      missingCoreMembers.push(member);
      continue;
    }
    coreFiles.push({ path: member, source: fs6.readFileSync(memberPath, "utf8") });
  }
  ...
  const contractConsumersWithoutVersion = contractConsumerServices.filter((moduleId) => {
    const indexPath = path7.join(root, "src", "modules", moduleId, "index.ts");
    if (!fs6.existsSync(indexPath)) return true;
    return fs6.readFileSync(indexPath, "utf8").indexOf(MODULE_PUBLIC_API_DECLARATION) === -1;
  });
...
function issue(path22, code, message) {
  return { path: path22, code, message: `${DOCUMENTS_ERROR_NAMESPACE}: ${message}` };
}
function pushIssue(issues, path22, code, message) {
  if (issues.length >= MAX_ISSUES) return;
  issues.push(issue(path22, code, message));
}
...
function canonicalDocumentJson(value) {
  ...
  let nodes = 0;
  const walk = (node, path22, depth) => {
    if (errors.length >= MAX_ISSUES) return null;
    if (depth > MAX_CANONICAL_DEPTH) {
      pushIssue(errors, path22, "depth_exceeded", `value nests deeper than ${MAX_CANONICAL_DEPTH}`);
      return null;
    }
    nodes += 1;
    if (nodes > MAX_CANONICAL_NODES) {
      pushIssue(errors, path22, "size_exceeded", `value exceeds ${MAX_CANONICAL_NODES} nodes`);
      return null;
    }
    ...
    if (kind === "string") {
      ...
      if (text.length > MAX_STRING_LENGTH) {
        pushIssue(errors, path22, "string_too_long", `string exceeds ${MAX_STRING_LENGTH} characters`);
        return null;
      }
    ...
    if (kind === "number") {
      ...
      if (!Number.isFinite(num)) {
        pushIssue(errors, path22, "non_finite_number", "numbers must be finite");
        return null;
      }
      return Object.is(num, -0) ? "0" : String(num);
    }
    if (kind !== "object") {
      pushIssue(errors, path22, "unsupported_type", `${kind} has no canonical JSON form`);
      return null;
    }
    if (active.has(node)) {
      pushIssue(errors, path22, "cycle_detected", "value contains a cycle");
      return null;
    }
    active.add(node);
    try {
      if (safeIsArray(node)) {
        ...
        if (length.ok === false) {
          pushIssue(errors, path22, "array_length_unreadable", length.reason);
          return null;
        }
        if (length.length > MAX_ARRAY_ITEMS) {
          pushIssue(errors, path22, "array_too_long", `array exceeds ${MAX_ARRAY_ITEMS} items`);
          return null;
        }
        const parts2 = [];
        for (let index = 0; index < length.length; index += 1) {
          ...
          if (!safeHasOwn(node, key)) {
            pushIssue(errors, `${path22}[${index}]`, "sparse_array_hole", "array holes have no canonical JSON form");
            return null;
          }
          const read = safeGet(node, key);
          if (read.ok === false) {
            pushIssue(errors, `${path22}[${index}]`, "property_read_threw", read.reason);
            return null;
          }
          const encoded = walk(read.value, `${path22}[${index}]`, depth + 1);
      ...
      if (keys.ok === false) {
        pushIssue(errors, path22, "own_keys_threw", keys.reason);
        return null;
      }
      if (keys.keys.length > MAX_OBJECT_KEYS) {
        pushIssue(errors, path22, "object_too_wide", `object exceeds ${MAX_OBJECT_KEYS} keys`);
        return null;
      }
      ...
      for (const key of sorted) {
        const read = safeGet(node, key);
        if (read.ok === false) {
          pushIssue(errors, `${path22}.${key}`, "property_read_threw", read.reason);
          return null;
        }
        if (read.value === void 0) {
          pushIssue(errors, `${path22}.${key}`, "undefined_value", "undefined has no canonical JSON form");
          return null;
        }
        const encoded = walk(read.value, `${path22}.${key}`, depth + 1);
        if (encoded === null) return null;
...
function sha256Of(text) {
  return `sha256:${(0, import_node_crypto.createHash)("sha256").update(text, "utf8").digest("hex")}`;
}
...
function readShape(issues, value, path22, allowed) {
  if (value === null || typeof value !== "object") {
    pushIssue(issues, path22, "not_an_object", `${path22} must be an object`);
    return false;
  }
  if (safeIsArray(value)) {
    pushIssue(issues, path22, "not_an_object", `${path22} must be an object, not an array`);
    return false;
  }
  const keys = safeOwnKeys(value);
  if (keys.ok === false) {
    pushIssue(issues, path22, "own_keys_threw", `${path22}: ${keys.reason}`);
    return false;
  }
  ...
  for (const key of [...keys.keys].sort()) {
    if (!allowedSet.has(key)) {
      pushIssue(issues, `${path22}.${key}`, "unexpected_key", `${path22}.${key} is not part of the closed schema`);
      ok = false;
    }
  }
  for (const key of allowed) {
    if (!safeHasOwn(value, key)) {
      pushIssue(issues, `${path22}.${key}`, "missing_field", `${path22}.${key} is required`);
      ok = false;
    }
  }
  return ok;
}
function readString(issues, parent, path22, key, options = {}) {
  const fieldPath = `${path22}.${key}`;
...
function readArray(issues, parent, path22, key, options = {}) {
  const fieldPath = `${path22}.${key}`;
  const read = safeGet(parent, key);
  if (read.ok === false) {
    pushIssue(issues, fieldPath, "property_read_threw", `${fieldPath}: property read threw`);
...
function readStringArray(issues, parent, path22, key, options = {}) {
  const items = readArray(issues, parent, path22, key, options);
...
function readItemArray(issues, parent, path22, key, options, readItem) {
  const items = readArray(issues, parent, path22, key, options);
  if (items === null) return null;
  const fieldPath = `${path22}.${key}`;
  const out = [];
...
function readProvenance(issues, parent, path22) {
  const read = safeGet(parent, "provenance");
  if (read.ok === false) {
    pushIssue(issues, `${path22}.provenance`, "property_read_threw", `${path22}.provenance: property read threw`);
    return null;
  }
  const provenancePath = `${path22}.provenance`;
...
function readPlanBody(issues, body, path22) {
  if (!readShape(issues, body, path22, ["objectives", "steps"])) return null;
  const objectives = readStringArray(issues, body, path22, "objectives", { min: 1, max: 64, itemMaxLength: 500 });
  const steps = readItemArray(issues, body, path22, "steps", { min: 1, max: 256 }, (itemIssues, item, itemPath) => {
...
function readSpecBody(issues, body, path22) {
  if (!readShape(issues, body, path22, ["requirements"])) return null;
...
function readHandoffBody(issues, body, path22) {
  if (!readShape(issues, body, path22, ["task_id", "status", "artifacts", "issues"])) return null;
  const taskId = readString(issues, body, path22, "task_id", { pattern: DOCUMENT_ITEM_ID_PATTERN });
  const status = readString(issues, body, path22, "status", { enumOf: HANDOFF_STATUSES });
  const artifacts = readStringArray(issues, body, path22, "artifacts", { max: 256, itemMaxLength: 1e3 });
  const handoffIssues = readStringArray(issues, body, path22, "issues", { max: 256, itemMaxLength: 1e3 });
  if (taskId === null || status === null || artifacts === null || handoffIssues === null) return null;
  return { task_id: taskId, status, artifacts, issues: handoffIssues };
}
function readReviewBody(issues, body, path22) {
  if (!readShape(issues, body, path22, ["verdict", "findings"])) return null;
  const verdict = readString(issues, body, path22, "verdict", { enumOf: REVIEW_VERDICTS });
...
function readVerifyBody(issues, body, path22) {
  if (!readShape(issues, body, path22, ["outcome", "checks"])) return null;
  const outcome = readString(issues, body, path22, "outcome", { enumOf: VERIFY_OUTCOMES });
...
function readRunStateLanes(runDir2) {
  const obj = readJsonObject(path8.join(runDir2, "run-state.json"));
...
function readHeartbeatAges(runDir2, now = Date.now()) {
  ...
  const dir = path8.join(runDir2, "in-progress");
  let names;
  try {
...
function readReceiptEvidence(runDir2) {
  const dir = path8.join(runDir2, "handoffs");
  let names;
  try {
    names = fs7.readdirSync(dir);
  } catch {
...
function appendEvent(runDir2, event, opts = {}) {
  ...
  if (opts.forceFallback || process.platform === "win32") {
    ...
    const path22 = laneFallbackPath(runDir2, laneId2);
    (0, import_node_fs2.mkdirSync)((0, import_node_path2.dirname)(path22), { recursive: true });
    const fd = (0, import_node_fs2.openSync)(path22, "a");
    try {
...
function sha256(text) {
  return `sha256:${crypto2.createHash("sha256").update(text, "utf8").digest("hex")}`;
}
function sealReceiptRecord(input) {
  ...
  return { ...body, record_hash: sha256(canonicalJson2(body)) };
}
function verifyReceiptRecord(record) {
  const { record_hash, ...body } = record;
  return typeof record_hash === "string" && sha256(canonicalJson2(body)) === record_hash;
}
...
function writeAllSync(fd, text) {
  ...
  while (written < buf.length) {
    written += fs9.writeSync(fd, buf, written, buf.length - written);
  }
}
function readAllSync(fd) {
  const size = fs9.fstatSync(fd).size;
  if (size === 0) return "";
  const buf = Buffer.allocUnsafe(size);
  let read = 0;
  while (read < size) {
    const n = fs9.readSync(fd, buf, read, size - read, read);
    if (n <= 0) break;
    read += n;
  }
...
function realpathOrNull(target) {
  try {
    return (fs9.realpathSync.native ?? fs9.realpathSync)(target);
  } catch {
    return null;
  }
}
...
function canonicalJournalPath(journalPath2) {
  ...
  for (let hop = 0; hop < CANONICAL_PATH_MAX_LINK_HOPS; hop += 1) {
    ...
    return path11.join(canonicalJournalPath(parent), path11.basename(current));
  }
  return current;
}
...
function acquireJournalAuthority(journalPath2, io = defaultJournalIo, lockOptions = {}, access = "read", checkpointPath = null) {
  ...
  const journalParent = path11.dirname(identity.path);
  ...
  const checkpointParentPath = checkpointPath === null ? null : canonicalJournalPath(path11.dirname(checkpointPath));
  const checkpointCanonical = checkpointParentPath === null || checkpointPath === null ? null : path11.join(checkpointParentPath, path11.basename(checkpointPath));
  ...
  const pin = () => {
    ...
    let writable = false;
    try {
      fd = fs9.openSync(identity.path, JOURNAL_ACCESS_FLAGS[access]);
    ...
    if (fd === null) {
      try {
        fd = fs9.openSync(identity.path, "r");
        writable = false;
      } catch (err) {
        return unstable(
  ...
  const pinCheckpointParent = () => {
    ...
    let fd = null;
    try {
      fd = fs9.openSync(checkpointParent, "r");
    } catch {
      fd = null;
    }
  ...
  const verify = (stage) => {
    ...
    if (parentInode !== null && parentDevice !== null) {
      const parentNow = statOrNull(path11.dirname(identity.path));
    ...
    if (checkpointPath !== null) {
      ...
      const parentNamedNow = canonicalJournalPath(path11.dirname(checkpointPath));
    ...
    defaultJournalIo = {
      appendLine(journalPath2, text, bound) {
        ...
        if (bound && bound.writable && bound.fd >= 0) {
          ...
          fs9.fsyncSync(bound.fd);
          return;
        }
        fs9.mkdirSync(path11.dirname(journalPath2), { recursive: true });
        const fd = fs9.openSync(journalPath2, "a");
        try {
          fs9.writeSync(fd, text, null, "utf8");
          fs9.fsyncSync(fd);
        } finally {
          fs9.closeSync(fd);
        }
      ...
      readAll(journalPath2, bound) {
        ...
        try {
          return fs9.readFileSync(journalPath2, "utf8");
        } catch {
          return null;
        }
      },
      ...
      truncate(journalPath2, size, bound) {
        ...
        if (bound && bound.writable && bound.fd >= 0) {
          fs9.ftruncateSync(bound.fd, size);
          return;
        }
        fs9.truncateSync(journalPath2, size);
      },
      ...
      acquireLock(lockPath2) {
        fs9.mkdirSync(path11.dirname(lockPath2), { recursive: true });
        try {
          fs9.mkdirSync(lockPath2);
        } catch (err) {
...
function makeProbePaths(parent, name) {
  const dir = path12.join(parent, name);
  fs10.mkdirSync(dir, { recursive: true });
  return { dir, journal: path12.join(dir, JOURNAL_LEAF), checkpoint: path12.join(dir, CHECKPOINT_LEAF) };
}
...
function layDownSealedJournal(port, paths, records, runId) {
  fs10.writeFileSync(paths.journal, `${records.map((record) => JSON.stringify(record)).join("\n")}
`, "utf8");
  ...
  fs10.writeFileSync(paths.checkpoint, `${JSON.stringify(checkpoint, null, 2)}
`, "utf8");
...
function evaluateReceiptJournalConformance(request) {
  ...
  const usableRoot = typeof root === "string" && root.length > 0 && path12.isAbsolute(root) && isExistingDirectory(root);
  if (!usableRoot) {
    return refuseEvaluation2(MH06_REFUSAL_CONTROLS.journalRootUnusable, "scenario_evidence_incomplete", [
  ...
  const identity = request.evidence_identity;
  const workspace = fs10.mkdtempSync(path12.join(root, PROBE_WORKSPACE_PREFIX));
  const verdicts = {};
  try {
    for (const stableId of MH06_SCENARIO_IDS) {
...
function liveLogPath2(runDir2) {
  return path13.join(runDir2, "logs", "v1.4-events.jsonl");
}
...
function readRunManifest(cwd, slug) {
  try {
    const raw = fs12.readFileSync(manifestPathFor(cwd, slug), "utf8");
...
function runWriteRunManifestCli(argv = process.argv.slice(2)) {
  ...
  if (!fs12.existsSync(args.cwd) || !fs12.statSync(args.cwd).isDirectory()) {
...
var fs12, path14, WAVE_STATUSES, PROGRAM_STATUSES;
...
function verifyReleaseClaim(claim, archive) {
  ...
  if (claim.archive_sha256 !== sha2562(archive) || claim.archive_size !== archive.length) {
    errors.push("archive checksum/size mismatch");
  }
  if (claim.manifest_sha256 !== sha2562(stableJson(claim.manifest)) || claim.manifest.entries.length === 0) {
...
`;
  const coreDir = path17.resolve(__dirname, "../../lifecycle/workflows");
  const coreWorkflows = {};
  for (const member of NEUTRAL_CORE_MEMBERS) {
    coreWorkflows[member] = fs14.readFileSync(path17.join(coreDir, member), "utf8");
  }
  ...
  for (const [rel, content] of Object.entries(files)) {
    const target = path17.join(base, rel);
    fs14.mkdirSync(path17.dirname(target), { recursive: true });
    fs14.writeFileSync(target, content);
  }
}
function batteryWorkspace() {
  if (batteryWorkspaceSingleton !== null) return batteryWorkspaceSingleton;
  const base = fs14.mkdtempSync(path17.join(os.tmpdir(), "a21x-integration-battery-"));
  const journalRoot = path17.join(base, "journal-root");
  const migrationJournalRoot = path17.join(base, "migration-journal-root");
  fs14.mkdirSync(journalRoot, { recursive: true });
  fs14.mkdirSync(migrationJournalRoot, { recursive: true });
  const moduleBase = path17.join(base, "module-workspace");
  buildBatteryModuleWorkspace(moduleBase);
  batteryWorkspaceSingleton = {
    journalRoot,
    migrationJournalRoot,
    pluginRoot: path17.join(moduleBase, "plugin"),
    consumerRoots: {
      website: path17.join(moduleBase, "website"),
      benchmark: path17.join(moduleBase, "benchmark")
    }
  };
...
function assertNoSymlinkComponents(base, resolved) {
  ...
  for (const segment of rel.split(path19.sep)) {
    ...
    const stat = fs15.lstatSync(current);
    if (stat.isSymbolicLink()) {
...
function nearestProjectRoot(startDir) {
  ...
  let nearestGuildDir = null;
  for (; ; ) {
    if (fs15.existsSync(path19.join(current, ".git"))) return current;
    if (nearestGuildDir === null) {
      const guildDir = path19.join(current, ".guild");
      try {
        if (fs15.existsSync(guildDir) && fs15.statSync(guildDir).isDirectory()) nearestGuildDir = current;
      } catch {
      }
    }
...
function matchTrustedBase(resolved) {
  const tmpBase = path19.resolve(os2.tmpdir());
...
function readMigrationJournal(handle) {
  ...
  const entries = fs15.existsSync(root) ? fs15.readdirSync(root) : [];
  ...
  const noFollow = typeof fs15.constants.O_NOFOLLOW === "number" ? fs15.constants.O_NOFOLLOW : 0;
  const fd = fs15.openSync(file, fs15.constants.O_RDONLY | noFollow);
  let content;
  try {
    content = fs15.readFileSync(fd, "utf8");
  } finally {
    fs15.closeSync(fd);
  }
...
function appendMigrationDecision(handle, input) {
  ...
  let lockFd;
  try {
    lockFd = fs15.openSync(lockPath2, "wx");
  } catch {
    throw new Error(
  ...
  try {
    ...
    const priorContent = fs15.existsSync(file) ? fs15.readFileSync(file, "utf8") : "";
    fs15.writeFileSync(tmp, `${priorContent}${JSON.stringify(record)}
`);
    fs15.renameSync(tmp, file);
    return record;
  } finally {
    fs15.closeSync(lockFd);
    fs15.rmSync(lockPath2, { force: true });
  }
}
...
function runStatePath(runDir2) {
  return path20.join(runDir2, "run-state.json");
}
function loadRunState(runDir2) {
  let raw;
  try {
    raw = fs16.readFileSync(runStatePath(runDir2), "utf8");
  } catch {
    return null;
  }
  let parsed;
...
function writeRunStateAtomic(runDir2, state) {
  fs16.mkdirSync(runDir2, { recursive: true });
...
function laneResumeCheckpointPath(runDir2, laneId2) {
  return path20.join(runDir2, "lanes", laneId2, "resume.json");
}
function readResumeEnabled(cwd) {
  const settingsPath = path20.join(resolveGuildRoot(cwd), ".guild", "settings.json");
  try {
...
function loadLaneResumeCheckpoint(runDir2, laneId2) {
  try {
    const raw = fs16.readFileSync(laneResumeCheckpointPath(runDir2, laneId2), "utf8");
    const parsed = JSON.parse(raw);
