...
function batteryWorkspace() {
  ...
  const base = fs15.mkdtempSync(path18.join(os2.tmpdir(), "a21x-integration-battery-"));
...
var fs17, path20, WAVE_STATUSES, PROGRAM_STATUSES;
...
function readSettingsModels(cwd) {
  try {
    const raw = fs18.readFileSync(path22.join(cwd, ".guild", "settings.json"), "utf8");
    const parsed = JSON.parse(raw);
...
function writeHostCapability(opts) {
  ...
  const hostDir = path22.join(opts.cwd, ".guild", "hosts", manifest.host_id);
  fs18.mkdirSync(hostDir, { recursive: true });
  const manifestPath = path22.join(hostDir, "capability.json");
