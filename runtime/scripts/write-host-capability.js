...
function mh07RequiredConsumerRoots(pluginRoot) {
  const workspace = path3.dirname(path3.resolve(pluginRoot));
  const roots = {};
  for (const name of MH07_REQUIRED_CONSUMERS) roots[name] = path3.join(workspace, name);
  return roots;
}
...
function moduleIdOfPath(root, target, moduleIds) {
  const rel = relativeTo(path3.join(root, "src", "modules"), target);
  if (rel.indexOf("..") === 0 || path3.isAbsolute(rel)) return null;
...
function classifyDestination(root, importer, specifier, fromModule, moduleIds) {
  ...
  if (rel.indexOf("..") === 0 || path3.isAbsolute(rel)) {
  ...
  const publicIndex = path3.join(root, "src", "modules", toModule, "index.ts");
  const isPublic = path3.resolve(resolved) === path3.resolve(publicIndex);
  return {
...
function mh07ScanDependencyGraph(root, extract = extractNeutralImportEdges, moduleIdsOverride) {
  ...
  const moduleIds = moduleIdsOverride ?? (fs2.existsSync(modulesDir) ? fs2.readdirSync(modulesDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name).sort() : []);
  ...
  for (const file of files) {
    ...
    const extracted = extract(fs2.readFileSync(file, "utf8"));
...
function consumerSourceReaches(consumerRoot, pluginRoot, extract) {
  const found = [];
  const pluginSource = path3.resolve(path3.join(pluginRoot, "src"));
  for (const file of walkTypeScript(consumerRoot, MH07_CONSUMER_SCAN_EXCLUDED_DIRECTORIES)) {
    for (const edge of extract(fs2.readFileSync(file, "utf8"))) {
      if (edge.kind !== "resolved" || edge.specifier === void 0) continue;
      if (edge.specifier.indexOf(".") !== 0) continue;
      const resolved = path3.resolve(resolveRelative(file, edge.specifier));
      if (resolved === pluginSource || resolved.indexOf(`${pluginSource}${path3.sep}`) === 0) {
        found.push({
...
function evaluateNeutralModuleBoundaries(request) {
  ...
  for (const name of consumerNames) {
    ...
    if (typeof consumerRoot !== "string" || !fs2.existsSync(consumerRoot)) {
      return refuse3(
        MH07_REFUSAL_CONTROLS.evidenceBindingMissing,
        "scenario_evidence_incomplete",
  ...
  for (const name of MH07_REQUIRED_CONSUMERS) {
    ...
    const bound = typeof suppliedRoot === "string" && path3.resolve(suppliedRoot) === path3.resolve(requiredRoot);
    if (!bound) {
      return refuse3(
        MH07_REFUSAL_CONTROLS.evidenceBindingMissing,
        "scenario_evidence_incomplete",
    ...
    try {
      position = fs2.lstatSync(suppliedRoot);
    } catch (error) {
  ...
  const sourcesOf = (moduleId) => walkTypeScript(
    path3.join(root, "src", "modules", moduleId),
    MH07_GRAPH_SCOPE.excluded_directories
  ).map((file) => ({ path: relativeTo(root, file), source: fs2.readFileSync(file, "utf8") }));
  ...
  for (const member of NEUTRAL_CORE_MEMBERS) {
    const memberPath = coreHostModule === null ? null : path3.join(root, "src", "modules", coreHostModule, "workflows", member);
    if (memberPath === null || !fs2.existsSync(memberPath)) {
      missingCoreMembers.push(member);
      continue;
    }
    coreFiles.push({ path: member, source: fs2.readFileSync(memberPath, "utf8") });
  }
  ...
  const contractConsumersWithoutVersion = contractConsumerServices.filter((moduleId) => {
    const indexPath = path3.join(root, "src", "modules", moduleId, "index.ts");
    if (!fs2.existsSync(indexPath)) return true;
    return fs2.readFileSync(indexPath, "utf8").indexOf(MODULE_PUBLIC_API_DECLARATION) === -1;
  });
...
function readRunStateLanes(runDir2) {
  const obj = readJsonObject(path4.join(runDir2, "run-state.json"));
...
function readHeartbeatAges(runDir2, now = Date.now()) {
  ...
  const dir = path4.join(runDir2, "in-progress");
  let names;
  try {
...
function readReceiptEvidence(runDir2) {
  const dir = path4.join(runDir2, "handoffs");
  let names;
  try {
    names = fs3.readdirSync(dir);
  } catch {
  ...
  for (const name of names.sort()) {
    ...
    let text = null;
    try {
      if (fs3.statSync(filePath).size <= MAX_RECEIPT_BYTES) {
...
var fs3, path4, DEFAULT_HEARTBEAT_TIMEOUT_MS, TERMINAL_STATUSES, MAX_RECEIPT_BYTES, NO_RECEIPT, USAGE;
...
function isUnderDurable(abs, guildDir) {
  const rel = path11.relative(path11.resolve(guildDir), path11.resolve(abs));
...
function resolveContainedRealDir(abs, root) {
  ...
  const rel = path12.relative(realRoot2, real);
  if (rel === "" || rel.startsWith("..") || path12.isAbsolute(rel)) return null;
  return real;
}
...
var crypto3, os2, path13, OVERRIDE_KEYS, GUILD_NAMESPACE;
...
function hostCapabilityCacheDir(cwd, hostId) {
  const storage = createGuildStorage(realRoot(cwd));
  return hostId === void 0 ? storage.cache("hosts") : storage.cache("hosts", hostId);
}
...
function appendEvent(runDir2, event, opts = {}) {
  ...
  if (opts.forceFallback || process.platform === "win32") {
    ...
    const path29 = laneFallbackPath(runDir2, laneId2);
...
function readSentinel(cwd) {
  ...
  try {
    const value = fs12.readFileSync(sentinelPath, "utf8").trim();
...
function runStatePath(runDir2) {
  return path17.join(runDir2, "run-state.json");
}
function loadRunState(runDir2) {
  let raw;
  try {
    raw = fs13.readFileSync(runStatePath(runDir2), "utf8");
  } catch {
    return null;
  }
  let parsed;
...
function writeRunStateAtomic(runDir2, state) {
  fs13.mkdirSync(runDir2, { recursive: true });
...
function laneResumeCheckpointPath(runDir2, laneId2) {
  return path17.join(runDir2, "lanes", laneId2, "resume.json");
}
function readResumeEnabled(cwd) {
  const settingsPath = path17.join(resolveGuildRoot(cwd), ".guild", "settings.json");
  try {
...
function loadLaneResumeCheckpoint(runDir2, laneId2) {
  try {
    const raw = fs13.readFileSync(laneResumeCheckpointPath(runDir2, laneId2), "utf8");
    const parsed = JSON.parse(raw);
...
function readPlanOwnerTaskIds(guildRoot, slug) {
  ...
  let raw;
  try {
    raw = fs14.readFileSync(path19.join(guildRoot, ".guild", "plan", `${slug}.md`), "utf8");
  } catch {
    return map;
  }
...
function canonicalJournalPath(journalPath2) {
  let current = path21.resolve(journalPath2);
  ...
  "src/modules/telemetry/workflows/receipt-journal.ts"() {
    ...
    defaultJournalIo = {
      appendLine(journalPath2, text, bound) {
        ...
        fs15.mkdirSync(path21.dirname(journalPath2), { recursive: true });
        const fd = fs15.openSync(journalPath2, "a");
        try {
      ...
      readAll(journalPath2, bound) {
        ...
        try {
          return fs15.readFileSync(journalPath2, "utf8");
        } catch {
          return null;
        }
      },
      ...
      truncate(journalPath2, size, bound) {
        ...
        fs15.truncateSync(journalPath2, size);
      },
      ...
      acquireLock(lockPath2) {
        fs15.mkdirSync(path21.dirname(lockPath2), { recursive: true });
        try {
          fs15.mkdirSync(lockPath2);
        } catch (err) {
...
function isExistingDirectory(target) {
  try {
    return fs16.statSync(target).isDirectory();
  } catch {
    return false;
  }
}
...
function evaluateReceiptJournalConformance(request) {
  ...
  const usableRoot = typeof root === "string" && root.length > 0 && path22.isAbsolute(root) && isExistingDirectory(root);
  if (!usableRoot) {
    return refuseEvaluation(MH06_REFUSAL_CONTROLS.journalRootUnusable, "scenario_evidence_incomplete", [
  ...
  const identity = request.evidence_identity;
  const workspace = fs16.mkdtempSync(path22.join(root, PROBE_WORKSPACE_PREFIX));
  const verdicts = {};
  try {
    for (const stableId of MH06_SCENARIO_IDS) {
...
function liveLogPath2(runDir2) {
  return path23.join(runDir2, "logs", "v1.4-events.jsonl");
}
...
`;
  const coreDir = path24.resolve(__dirname, "../../lifecycle/workflows");
  const coreWorkflows = {};
  for (const member of NEUTRAL_CORE_MEMBERS) {
    coreWorkflows[member] = fs18.readFileSync(path24.join(coreDir, member), "utf8");
  }
  ...
  for (const [rel, content] of Object.entries(files)) {
    const target = path24.join(base, rel);
    fs18.mkdirSync(path24.dirname(target), { recursive: true });
    fs18.writeFileSync(target, content);
  }
}
function batteryWorkspace() {
  if (batteryWorkspaceSingleton !== null) return batteryWorkspaceSingleton;
  const base = fs18.mkdtempSync(path24.join(os3.tmpdir(), "a21x-integration-battery-"));
  const journalRoot = path24.join(base, "journal-root");
  const migrationJournalRoot = path24.join(base, "migration-journal-root");
  fs18.mkdirSync(journalRoot, { recursive: true });
  fs18.mkdirSync(migrationJournalRoot, { recursive: true });
  const moduleBase = path24.join(base, "module-workspace");
  buildBatteryModuleWorkspace(moduleBase);
  batteryWorkspaceSingleton = {
    journalRoot,
    migrationJournalRoot,
    pluginRoot: path24.join(moduleBase, "plugin"),
    consumerRoots: {
      website: path24.join(moduleBase, "website"),
      benchmark: path24.join(moduleBase, "benchmark")
    }
  };
...
// src/modules/distribution/workflows/release-package-identity.ts
...
init_sealed_collections();
var HOST_MODES = Object.freeze(["read_only", "ask", "accept_edits", "auto", "bypass_all"]);
...
var HOST_MODE_SET = new Set(HOST_MODES);
...
function resolveHostKind(host, env) {
  if (host !== void 0 && ALL_HOST_KINDS.includes(host)) return host;
  const raw = (env["GUILD_HOST"] ?? "").trim().toLowerCase();
  if (ALL_HOST_KINDS.includes(raw)) return raw;
  return "claude";
}
function resolveIndependentAgents(hostKind, env) {
...
function readSettingsModels(cwd) {
  try {
    const raw = fs21.readFileSync(path28.join(cwd, ".guild", "settings.json"), "utf8");
    const parsed = JSON.parse(raw);
    ...
    if (Array.isArray(models.list)) {
      out.list = models.list.filter((m) => typeof m === "string");
    }
    return out;
  } catch {
    return {};
...
function buildCapability(opts) {
  ...
  const models = settings.list && settings.list.length > 0 ? settings.list : Array.from(/* @__PURE__ */ new Set([tierModels.cheap, tierModels.mid, tierModels.powerful]));
  ...
  return {
    ...
    advertised_at: (/* @__PURE__ */ new Date()).toISOString(),
    ...
    tool_support: {
      subagent: true,
      // always available — the universal fallback backend.
      agent_team: tmux,
      // tmux-backed team panes require tmux.
      independent_agents: resolveIndependentAgents(hostKind, env),
      tmux,
      mcp: true,
      // Claude Code MCP loader is available on supported hosts.
...
function writeHostCapability(opts) {
  ...
  fs21.mkdirSync(path28.dirname(manifestPath), { recursive: true });
...
function parseArgs4(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--cwd" && argv[i + 1]) out.cwd = argv[++i];
      ...
      const v = argv[++i];
      if (ALL_HOST_KINDS.includes(v)) out.host = v;
...
function main() {
  const parsed = parseArgs4(process.argv.slice(2));
...
`
  ...
  try {
    ...
    process.stderr.write(`[write-host-capability] ERROR: ${e.message}
`);
    process.exit(2);
  }
}
