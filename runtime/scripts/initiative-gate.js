...
var initiative_gate_exports = {};
__export(initiative_gate_exports, {
  buildD8Input: () => buildD8Input,
  checkInitiativeEvidenceRefs: () => checkInitiativeEvidenceRefs,
  loadInitiativeManifest: () => loadInitiativeManifest,
  parseGateArgs: () => parseGateArgs,
  runCloseCheck: () => runCloseCheck,
...
function loadInitiativeManifest(root, id) {
  for (const bucket of ["active", "archived"]) {
    ...
    return { path: p, raw: inner };
  }
  return null;
}
var EXECUTION_STATUS_ALIASES = {
  in_progress: "active",
  "in-progress": "active",
  completed: "done",
  complete: "done"
};
var DOCUMENTATION_STATUS_ALIASES = {
...
function buildD8Input(raw, execVerified) {
  const warnings = [];
...
function classifyEvidenceRef(root, source, rawRef) {
...
function checkInitiativeEvidenceRefs(root, initiativeDir, manifest) {
  const checked = [];
  const missing = [];
  const invalid = [];
  const consume = (source, rawRefs) => {
    if (rawRefs === void 0) return;
  ...
  if (fs.existsSync(workItemsDir)) {
    ...
    for (const name of names) {
      ...
      consume(source, parsed["evidence_refs"]);
    }
  }
...
${USAGE}` };
  ...
  for (let i = 0; i < rest.length; i++) {
    const arg = rest[i];
    if (arg === "--initiative" && rest[i + 1] !== void 0) initiative = rest[++i];
    else if (arg.startsWith("--initiative=")) initiative = arg.slice("--initiative=".length);
    else if (arg === "--root" && rest[i + 1] !== void 0) root = rest[++i];
    else if (arg.startsWith("--root=")) root = arg.slice("--root=".length);
    else if (arg === "--exec-verified" && rest[i + 1] !== void 0) execVerified = rest[++i] === "true";
...
function main() {
  ...
  if (parsed.command === "close-check") {
    const out2 = runCloseCheck(parsed.root, parsed.initiative, parsed.execVerified);
    if ("error" in out2) {
      process.stderr.write(`initiative-gate: ${out2.error}
`);
      return 2;
    }
    process.stdout.write(JSON.stringify(out2, null, 2) + "\n");
    return out2.pass ? 0 : 1;
  }
