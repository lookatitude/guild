...
## Team membership and purpose propagation

- Logical-team membership is task-derived and uncapped; backend slots schedule waves and never shrink the approved roster. A membership change requires a new user-approved proposal revision.
- A `followups:` entry that implies adding or removing a specialist is a team restructure for the user to decide, never a silent reassignment.
- An authoritative purpose floor propagates through every child dispatch, retry, resumption, and receipt. Descendants may raise a floor, never lower or erase it, and record the root `purpose_origin`.
