...
1. [architecture-overview.md](../../../../.guild/wiki/entities/architecture-overview.md) - component map, state boundaries, invariants, and the one-machine reconciliation.
2. [plugin-module-architecture.md](../../../../.guild/wiki/entities/plugin-module-architecture.md) - current plugin module layout, module ownership manifests, compatibility shims, and packaging validation.
3. [phase-entrypoints.md](../entities/phase-entrypoints.md) - how to start from `init`, `ideate`, `plan`, `build`, `qa`, or `ops`.
4. [lifecycle-overview.md](../entities/lifecycle-overview.md) - every phase, gate, loop, and artifact across the reconciled linear / phase / initiative lenses.
5. [team-composition.md](../entities/team-composition.md) - how each phase gets a tailored team, backend, skills, tools, advisory agents, and MCP access.
6. [knowledge-and-advisory.md](../entities/knowledge-and-advisory.md) - wiki, memory recall, learning, and per-agent advisors.
7. [agent-and-skill-factory.md](../entities/agent-and-skill-factory.md) - how new concise Claude Code-first skills and personas are created, evaluated, shadowed, and registered.
8. [tools-and-mcp.md](../standards/tools-and-mcp.md) - tool and MCP attachment rules for subagents and tmux agent teams.
9. [adversarial-review.md](../../../../.guild/wiki/entities/adversarial-review.md) - built-in challenger behavior, cross-host reciprocal review broker, and review gates.
10. [target-architecture.md](../../../../.guild/wiki/entities/target-architecture.md) - frozen `task_run` / `host_adapter` contracts, graph artifacts, and the portable core.
11. [edge-cases.md](../entities/edge-cases.md) - failure modes and advisory defaults.
12. [codebase-understanding.md](../entities/codebase-understanding.md) - brownfield reverse-spec engine, knowledge-graph artifacts, refresh policy, internalized (forked, never a runtime dependency). Companion: [knowledge-classification-schema.md](../entities/knowledge-classification-schema.md) - the `guild.knowledge_graph.v2` node metadata contract (`category` enum / `importance` / `confidence` / `labels[]` / `topic_path`) read by `kg-query` ranking + `wiki-lint`.
13. [review-checklist.md](../standards/review-checklist.md) - completion and implementation checklist.
14. [review-trail.md](../../../../.guild/wiki/_archive/v2-design/sources/review-trail.md) - adversarial review findings and clean sign-off.
