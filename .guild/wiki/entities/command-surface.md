...
- **Quality release/blocker gate (`/guild qa`).** After the `qa-test-strategy`
  producer and the composer-derived G-quality challenger panel (`security` baseline + `architect` gated on `multi_component`) converge,
