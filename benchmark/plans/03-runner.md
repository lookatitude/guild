...
1. **argv** — a JSON array (never a shell string). The `claude` binary is the resolved absolute path, never the bare string `"claude"`. Per `p3-runner-architecture.md §2.2` and ADR-004, the prompt is passed via `--prompt-file`, not as a positional argument (security mitigation M5).
...
Mode 2 will **not** spawn `claude` unless `GUILD_BENCHMARK_LIVE=1` is set in the environment. Without it, `npm run benchmark -- run` behaves like `--dry-run` (prints the plan and exits 0). This is the default-safe posture: CI never burns tokens, and an operator who runs `npm run benchmark -- run` exploratorily gets a plan dump rather than a real spawn. Explicit opt-in is the security mitigation against accidental cost or accidental side-effects on the operator's `~/.claude/` auth context.
...
| `stdio` | `["ignore", "pipe", "pipe"]` (no parent stdin; pipes tee'd to disk) | `p3-runner-architecture.md §2.5` |
...
- **claude argv template — KNOWN ISSUE.** Per the T2 backend follow-up (`.guild/runs/run-2026-04-26-benchmark-factory-p3/handoffs/T2-backend.md §Followups #1`), the default argv template `claude --print --prompt-file <path> --workdir <ws> --output-format stream-json` was rejected by the real `claude` binary during smoke (`error: unknown option '--prompt-file'`). Verify with `--dry-run` against your installed `claude` version. The long-term fix is the planned `GUILD_BENCHMARK_ARGV_TEMPLATE` env override (or a `case.yaml` `runner_args:` field) so operators can tune the flag set without code changes; until that ships, an operator whose `claude` rejects the default template can edit `runner.ts`'s `buildArgv()` locally and re-run.
