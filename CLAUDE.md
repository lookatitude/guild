...
The wiki read path uses a lazy SQLite read-through cache (`index: "auto"`, default); disable with `index: "off"`. See the Guild docs site → `https://guildstack.dev/docs/configuration` (`defaults.index.*`).
...
An explicit `agent_mode` value other than `auto` **pins** the backend, subject to availability — pinning `team` on a tmux-less host is rejected/warned (owner: `tooling-engineer`). **`defaults.agent_team` is hard-removed in v2.0**: it is rejected as an unknown key by `--validate`, stripped by resolve mode, and the `.guild/config.yml` runtime reader is gone. There is no warn-once alias — `agent_team` in any config will cause a validation error. Migrate via `/guild:migrate`.

Remote cross-host SSH dispatch is gated by `defaults.cross_host` (`.guild/settings.json`) and declared per-team via `host:` in `team.yaml`. See the Guild docs site → `https://guildstack.dev/docs/configuration` (`defaults.cross_host.*`).
