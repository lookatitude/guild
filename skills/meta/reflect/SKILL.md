...
1. `.guild/runs/<run-id>/summary.md` — the compact run summary produced by `scripts/trace-summarize.ts` (P5 tooling-engineer scope) from `events.ndjson`. Contains per-skill trigger counts, per-specialist dispatch/duration, context-bundle sizes, and any `error` events. Reflect reads this instead of the raw event log so it stays grep-able and cheap to re-process.
...
1. **Skill improvement.** A skill triggered incorrectly N >= 2 times within a single run, OR a specialist's followup explicitly names a skill gap (e.g., "guild:context-assemble missed the wiki/decisions/ page"), OR the trigger/no-op ratio in the summary exceeds 0.5. Evidence for a proposal here is the mismatch between the skill's `should_trigger` / `should_not_trigger` evals and the actual routing captured in `summary.md`, or the verbatim followup line naming the skill. Record the skill name, the failure mode, and a quote from the evidence.
