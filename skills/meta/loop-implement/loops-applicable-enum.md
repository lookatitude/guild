...
fixed order in `LOOPS_APPLICABLE_VALUES` (implemented in the separate guild-benchmark repo's `src/loop-applicable.ts`):
...
The separate guild-benchmark repo's `src/loop-applicable.ts` implements all 4 cases; qa pins each in `loop-implement.test.ts`:
