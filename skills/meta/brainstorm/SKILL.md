...
**The reviewer slot is a phase participant — sequence the gate BEFORE the broker.** A cross-host G-spec reviewer is a dispatchable participant of the `ideate` phase (`participation_kind: reviewer_cross_host`), so its dispatch rides the phase's `guild.team_proposal.v2` → `guild.team_decision.v1` gate. Operational order inside this skill: (1) write the draft spec; (2) **resolve the broker policy first, without dispatching** — the fire/skip predicate (`risk ≥ high` / `review: cross` / `--review=cross` / config; self-build = always-on) is a deterministic read of run posture + config, so evaluate it before composing anything: if it resolves to no-fire, record `status: "skipped"`, compose NO proposal, and go straight to spec user-approval (nothing dispatches); (3) if it fires, compose the **minimal ideate-phase proposal** (typically the single reviewer slot; `spec_ref` = the just-written draft spec bytes — an *early-phase* proposal per `guild:team-compose §"Team decision gate"`); (4) hold for the user's `approve`; (5) only then invoke the broker; (6) proceed to spec user-approval. The full spec-driven composition still happens later at `guild:team-compose` for the plan phase. An advisory/review label never bypasses the gate.
