...
`instance_id` (fresh per attempt) is a RUNTIME identity, distinct from the schedule's stable `participant_id` (constant across retries): record the lane's `participant_id` alongside each entry so the `## Stop condition` set-equality check is verifiable from the typed mirror — and because the team_result is fail-soft/omittable, the set-equality gate NEVER rests on it: the Stop condition checks the authoritative receipts + schedule directly. The fresh, distinct `instance_id`s recorded here are the typed proof that each lane ran as its own runtime identity — the team_result is the mirror `guild:review`/benchmark can read without rehydrating full transcripts. (Contract: `guild.team_result.v1` in `src/modules/teams/workflows/station-composer.ts`; writer + run-tree path in `station-signals.ts`.)
