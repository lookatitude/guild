...
1b. **The phase's team decision (`guild.team_decision.v1`) — the dispatch gate.** Dispatch accepts only a **current `approve` decision whose `proposal_hash` equals the hash recomputed from the persisted `guild.team_proposal.v2` artifact** (canonical-YAML SHA-256, hash field omitted — the frozen team-contracts rule). Stale, missing, or hash-mismatched decisions **fail closed**: refuse to dispatch and loop back to `guild:team-compose`'s decision gate (`guild:team-compose §"Team decision gate"`). This applies to EVERY dispatchable participant — workers, advisors, challengers, and reviewer slots alike — in every phase; an advisory label never bypasses the gate. Guild never auto-approves. Any post-approval change to a participant, obligation, dependency, tier/purpose, capability scope, backend, wave structure, concurrency, cost posture, or review independence invalidates the decision and requires renewed user approval.
...
4. `.guild/runs/<run-id>/` — the run directory already created by the lifecycle start. Consume the lifecycle-owned canonical `run-<yyyymmdd-hhmmss>-<slug>` identity from the start result / `.guild/runs/current-run-id` sentinel and pass it unchanged as `--run-id` to the launcher. This skill and the launcher never mint, reshape, or replace a run id; every context bundle, handoff receipt, and assumption log roots under the inherited directory. A missing, unbound, or non-canonical identity blocks dispatch and must be repaired by starting the lifecycle, not by inventing a fallback.
...
1. **Context bundle.** Invoke `guild:context-assemble` for the lane; it writes `.guild/context/<run-id>/<specialist>-<task-id>.md` per §9.3. Read back the bundle's handoff receipt to confirm `bundle_path`, `token_estimate`, `layers_included`, and the linter-returned `definition_ref`. Join the lane owner to the team specialist and require the bundle ref to equal the team's committed ref exactly. A missing/malformed/mismatched ref or bundle blocks dispatch — never fall back to an absolute definition path or chat context.
...
5. **Print + record.** Surface the dispatch line (`lane <task-id> · score N · tier <tier> · model <model>`) and write the score + resolved tier into the run record under `.guild/runs/<run-id>/` (never silent — spec Risk). **Approved-tier consistency:** the approved `guild.team_proposal.v2` records each participant's tier — the re-score CONFIRMS it; if they differ, that is a §4 invalidating change (tier/purpose): do not silently dispatch at either value — surface a restructure (new proposal version) for renewed user approval. When `models.enabled: false`, skip scoring and dispatch at the backend default — EXCEPT `purpose: research` lanes, which dispatch `powerful` regardless (the step-3b floor holds with scoring disabled).
...
- **Subagent / in-process (`agent`) — model-driven path:** the launcher refuses this rung when any lane has a resolved scope. The resolver contract below is retained for the future verified child-env capability and for the legacy unscoped unknown/project-role path; it does not authorize a scoped production `Agent()` call. Every `--dry-run`—scoped or unscoped—emits a non-dispatchable, write-free preview (`dispatchAllowed:false`, `previewOnly:true`) for inspection only; it consumes no approval override and writes no run-tree state:
  ...
  if (isProject && (!spec.definition_ref || spec.definition_ref.schema_version !== "guild.project_definition_ref.v1"))
    throw new Error(`lane ${lane.taskId}: project specialist ${spec.name} has no validated definition_ref`);
  const bundleLint = JSON.parse(runContextBundleLint(bundlePath));
  if (!bundleLint.pass || JSON.stringify(bundleLint.definition_ref) !== JSON.stringify(spec.definition_ref))
    throw new Error(`lane ${lane.taskId}: context bundle definition_ref does not match the team`);
...
The backend is **not** chosen here, and it is **not** chosen at `guild:team-compose` either. It is resolved **once at command intake** by `runStartPreflight` (U3) per the D5 `agent_mode` ladder — **including the first-class cmux rung: cmux > tmux when the intake probe records `CMUX_WORKSPACE_ID`** — and frozen in the run's resolved-settings snapshot (U6 writes `.guild/runs/<run-id>/resolved-settings.json`). The snapshot carries the decision as **`snapshot.dispatch` = `{backend: cmux|tmux|agent|subagent, reason, facts}`** (W4, initiative `run-identity-and-dispatch`). **`execute-plan` reads that snapshot via `readResolvedSettingsSnapshot` (`scripts/lib/run-lifecycle.ts`) and acts on `snapshot.dispatch.backend` (falling back to `snapshot.effective.agent_mode` + `snapshot.providers` only for a legacy snapshot that predates the field)** — it does NOT re-resolve settings, re-run the preflight, or re-read the ambient environment. The launcher enforces the same authority: `agent-team-launcher.ts` consumes the frozen `snapshot.dispatch` and performs **no dispatch-time ambient CMUX/tmux re-probe** when the block is present, so a mid-run environment change (a cmux workspace appearing or vanishing, tmux installed mid-run) **cannot** select a different backend than the run-start decision. `team.yaml` is authoritative for the **team composition** (which specialists, their scope, dependencies, tiers, agent-definition paths), **not** for the backend selection — the backend authority is the resolved snapshot. Mid-run config edits do not silently change execution behavior because the snapshot is fixed at intake. Three invariants you enforce at dispatch — full detail (backend table, launcher invocation, self-build routing table, parallelism rules) in `dispatch.md`:
...
**Backend capacity schedules waves — it never shrinks the team (`guild.team_schedule.v1`).** When the backend cannot run every approved participant concurrently, partition the approved set into dependency-valid execution **waves** (each approved `participant_id` in exactly one wave) and record the schedule bound to the `proposal_hash`/`decision_hash`. The approved proposal's `proposed_schedule` is a non-binding preview (frozen §3), but a binding schedule whose **wave structure or concurrency departs from what the user approved is a §4 invalidating change** — surface it for renewed approval, never dispatch on the changed shape silently. Invariants are **set equality, not counts**: approved participant-id set = base-wave union = dispatch-receipt id set = terminal-outcome id set (removal only via a NEW proposal+decision cycle, cited by decision hash). Retries attach to the same `participant_id` as new attempts — never new participants; a failed or interrupted wave preserves all not-yet-terminal ids for resume — omission is NEVER removal. Resource exhaustion produces an explicit scheduling failure or a request for a user-approved backend/concurrency change; it can never truncate, filter, or reorder the approved roster. Legacy `--team-size N` / `defaults.team.size` are migration-window concurrency *hints* only (clamped to verified backend capacity, with a clamp diagnostic; `allow_larger` is a warned no-op) — none of them can touch the roster.
...
## Reconcile the launcher-authored `guild.team_result.v1` (G6b-2b / TaskCell AT12)

The production launcher writes `.guild/runs/<run-id>/team-result/<station>.json`
immediately after a dispatch batch is committed. That launcher-authored
`guild.team_result.v1` is the **single writer of runtime identity**: its lane
`instance_id` values are copied from the exact TaskCell instances handed to the
selected backend. After handoff collection, this skill enriches that persisted
artifact; it does not create a second result.

1. Read the persisted result with `readTeamResult` and fail closed if it is
   missing or invalid. The launcher already knew the companion `team_plan_ref`;
   do not reconstruct it from chat or a filename convention here.
2. Join each result lane to the canonical
   `.guild/runs/<run-id>/task-cells/**/assignment.json` by `instance_id`. Require
   **identity SET equality** between result lanes and valid assignments, and
   require each lane `role` to equal the assignment's `worker_role`. A missing,
   extra, duplicated, or role-mismatched identity is a dispatch-record defect and
   blocks review.
3. For each joined lane, update only `handoff_ref` and `acceptance_ref` with the
   assignment's canonical TaskCell paths when those durable files exist. This
   skill **MUST NOT mint, derive, replace, reorder, or drop `instance_id`**. It
   also must not add `participant_id`: `guild.team_result.v1` is a closed schema;
   participant set equality remains the separate authoritative schedule/receipt
   close gate.
4. Run the deterministic reconciler after handoff collection (and again after
   ...
   A non-zero exit is a hard review blocker; do not hand-edit the result.
5. The reconciler persists the enriched result through `writeTeamResult`, which re-validates it
   fail closed. A write error is no longer described as harmless once the
   launcher emitted TaskCells: stale or conflicting typed identity evidence is a
   hard review blocker. Receipts remain the authoritative handoff content; the
...
3. **Apply inline to in-flight work.** Identify which lanes are
   ...
   - **Team-decision invalidation check (frozen §4).** If an absorption edit
     changes a participant's scope-defining obligations, dependencies,
     tier/purpose, capability scope, backend, wave structure, concurrency, cost
     posture, or review independence, the prior `guild.team_decision.v1` is
     invalidated for the affected participants: hold their not-yet-dispatched
     lanes, surface a restructured proposal (new version), and obtain a fresh
     approve before those lanes dispatch. Purely additive guidance that changes
     none of the above absorbs without re-approval.
...
- **participant-id SET EQUALITY, not a count**: the approved proposal's participant-id set = the dispatch-receipt id set = the terminal-outcome id set (`guild.team_schedule.v1` invariant; the only omission allowed is `explicitly_removed_by_new_decision`, citing the new decision hash) — a matching count with a swapped or missing id is NOT complete;
