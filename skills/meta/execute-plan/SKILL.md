...
1. **Trigger.** The low-tier agent emits `status: "escalate"` + an `escalate_reason` in its `guild.handoff.v2` envelope (the in-flight dispatch envelope — canonical body at ADR §5, bound by pointer; distinct from and never superseding the frozen `guild.handoff_receipt.v1`). The coordinator **also** triggers on the configured heuristics: uncertainty markers in output (`models.escalationMarkers`) OR — deferred per ADR O-3, not shipping now — anomalously short output. (The "short-output" heuristic is tune-after-build; the deterministic `status: escalate` + uncertainty-marker triggers ship now.)
