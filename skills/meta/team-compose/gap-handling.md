...
- **A · create** — mint the missing artifact before proceeding: a **role** gap invokes `guild:create-specialist`; a **capability/skill** gap invokes `guild:create-skill`. For a role gap, selecting A is explicit human creation authority: the new role does not need prior runs, reflections, or gap records, but it still must pass its interview, boundary scan, applicable boundary-edit evals, and new-artifact paired eval gate. Historical shadow replay runs when an applicable corpus exists; its absence alone does not block a human-requested role. Capability gaps retain `guild:create-skill`'s own mint contract. Per the **v2 DH-3 boundary**, the new agent/skill is minted into the consuming repo's `.guild/agents/<role>.md` / `.guild/skills/<name>/` carrying `derived_from_template: guild.agent_template.v2` / `guild.skill_template.v1` — **never** into the read-only plugin install dir. The freshly-minted artifact is proposed to the user for explicit approval before the phase team finalizes (see Approval shapes); on a prospective gate failure the gap falls back to B/C/D.
...
Exactly **one** of two approval shapes runs before step 4 writes the per-phase team file (`.guild/team/<slug>.<phase>.yaml`), selected by whether Classify surfaced any gap the user resolved with option A (create):
...
- **(ii) One or more specialists must be created.** At least one gap was resolved with option A. The selection itself records human creation authority. Each freshly-minted specialist is **proposed for approval first** — one approval per minted role, surfaced as it returns from `guild:create-specialist`, before that role is added to the roster — **and then** the team constitution is approved as in (i). A minted specialist the user rejects falls back to options B/C/D for that gap; it is **never** silently added to the team.
...
The top-level `backend` field is a **mirror** (for audit/readability) of the snapshot-resolved backend — it is **not** the authority. The execution backend is resolved at run-start intake by `runStartPreflight` and read from the run's resolved-settings snapshot (`readResolvedSettingsSnapshot` → `snapshot.effective.agent_mode`); the team file is composition-only. **Every per-phase file mirrors the SAME intake-resolved backend value — it is never re-resolved per phase** (ADR A4). The team is **uncapped** — the file lists every participant of the approved `guild.team_proposal.v2`, exactly; no entry may be dropped to fit a size target or backend capacity (`SKILL.md ## Hard rules`). Legacy `--team-size N` / `defaults.team.size` are migration-window concurrency *hints* clamped by verified backend capacity, and `allow_larger` is a warned no-op — none of them caps the roster. *(Superseded 2026-07-30, lane T2b: this line previously read "capped at 6 entries per phase … unless `allow_larger: true` is set".)*
