...
| `GUILD_STATUSLINE` | `0` | The status-line script emits no output. See the Guild docs site → `<GUILD_DOCS_URL>/docs/status-line` to wire it in. | `0` (off), `1` (on). Empty string is treated as `0`. |
...
| Status-line wiring | Optional. See the Guild docs site → `<GUILD_DOCS_URL>/docs/status-line` to wire the new script. |
...
| Enable the status-line | `GUILD_STATUSLINE=1 /guild "<brief>"` (then wire `~/.claude/settings.json` per the Guild docs site → `<GUILD_DOCS_URL>/docs/status-line`) |
