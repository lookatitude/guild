...
### Known issue + open backlog
37. **Removed-flag parser leftovers (`--loops` / `--loop-cap` / `--codex-cap`)** — docs declare these deleted from the CLI, but `read-guild-config.ts:534-541` still silently accepts them (plus 3 doc/skill references). Operator-dispositioned 2026-06-06 as a filed plugin bug needing a direction decision (remove parser vs un-deprecate); ship as a known issue. *( `.guild/initiatives/active/website-content-from-docs/FINDINGS-plugin-flag-removal-incomplete.md`)*
